...
//!
//! ```rust,ignore
//! use aspen_secrets::identity::NodeIdentityProvider;
//!
...
impl NodeIdentityProvider {
    ...
    /// # Errors
    ///
    /// Returns `SecretsError::ParseSigningKey` if the hex string is invalid
    ...
    pub fn from_hex(hex: &str) -> Result<Self, SecretsError> {
        ...
        if hex.len() != SECRET_KEY_HEX_LENGTH {
            return Err(SecretsError::ParseSigningKey {
                reason: format!(
        ...
        let bytes = ::hex::decode(hex).map_err(|e| SecretsError::ParseSigningKey {
    ...
    pub async fn load_or_generate(path: &Path) -> Result<Self, SecretsError> {
        if path.exists() {
            let contents = tokio::fs::read_to_string(path).await.map_err(|e| SecretsError::LoadIdentity {
        ...
        if let Some(parent) = path.parent() {
            tokio::fs::create_dir_all(parent).await.map_err(|e| SecretsError::LoadIdentity {
                path: path.to_path_buf(),
        ...
        tokio::fs::write(path, hex_key.as_bytes()).await.map_err(|e| SecretsError::LoadIdentity {
            path: path.to_path_buf(),
