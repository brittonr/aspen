//! JSON format: encrypt/decrypt individual values in a JSON document.
//!
//! Uses `serde_json` to walk the JSON tree, encrypting/decrypting
//! leaf values in-place. Preserves JSON structure (objects, arrays).
...
/// Encrypt all leaf values in a JSON document.
///
/// Walks the JSON tree, encrypting string/number/bool values to
    ...
    if let Value::Object(map) = root {
        ...
        for key in keys {
            ...
            if let Some(item) = map.get_mut(&key) {
                encrypt_value_recursive(item, &key, data_key, regex.as_ref(), &mut values, &mut count)?;
            }
        }
    }
...
pub fn decrypt_json_values(root: &mut Value, data_key: &[u8; 32]) -> Result<Vec<(String, String)>> {
    ...
    if let Value::Object(map) = root {
        ...
        for key in keys {
            ...
            if let Some(item) = map.get_mut(&key) {
                decrypt_value_recursive(item, &key, data_key, &mut values)?;
            }
        }
    }
...
/// Parse JSON, encrypt values, inject metadata, and serialize.
///
...
pub fn encrypt_document(
    ...
    let mut root: Value = serde_json::from_str(contents).map_err(|e| SopsError::ParseFile {
        path: input_path.to_path_buf(),
        reason: e.to_string(),
    })?;

    let values = encrypt_json_values(&mut root, data_key, encrypted_regex)?;
    inject_metadata(&mut root, metadata)?;

    let output = serde_json::to_string_pretty(&root).map_err(|e| SopsError::Serialization {
...
/// Parse JSON, decrypt values, remove metadata, and serialize.
///
...
pub fn decrypt_document(
    ...
    let values = decrypt_json_values(&mut root, data_key)?;
...
/// Extract SOPS metadata from JSON contents.
pub fn extract_metadata_from_contents(
    contents: &str,
    input_path: &std::path::Path,
) -> Result<Option<SopsFileMetadata>> {
    let root: Value = serde_json::from_str(contents).map_err(|e| SopsError::ParseFile {
        path: input_path.to_path_buf(),
        reason: e.to_string(),
    })?;
    ...
    let metadata: SopsFileMetadata =
        serde_json::from_value(sops_value.clone()).map_err(|e| SopsError::InvalidMetadata {
            reason: format!("invalid sops metadata in JSON: {e}"),
        })?;

    Ok(Some(metadata))
}

/// Inject SOPS metadata into a JSON document.
pub fn inject_metadata(root: &mut Value, metadata: &SopsFileMetadata) -> Result<()> {
    let sops_value = serde_json::to_value(metadata).map_err(|e| SopsError::Serialization {
        reason: format!("failed to serialize SOPS metadata to JSON: {e}"),
    })?;

    if let Value::Object(map) = root {
        map.insert("sops".to_string(), sops_value);
    }
    Ok(())
}

/// Update metadata in an existing JSON document (for rotate/updatekeys).
pub fn update_metadata_in_document(
    ...
    serde_json::to_string_pretty(&root).map_err(|e| SopsError::Serialization {
        reason: format!("failed to serialize JSON: {e}"),
    })
}

/// Extract a single value from decrypted JSON by dotted path.
pub fn extract_value(json_str: &str, path: &str) -> Result<String> {
    let root: Value = serde_json::from_str(json_str).map_err(|e| SopsError::ParseFile {
...
fn encrypt_value_recursive(
    ...
    match value {
        ...
        Value::Null => {
            // Null values are not encrypted but collected for MAC
    ...
    match value {
        ...
        Value::String(s) => {
            if is_sops_encrypted(s) {
                ...
                *value = restore_typed_json_value(&plaintext, &value_type);
            } else {
                // Unencrypted string — collect for MAC
                values.push((path.to_string(), s.clone()));
            }
        }
        Value::Number(n) => {
            // Unencrypted number — collect for MAC
            values.push((path.to_string(), n.to_string()));
        }
        Value::Bool(b) => {
            // Unencrypted bool — collect for MAC
...
/// Restore a typed JSON value from a plaintext string and type tag.
fn restore_typed_json_value(plaintext: &str, value_type: &str) -> Value {
    match value_type {
        "int" => plaintext
            .parse::<i64>()
            .map(|n| Value::Number(serde_json::Number::from(n)))
            .unwrap_or_else(|_| Value::String(plaintext.to_string())),
        "float" => serde_json::Number::from_f64(plaintext.parse::<f64>().unwrap_or(0.0))
            .map(Value::Number)
            .unwrap_or_else(|| Value::String(plaintext.to_string())),
...
mod tests {
    ...
    #[test]
    fn test_encrypt_decrypt_json_document() {
        let key = [42u8; 32];
        let json_str = r#"{
            "api_key": "sk-test-123",
            "port": 8080,
        ...
        let mut root: Value = serde_json::from_str(json_str).unwrap();

        // Encrypt
        let values = encrypt_json_values(&mut root, &key, None).unwrap();
        assert_eq!(values.len(), 5);

        let encrypted_str = serde_json::to_string_pretty(&root).unwrap();
        ...
        assert_eq!(root2["api_key"], "sk-test-123");
        assert_eq!(root2["database"]["password"], "s3cret");
    }
    ...
    fn test_type_preservation() {
        ...
        let mut root: Value = serde_json::from_str(json_str).unwrap();
        let _values = encrypt_json_values(&mut root, &key, None).unwrap();
        ...
        let mut root2 = root;
        let _values = decrypt_json_values(&mut root2, &key).unwrap();
    ...
    fn test_encrypted_regex_filter() {
        ...
        let mut root: Value = serde_json::from_str(json_str).unwrap();
        let _values = encrypt_json_values(&mut root, &key, Some("key|password")).unwrap();
    ...
    fn test_metadata_injection_extraction() {
        ...
        let mut root = serde_json::json!({"secret": "value"});
        inject_metadata(&mut root, &meta).unwrap();

        let output = serde_json::to_string_pretty(&root).unwrap();
        assert!(output.contains("\"sops\""));

        let extracted = extract_metadata_from_contents(&output, std::path::Path::new("test.json")).unwrap().unwrap();
    ...
    fn test_extract_value_dotted_path() {
        let json_str = r#"{"database": {"host": "localhost", "port": 5432}}"#;

        assert_eq!(extract_value(json_str, "database.host").unwrap(), "localhost");
        assert_eq!(extract_value(json_str, "database.port").unwrap(), "5432");
    }

    #[test]
    fn test_encrypt_decrypt_document_roundtrip() {
        let key = [42u8; 32];
        let json_str = r#"{"api_key": "sk-123", "port": 8080}"#;
        let meta = SopsFileMetadata::new();

        let (encrypted, values) =
            encrypt_document(json_str, &key, None, &meta, std::path::Path::new("test.json")).unwrap();
