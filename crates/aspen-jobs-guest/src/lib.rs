...
use alloc::vec::Vec;
// Allocator and heap are only needed for the real guest binary, not tests.
// In test mode, std provides the allocator and panic handler.
#[cfg(not(test))]
...
/// No-op in test mode; std provides the allocator.
#[cfg(test)]
pub fn init_heap() {}

// Panic handler for no_std — only in non-test builds (std provides its own in test mode)
#[cfg(not(test))]
#[panic_handler]
...
mod tests {
    ...
    #[test]
    fn job_output_success() {
        let output = JobOutput::success(json!({"key": "value"}));
        assert!(output.is_success);
        assert_eq!(output.data, json!({"key": "value"}));
        assert!(output.error.is_none());
    }

    #[test]
    fn job_output_failure() {
        let output = JobOutput::failure(String::from("something broke"));
        assert!(!output.is_success);
    ...
    fn job_output_serializes() {
        let output = JobOutput::success(json!(42));
        ...
        assert_eq!(parsed["is_success"], json!(true));
        assert_eq!(parsed["data"], json!(42));
    }

    #[test]
    fn job_input_deserializes() {
        let raw = json!({
            "data": [104, 101, 108, 108, 111],
            "config": {"timeout": 30}
        });
        let input: JobInput = serde_json::from_value(raw).expect("deserialize");
        assert_eq!(input.data, b"hello");
        assert!(input.config.is_some());
    }

    #[test]
    fn job_input_optional_config() {
        let raw = json!({"data": []});
        let input: JobInput = serde_json::from_value(raw).expect("deserialize");
