...
//!    in `crates/aspen-coordination/verus/` are verified separately.
//!
//! # Usage
//!
...
//!
//! fn try_acquire(&self) -> Result<LockGuard, Error> {
//!     ghost! {
...
//!         assert(lock_invariant(post_state));
//!     }
//! }
//! ```
//!
...
/// ```rust,ignore
/// proof! {
///     acquire_preserves_fencing_monotonicity(pre, post);
///     assert(lock_invariant(post_state));
/// }
/// ```
#[macro_export]
...
mod tests {
    ...
    fn test_spec_view_compiles() {
        ...
        let holder: String = "node-1".to_string();
        holder.view();
