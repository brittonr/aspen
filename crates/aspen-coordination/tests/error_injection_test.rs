...
use aspen_testing::{DeterministicKeyValueStore, FailingKeyValueStore};
...
async fn test_rate_limiter_fail_open_on_not_leader() {
    ...
    failing
        .set_write_error("not leader; current leader: Some(2); forward")
        .await;
    ...
    assert!(
        result.is_ok(),
        "Rate limiter should fail-open on NotLeader, got: {:?}",
        result
    );
}
...
async fn test_counter_propagates_write_errors() {
    ...
    assert!(
        result.is_err(),
        "Counter should propagate write errors, got: {:?}",
        result
    );
}
...
async fn test_counter_propagates_read_errors() {
    ...
    assert!(
        result.is_err(),
        "Counter should propagate read errors, got: {:?}",
        result
    );
}
...
async fn test_sequence_propagates_write_errors() {
    ...
    assert!(
        result.is_err(),
        "Sequence should propagate write errors during batch reservation"
    );
}
...
async fn test_sequence_recovers_after_transient_failure() {
    ...
    assert!(
        id2 > id1,
        "Sequence must maintain monotonicity after recovery: {} should be > {}",
        id2,
        id1
    );
}
...
async fn test_lock_propagates_write_errors() {
    ...
    assert!(
        result.is_err(),
        "Lock should propagate write errors"
    );
}
...
async fn test_lock_propagates_read_errors() {
    ...
    assert!(
        result.is_err(),
        "Lock should propagate read errors"
    );
}
