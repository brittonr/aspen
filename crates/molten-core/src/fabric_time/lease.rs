...
fn capped_exponential_delay(policy: RetryPolicy, attempt: u64) -> u64 {
    assert!(policy.base_delay_ticks > 0);
    assert!(policy.base_delay_ticks <= policy.maximum_delay_ticks);
    if attempt >= u64::from(u64::BITS) {
        return policy.maximum_delay_ticks;
    }
    // The width guard admits the shift. Multiplication checks loss of high bits.
