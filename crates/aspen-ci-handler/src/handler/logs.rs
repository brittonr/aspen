...
pub(crate) async fn handle_get_job_logs(
...
pub(crate) async fn handle_subscribe_logs(
    ctx: &ClientProtocolContext,
    run_id: String,
    job_id: String,
...
pub(crate) async fn handle_get_job_output(
    ctx: &ClientProtocolContext,
    _run_id: String,
    job_id: String,
) -> anyhow::Result<ClientRpcResponse> {
    ...
    async fn resolve_output(_ctx: &ClientProtocolContext, value: Option<&serde_json::Value>) -> (Option<String>, bool) {
        ...
        match output_ref {
            ...
            OutputRef::Blob { hash, .. } => {
                ...
                #[cfg(feature = "blob")]
                {
                    if let Some(ref blob_store) = _ctx.blob_store {
