...
fn control_live_transport_receipt_path(
    envelope_ref: &str,
    operation: &str,
) -> Result<crate::node_state::RelativePath> {
...
fn control_live_send_receipt_path(send_ref: &str) -> Result<crate::node_state::RelativePath> {
    ingress_receipt_leaf(&format!("{}.live-send.receipt.preserves", ref_file_stem(send_ref)))
}

fn control_live_send_retry_receipt_path(retry_ref: &str) -> Result<crate::node_state::RelativePath> {
    ingress_receipt_leaf(&format!(
...
fn control_live_send_duplicate_receipt_path(duplicate_ref: &str) -> Result<crate::node_state::RelativePath> {
    ingress_receipt_leaf(&format!(
...
fn control_live_workflow_receipt_path(workflow_ref: &str) -> Result<crate::node_state::RelativePath> {
    ingress_receipt_leaf(&format!(
...
fn control_live_listener_receipt_path(listener_ref: &str) -> Result<crate::node_state::RelativePath> {
    node_leaf_path(
        CONTROL_SERVICE_DIR,
...
fn control_authority_receipt_path(envelope_ref: &str) -> Result<crate::node_state::RelativePath> {
    ingress_receipt_leaf(&format!(
...
fn ingress_receipt_leaf(leaf: &str) -> Result<crate::node_state::RelativePath> {
...
fn record_strings(value: &preserves::Value<preserves::IOValue>, tag: &str) -> Result<Vec<String>> {
    ...
    let fields = record_value
        ...
        .ok_or_else(|| Failure::invalid_harness(format!("expected <{tag} [...]>")))?;
    let items = fields[0]
        .collect_sequence()
        .ok_or_else(|| Failure::invalid_harness(format!("{tag} must contain a sequence")))?
        .into_owned();
    ...
    for item in items {
        let item = item
            ...
            .ok_or_else(|| Failure::invalid_harness(format!("{tag} sequence contains non-string")))?;
        values.push(item);
    }
    Ok(values)
}
...
fn record_optional_string(value: &preserves::Value<preserves::IOValue>, tag: &str) -> Result<Option<String>> {
    ...
    let fields = record_value
        .collect_simple_record(tag, Some(1))
        .ok_or_else(|| Failure::invalid_harness(format!("expected <{tag} optional>")))?;
    ...
    let some = inner
        ...
        .ok_or_else(|| Failure::invalid_harness(format!("{tag} must contain <some string> or <none>")))?;
    let value = some[0]
        .as_string()
        .map(|value| value.into_owned())
        .ok_or_else(|| Failure::invalid_harness(format!("{tag} <some> must contain a string")))?;
    Ok(Some(value))
}
...
fn record_optional_value(value: &preserves::Value<preserves::IOValue>, tag: &str) -> Result<Option<IoValue>> {
    ...
    let some = inner
        .collect_simple_record("some", Some(1))
        .ok_or_else(|| Failure::invalid_harness(format!("{tag} must contain <some value> or <none>")))?;
...
fn record_optional_u64_string(value: &preserves::Value<preserves::IOValue>, tag: &str) -> Result<Option<u64>> {
    match record_optional_string(value, tag)? {
        Some(value) => value.parse::<u64>().map(Some).map_err(|_| {
            Failure::invalid_harness(format!("{tag} optional value must contain an unsigned integer string"))
        }),
        None => Ok(None),
    }
}
...
fn validate_decision(decision: &str) -> Result<()> {
    if matches!(decision, "pass" | "deny" | "fail") {
        ...
        Err(Failure::invalid_harness(format!("invalid node control decision `{decision}`")))
    }
}

fn validate_live_send_timeout(timeout_ms: u64) -> Result<()> {
    if timeout_ms == 0 {
        return Err(Failure::invalid_harness("node control live send timeout must be positive"));
    }
...
fn validate_live_send_attempts(max_attempts: u64) -> Result<()> {
    if max_attempts == 0 {
        return Err(Failure::invalid_harness("node control live send attempts must be positive"));
    }
...
fn validate_service_tick_limit(max_ticks: u64) -> Result<usize> {
    if max_ticks == 0 {
        return Err(Failure::invalid_harness("node control service max ticks must be positive"));
    }
    ...
    usize::try_from(max_ticks)
        .map_err(|_| Failure::invalid_harness("node control service max ticks does not fit this platform"))
}

fn validate_loop_request_limit(max_requests: u64) -> Result<usize> {
    if max_requests == 0 {
        return Err(Failure::invalid_harness("node control loop max requests must be positive"));
    }
    ...
    usize::try_from(max_requests)
        .map_err(|_| Failure::invalid_harness("node control loop max requests does not fit this platform"))
}
...
fn record_value(value: &preserves::Value<preserves::IOValue>, tag: &str) -> Result<IoValue> {
    ...
    let fields = record_value
        .collect_simple_record(tag, Some(1))
        .ok_or_else(|| Failure::invalid_harness(format!("expected <{tag} value>")))?;
...
fn record_values(value: &preserves::Value<preserves::IOValue>, tag: &str) -> Result<Vec<IoValue>> {
    ...
    let fields = record_value
        .collect_simple_record(tag, Some(1))
        .ok_or_else(|| Failure::invalid_harness(format!("expected <{tag} values>")))?;
    let items = fields[0]
        .collect_sequence()
        .ok_or_else(|| Failure::invalid_harness(format!("{tag} must contain a sequence")))?;
...
fn record_ref_strings(value: &preserves::Value<preserves::IOValue>, tag: &str) -> Result<Vec<String>> {
    ...
    for item in items.iter() {
        let reference = item
            .as_string()
            .map(|value| value.into_owned())
            .ok_or_else(|| Failure::invalid_harness(format!("{tag} entries must be strings")))?;
        validate_ingress_ref(&reference, tag)?;
        refs.push(reference);
    }
    Ok(refs)
...
fn record_u64_string(value: &preserves::Value<preserves::IOValue>, tag: &str) -> Result<u64> {
    record_string(value, tag)?.parse::<u64>().map_err(|error| {
        Failure::invalid_harness(format!("{tag} must contain an unsigned integer string: {error}"))
    })
}
