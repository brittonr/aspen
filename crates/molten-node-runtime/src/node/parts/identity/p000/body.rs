...
pub fn resolve_with_root(config: &Config, root: &crate::node_state::DirectoryView) -> Result<Resolution> {
    ...
    match source_decision.kind {
        IrohSecretSourceDecisionKind::LoadExplicit => {
            ...
            let secret_record = crate::transport_key_record::from_secret_hex(explicit_key)?;
        ...
        IrohSecretSourceDecisionKind::LoadBackend => {
            ...
            let secret_record = crate::transport_key_record::from_secret_hex(backend_key)?;
            let material = derive_endpoint_material(&secret_record, &backend_ref)?;
            finish_resolution(ResolutionInput {
                config,
                root,
        ...
        IrohSecretSourceDecisionKind::GenerateAndPersist => {
            let secret_record = crate::transport_key_record::generate();
