use super::*;
use crate::octet_gate::startup_snapshot::tests::fixture;
use crate::octet_gate::startup_snapshot::tests::repin_test_data;
use crate::test_support::process_workspace;

fn write_fixture(root: &Path, policy_path: &Path) -> String {
    let (mut policy, mut descriptor, members) = fixture();
    let executable = descriptor.cohort.executable_blake3.clone();
...
fn verify_fixture(policy: &Path, root: &Path, executable: &str) -> Result<VerificationReport> {
    let (root, plan) = load_plan_with_measure(policy, root, || Ok(executable.to_owned()))?;
...
fn verification_only_report_never_authorizes_startup() {
    ...
    let executable = write_fixture(&root, &policy);
    let before: Vec<_> = std::fs::read_dir(&root).unwrap().map(|entry| entry.unwrap().file_name()).collect();
    let report = verify_fixture(&policy, &root, &executable).unwrap();
...
fn wrong_runtime_binary_is_rejected_before_member_reads() {
    ...
    std::fs::write(&policy_path, serde_json::to_vec(&policy).unwrap()).unwrap();
    std::fs::write(root.join("bundle.json"), serde_json::to_vec(&descriptor).unwrap()).unwrap();
    assert!(verify_fixture(&policy_path, &root, &"a".repeat(64)).unwrap_err().to_string().contains("Executable"));
}
...
fn verified_fixture_cannot_authorize_lifecycle_startup() {
    ...
    let policy = policy_dir.join("cohort.json");
    let executable = write_fixture(&root, &policy);
    let (cap, plan) = load_plan_with_measure(&policy, &root, || Ok(executable)).unwrap();
    let gate = verified_source_gate(&cap, &plan).unwrap();
    assert!(reject_unapproved_cohort(&cap, &plan).unwrap_err().to_string().contains("real-cohort-not-approved"));
    assert!(admit_startup_source_gate(&policy, &root).is_err());
    ...
    .unwrap_err();
    assert!(error.to_string().contains("startup-evidence-"));
    assert!(!state.exists());
