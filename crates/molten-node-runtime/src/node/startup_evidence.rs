...
fn load_plan(policy_path: &Path, bundle_path: &Path) -> Result<(Dir, EvidencePlan)> {
    load_plan_with_measure(policy_path, bundle_path, measure_current_executable)
}

fn load_plan_with_measure(
    policy_path: &Path,
    bundle_path: &Path,
    measure_executable: impl FnOnce() -> Result<String>,
) -> Result<(Dir, EvidencePlan)> {
    ...
    let executable = measure_executable()?;
...
pub fn admit_startup_source_gate(policy_path: &Path, bundle_path: &Path) -> Result<AdmittedSourceGate> {
    let (root, plan) = load_plan(policy_path, bundle_path)?;
    reject_unapproved_cohort(&root, &plan)
}

fn reject_unapproved_cohort(root: &Dir, plan: &EvidencePlan) -> Result<AdmittedSourceGate> {
...
fn verified_source_gate(root: &Dir, plan: &EvidencePlan) -> Result<AdmittedSourceGate> {
    ...
    let gate = crate::octet_gate::startup_snapshot::evaluate(crate::octet_gate::startup_snapshot::Snapshot {
        plan,
        members: &members,
    })?;
