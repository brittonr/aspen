pub(crate) fn init(input: super::command::base::Init) -> molten_node_runtime::error::Result<()> {
    ...
    let init = if let Some(profile_ref) = profile_ref {
        ...
        let overrides = molten_node_runtime::node_profile_config::NodeProfileOverrides {
            state_root_ref: override_state_root_ref,
            adapters: None,
            policy_refs: None,
        };
        molten_node_runtime::node_daemon::init_with_profile(&molten_node_runtime::node_daemon::ProfileInitInput {
    ...
    if let Some(path) = config_out.as_ref() {
        super::core::write_file(path, &molten_node_runtime::preserves_rail::to_text(&init.config_value)?)?;
    }
    if let Some(path) = identity_receipt_out.as_ref() {
        super::core::write_file(path, &molten_node_runtime::preserves_rail::to_text(&init.identity_receipt_value)?)?;
    }
    if let Some(path) = profile_resolution_out.as_ref() {
        super::core::write_file(path, &molten_node_runtime::preserves_rail::to_text(&init.profile_resolution_value)?)?;
    }
    println!(
...
fn checked_node_profile_from_cli(
...
) -> molten_node_runtime::error::Result<molten_node_runtime::node_profile_config::CheckedNodeProfile> {
    ...
    let profile_state_root_ref = input.profile_state_root_ref.ok_or_else(|| {
        molten_node_runtime::error::Failure::invalid_harness("--profile-state-root-ref is required with --profile-ref")
    })?;
    let adapters = parse_adapter_profiles(&input.adapter_profiles)?;
    Ok(molten_node_runtime::node_profile_config::CheckedNodeProfile {
...
) -> molten_node_runtime::error::Result<Option<molten_node_runtime::node_daemon::StartupEvidencePaths<'a>>> {
    match (policy, bundle) {
        (Some(policy), Some(bundle)) => Ok(Some(molten_node_runtime::node_daemon::StartupEvidencePaths { policy, bundle })),
        (None, None) => Ok(None),
        _ => Err(molten_node_runtime::error::Failure::invalid_harness(
            "node startup evidence requires both --startup-policy and --startup-bundle",
        )),
    }
}

fn parse_adapter_profiles(values: &[String]) -> molten_node_runtime::error::Result<Vec<molten_node_runtime::node_runtime::NodeAdapterBinding>> {
    let mut adapters = Vec::with_capacity(values.len());
    for value in values {
        let (name, profile_ref) = value.split_once('=').ok_or_else(|| {
            molten_node_runtime::error::Failure::invalid_harness("--adapter-profile must use name=blake3:<hash>")
        })?;
        adapters.push(molten_node_runtime::node_runtime::node_adapter_binding(name, profile_ref)?);
    }
    Ok(adapters)
}

pub(crate) fn run(input: super::command::base::Run) -> molten_node_runtime::error::Result<()> {
    ...
    let run = molten_node_runtime::node_daemon::run_local(&molten_node_runtime::node_daemon::RunInput {
        state_root: &state_root,
        startup_evidence,
    })?;
    if let Some(path) = startup_out.as_ref() {
        super::core::write_file(path, &molten_node_runtime::preserves_rail::to_text(&run.startup_value)?)?;
    }
    println!(
...
pub(crate) fn run_loop(input: super::command::base::RunLoop) -> molten_node_runtime::error::Result<()> {
    ...
    let loop_run = molten_node_runtime::node_daemon::run_control_loop(&molten_node_runtime::node_daemon::ControlLoopInput {
        state_root: &state_root,
        max_requests,
    })?;
    if let Some(path) = heartbeat_out.as_ref() {
        super::core::write_file(path, &molten_node_runtime::preserves_rail::to_text(&loop_run.heartbeat_receipt_value)?)?;
    }
...
pub(crate) fn serve(input: super::command::base::Serve) -> molten_node_runtime::error::Result<()> {
    ...
    if startup_evidence.is_some() && (live_iroh || content_config.is_none()) {
        return Err(molten_node_runtime::error::Failure::invalid_harness(
    ...
    if live_iroh {
        ...
        let request = molten_node_runtime::node_daemon::ControlServeInput {
            state_root: &state_root,
            topic: &topic,
            max_ticks,
            max_requests_per_tick,
        ...
        let served = if let Some(path) = content_config {
            ...
            let config = serde_json::from_slice(&bytes).map_err(|error| {
                molten_node_runtime::error::Failure::invalid_harness(format!("node content config: {error}"))
            })?;
            let result = molten_node_runtime::node_daemon::serve_control_content(
                &request,
                config,
                molten_node_runtime::preserves_rail::content_ref_from_bytes(&bytes),
                startup_evidence,
            )?;
            if result.decision != "pass" {
                return Err(molten_node_runtime::error::Failure::invalid_harness("node content service lifecycle denied"));
            }
            result
        } else {
            molten_node_runtime::node_daemon::serve_control(&request)?
        };
...
fn serve_live(
    ...
    let runtime = tokio::runtime::Builder::new_multi_thread()
        .enable_all()
        .build()
        .map_err(molten_node_runtime::error::Failure::from)?;
    let served = runtime.block_on(molten_node_runtime::node_daemon::serve_control_live_listener(
        &molten_node_runtime::node_daemon::ControlLiveServeInput {
            state_root: &state_root,
            topic: &topic,
    ...
    if let Some(path) = service_receipt_out.as_ref() {
        super::core::write_file(path, &molten_node_runtime::preserves_rail::to_text(&served.service.service_receipt_value)?)?;
    }
    if let Some(path) = live_ticket_out.as_ref()
        && let Some(ticket_value) = served.live_ticket_value.as_ref()
    {
        super::core::write_file(path, &molten_node_runtime::preserves_rail::to_text(ticket_value)?)?;
    }
    super::core::emit_named_receipt(
        receipt_out.as_ref(),
...
pub(crate) fn status(input: super::command::base::Status) -> molten_node_runtime::error::Result<()> {
    let super::command::base::Status {
        state_root,
        health_out,
        receipt_out,
    } = input;
    let status = molten_node_runtime::node_daemon::status_local(&molten_node_runtime::node_daemon::StatusInput {
        state_root: &state_root,
    })?;
    if let Some(path) = health_out.as_ref() {
        super::core::write_file(path, &molten_node_runtime::preserves_rail::to_text(&status.health_value)?)?;
    }
...
pub(crate) fn stop(input: super::command::base::Stop) -> molten_node_runtime::error::Result<()> {
    let super::command::base::Stop {
        state_root,
        shutdown_out,
        receipt_out,
    } = input;
    let stop = molten_node_runtime::node_daemon::stop_local(&molten_node_runtime::node_daemon::StopInput {
        state_root: &state_root,
    })?;
    if let Some(path) = shutdown_out.as_ref() {
        super::core::write_file(path, &molten_node_runtime::preserves_rail::to_text(&stop.shutdown_value)?)?;
    }
...
pub(crate) fn show(input: super::command::base::Show) -> molten_node_runtime::error::Result<()> {
    let value = super::core::read_preserves_file(&input.artifact)?;
    println!("{}", molten_node_runtime::node_daemon::summary(&value)?);
    Ok(())
}
