pub(crate) fn run(input: super::super::command::live::Apply) -> molten_node_runtime::error::Result<()> {
...
impl Loaded {
    fn read(input: &super::super::command::live::Apply) -> molten_node_runtime::error::Result<Self> {
...
fn execute(
    ...
    loaded: &Loaded,
) -> molten_node_runtime::error::Result<molten_node_runtime::node_daemon::ControlLiveWorkflowBundleApply> {
    ...
    let topology_profile = input
        ...
        .as_deref()
        .map(|profile_ref| {
            Ok::<_, molten_node_runtime::error::Failure>(molten_node_runtime::node_daemon::LiveTopologyProfile {
                profile_ref,
                expected_node: input.expected_node.as_deref().ok_or_else(|| {
                    molten_node_runtime::error::Failure::invalid_harness("--topology-profile-ref requires --expected-node")
                })?,
                expected_peer: input.from_peer.as_deref().or(input.expected_peer.as_deref()).ok_or_else(|| {
                    molten_node_runtime::error::Failure::invalid_harness(
                        "--topology-profile-ref requires --from-peer or --expected-peer",
                    )
                })?,
                expected_topic: input.expected_topic.as_deref().ok_or_else(|| {
                    molten_node_runtime::error::Failure::invalid_harness("--topology-profile-ref requires --expected-topic")
                })?,
    ...
    runtime.block_on(molten_node_runtime::node_daemon::apply_control_live_workflow_bundle(
        &molten_node_runtime::node_daemon::ControlLiveWorkflowBundleApplyInput {
...
fn write_outputs(
    input: &super::super::command::live::Apply,
    applied: &molten_node_runtime::node_daemon::ControlLiveWorkflowBundleApply,
