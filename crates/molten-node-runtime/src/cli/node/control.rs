...
pub(crate) fn submit(input: super::command::control::Submit) -> molten_node_runtime::error::Result<()> {
    ...
    let submitted = molten_node_runtime::node_daemon::submit_control_request(&molten_node_runtime::node_daemon::ControlSubmitInput {
...
pub(crate) fn dispatch(input: super::command::control::Dispatch) -> molten_node_runtime::error::Result<()> {
    let super::command::control::Dispatch {
        state_root,
        request,
        receipt_out,
    } = input;
    let dispatched = molten_node_runtime::node_daemon::dispatch_control_request(&molten_node_runtime::node_daemon::ControlDispatchInput {
        state_root: &state_root,
...
pub(crate) fn deny(input: super::command::control::Deny) -> molten_node_runtime::error::Result<()> {
    ...
    let request_value = super::core::read_preserves_file(&request)?;
    let request = molten_node_runtime::node_runtime::parse_control_request(&request_value)?;
    let receipt = molten_node_runtime::node_runtime::control_deny_receipt_value(&request, &startup, &diagnostic)?;
    super::core::emit_named_receipt(receipt_out.as_ref(), "node control receipt", &receipt)?;
    println!("node control deny receipt={}", molten_node_runtime::preserves_rail::canonical_hash(&receipt)?);
    Ok(())
}
