...
fn valid_test_node_address(seed: u8) -> NodeAddress {
    ...
    NodeAddress::from_parts(
        endpoint_id,
        [NodeTransportAddr::Ip(SocketAddr::from(([127, 0, 0, 1], 8000u16.saturating_add(u16::from(seed)))))],
    )
}

#[test]
fn test_std_wrappers_work() {
    ...
    let parsed = AspenHookTicket::deserialize(&serialized).expect("std wrapper deserialize should succeed");
