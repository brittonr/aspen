...
impl WorkerService {
    /// Validate worker service configuration.
    fn new_validate_config(config: &WorkerConfig) -> Result<()> {
    ...
    /// Create distributed pool configuration.
    fn new_build_distributed_config(
        node_id: u64,
        iroh_node_id: iroh::PublicKey,
        config: &WorkerConfig,
    ...
    pub fn new(
        ...
        Self::new_validate_config(&config)?;
        ...
        let (distributed_pool, distributed_router) = if config.enable_distributed {
            let distributed_config = Self::new_build_distributed_config(node_id, iroh_node_id, &config);
