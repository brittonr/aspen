...
async fn bootstrap_base_node(config: &NodeConfig) -> Result<BaseNodeResources> {
    ...
    let iroh_config = super::network_init::build_iroh_config_from_node_config(config)
        .context("failed to build Iroh configuration")?;
    let iroh_manager =
    ...
    let peer_addrs =
        super::network_init::parse_peer_addresses(&config.peers).context("failed to parse peer addresses")?;
...
async fn create_shard_raft_instance(
    ...
    // SAFETY: This transmute between `Raft<AppTypeConfig>` (from aspen-raft) and
    // `Raft<TransportAppTypeConfig>` (from aspen-transport) is safe because:
    // 1. Both types are generated by the same `openraft::declare_raft_types!` macro with identical type
    //    parameters (D=AppRequest, R=AppResponse, NodeId, Node).
    // 2. Compile-time static assertions in `aspen_raft::types::_transmute_safety_static_checks` verify
    //    both Raft handle types have identical size and alignment.
    // 3. The aspen-transport crate re-exports the same AppTypeConfig to maintain a single source of
    //    truth.
    // 4. If the types ever diverge, the static_assertions crate will cause a compile error, preventing
