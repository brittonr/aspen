...
async fn create_shard_raft_instance(
    ...
    // Transport now re-exports the leaf AppTypeConfig, so the Raft handle
    // can cross this boundary without any unsafe conversion.
    let transport_raft: openraft::Raft<TransportAppTypeConfig> = raft.as_ref().clone();
