pub struct DirectoryView {
    ...
    pub(super) kind: super::authority::NamespaceKind,
    pub(super) scope: std::path::PathBuf,
    pub(super) dir: cap_std::fs::Dir,
}

impl std::fmt::Debug for DirectoryView {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter.debug_struct("DirectoryView").field("kind", &self.kind).finish_non_exhaustive()
    }
}

impl DirectoryView {
    pub fn open(kind: super::authority::NamespaceKind, path: &std::path::Path) -> crate::error::Result<Self> {
        let root = super::authority::Root::open(path)?;
        Self::from_dir(kind, root.try_clone_dir()?)
    }

    pub fn open_existing(kind: super::authority::NamespaceKind, path: &std::path::Path) -> crate::error::Result<Self> {
        let root = super::authority::Root::open_existing(path)?;
        Self::from_dir(kind, root.try_clone_dir()?)
    }

    pub fn from_dir(kind: super::authority::NamespaceKind, dir: cap_std::fs::Dir) -> crate::error::Result<Self> {
        let root_dir = dir.try_clone().map_err(crate::error::Failure::from)?;
        Ok(Self {
    ...
    pub fn kind(&self) -> super::authority::NamespaceKind {
        self.kind
    }
    ...
    pub fn write_atomic_leaf(&self, path: &super::locator::RelativePath, bytes: &[u8]) -> crate::error::Result<()> {
        use std::io::Write;
        super::filesystem::validate_write_size(bytes, super::MAX_NODE_STATE_FILE_BYTES)?;
        if path.as_path().components().count() != 1 {
            return Err(super::invalid("atomic namespace write requires one leaf"));
        }
        if !matches!(self.entry_kind(path)?, None | Some(super::authority::EntryKind::RegularFile)) {
            return Err(super::invalid("atomic namespace destination must be a regular file"));
        }
        let mut file = cap_tempfile::TempFile::new(&self.dir).map_err(crate::error::Failure::from)?;
        file.write_all(bytes).map_err(crate::error::Failure::from)?;
        file.as_file().sync_all().map_err(crate::error::Failure::from)?;
        file.replace(path.as_path().as_os_str()).map_err(crate::error::Failure::from)
    }
    ...
    pub fn unix_mode(&self, path: &super::locator::RelativePath) -> crate::error::Result<Option<u32>> {
        match self.observe_file(path)? {
            super::authority::FileObservation::Missing => Ok(None),
            super::authority::FileObservation::NonRegular(_) => {
                Err(super::invalid(format!("node state leaf {} must be a regular file", path.display())))
            }
            super::authority::FileObservation::Regular(file) => Ok(file.unix_mode()),
        }
    }
    ...
    pub fn list_entries(&self) -> crate::error::Result<Vec<DirectoryEntry>> {
        super::enumeration::list_entries(&self.dir, &self.root, self.kind, &self.scope)
    }

    pub fn open_subdir(&self, path: &super::locator::RelativePath) -> crate::error::Result<Self> {
    ...
    pub fn read_entry(&self, entry: &DirectoryEntry, max_bytes: u64) -> crate::error::Result<Vec<u8>> {
        self.validate_entry(entry)?;
        if entry.kind != super::authority::EntryKind::RegularFile {
            return Err(super::invalid(format!("node state entry {} must be a regular file", entry.name)));
        }
        self.read(&entry.path, max_bytes)
    }

    pub fn remove_entry(&self, entry: &DirectoryEntry) -> crate::error::Result<()> {
        self.validate_entry(entry)?;
        if entry.kind != super::authority::EntryKind::RegularFile {
            return Err(super::invalid(format!("node state entry {} must be a regular file", entry.name)));
        }
    ...
    pub(crate) fn try_clone_dir(&self) -> crate::error::Result<cap_std::fs::Dir> {
        self.dir.try_clone().map_err(crate::error::Failure::from)
    }

    fn validate_entry(&self, entry: &DirectoryEntry) -> crate::error::Result<()> {
...
pub struct DirectoryEntry {
    pub(super) root: std::sync::Arc<super::authority::RootDirectory>,
    pub(super) namespace: super::authority::NamespaceKind,
    pub(super) scope: std::path::PathBuf,
    pub name: String,
    pub path: super::locator::RelativePath,
    pub kind: super::authority::EntryKind,
}
