use molten_node_host::node_state::{RelativePath, Root};
...
fn atomic_leaf_replaces_whole_file_and_rejects_other_shapes() {
    ...
    let root = Root::from_dir(temp.try_clone().unwrap());
    let ns = root.control_service().unwrap();
    let leaf = RelativePath::parse("status.json").unwrap();
    ...
    let directory = RelativePath::parse("directory").unwrap();
    ns.create_dir_all(&directory).unwrap();
    assert!(ns.write_atomic_leaf(&directory, b"bad").is_err());
    let nested = RelativePath::parse("absent/leaf").unwrap();
    assert!(ns.write_atomic_leaf(&nested, b"bad").is_err());
    assert!(!ns.try_exists(&RelativePath::parse("absent").unwrap()).unwrap());
}
