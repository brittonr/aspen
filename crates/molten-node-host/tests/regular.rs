...
fn regular_writes_and_database_opens_preserve_file_rules() {
    ...
    let leaf = molten_node_host::node_state::RelativePath::parse("nested/value").unwrap();
    ...
    let directory = molten_node_host::node_state::RelativePath::parse("nested").unwrap();
...
fn missing_parents_remain_missing_during_observation() {
    ...
    let root = molten_node_host::node_state::Root::from_dir(temp.try_clone().unwrap());
    let namespace = root.control_service().unwrap();
    let leaf = molten_node_host::node_state::RelativePath::parse("absent/leaf").unwrap();
    assert!(matches!(
        namespace.observe_file(&leaf).unwrap(),
        molten_node_host::node_state::FileObservation::Missing
    ));
...
fn symlink_leaves_and_parents_deny_without_changing_target() {
    ...
    let root = molten_node_host::node_state::Root::from_dir(temp.try_clone().unwrap());
    let namespace = root.control_service().unwrap();
    let target = molten_node_host::node_state::RelativePath::parse("target/value").unwrap();
    ...
    for locator in ["link", "parent/value"] {
        let path = molten_node_host::node_state::RelativePath::parse(locator).unwrap();
