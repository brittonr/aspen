...
impl AspenFs {
    ...
    pub(crate) fn read_metadata(&self, key: &str) -> std::io::Result<Option<FileMetadata>> {
    ...
    fn get_size_and_meta(&self, path: &str, entry_type: EntryType) -> std::io::Result<(u64, Option<FileMetadata>)> {
        ...
        let (size, meta) = match entry_type {
            EntryType::File => {
                let stored_sz = crate::chunking::chunked_size(self, &key)?.unwrap_or(0);
                // Consider buffered writes that may extend the file
                let sz = self.write_buffer.effective_size(&key, stored_sz);
