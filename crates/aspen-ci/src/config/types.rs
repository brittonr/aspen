...
impl From<Priority> for aspen_jobs::Priority {
    ...
    fn check_circular_dependencies(&self) -> crate::error::Result<()> {
        ...
        for stage in &self.stages {
            if !visited.contains(stage.name.as_str()) {
                if let Err(cycle) = visit(&stage.name, &deps, &mut visited, &mut path) {
                    return Err(CiError::CircularDependency { path: cycle });
                }
            }
        }
...
mod tests {
    ...
    fn test_priority_conversion() {
        assert!(matches!(aspen_jobs::Priority::from(Priority::High), aspen_jobs::Priority::High));
        assert!(matches!(aspen_jobs::Priority::from(Priority::Normal), aspen_jobs::Priority::Normal));
        assert!(matches!(aspen_jobs::Priority::from(Priority::Low), aspen_jobs::Priority::Low));
    }
}
