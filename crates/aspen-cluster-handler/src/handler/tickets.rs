...
pub(crate) async fn handle_get_cluster_ticket(ctx: &ClientProtocolContext) -> anyhow::Result<ClientRpcResponse> {
    ...
    let mut added_peers = 1u32;

    // Include other cluster nodes so clients can failover on NOT_LEADER errors.
    // Without multi-peer tickets, a client connecting to a follower gets NOT_LEADER
    ...
    if let Ok(cluster_state) = ctx.controller.current_state().await {
        for node in cluster_state.nodes.iter().take(AspenClusterTicket::MAX_BOOTSTRAP_PEERS) {
            ...
            if let Some(iroh_addr) = node.iroh_addr() {
                // Skip our own endpoint (already added as first peer)
    ...
    debug!(bootstrap_peers = added_peers, "GetClusterTicket: generated ticket with cluster peers");
