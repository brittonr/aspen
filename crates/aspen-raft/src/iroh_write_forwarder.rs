...
fn batch_set_operations_from_pairs(pairs: &[(String, String)]) -> Vec<BatchWriteOperation> {
    pairs
        .iter()
...
fn batch_delete_operations_from_keys(keys: &[String]) -> Vec<BatchWriteOperation> {
    keys.iter().map(|key| BatchWriteOperation::Delete { key: key.clone() }).collect()
}

fn batch_operations_from_batch(operations: &[aspen_kv_types::BatchOperation]) -> Vec<BatchWriteOperation> {
    operations
        .iter()
...
fn write_command_to_rpc_request(
    ...
    match command {
        ...
        WriteCommand::CompareAndSwap {
            ...
            key: key.clone(),
            expected: expected.as_ref().map(|value| value.as_bytes().to_vec()),
        ...
        WriteCommand::SetMulti { pairs } | WriteCommand::SetMultiWithTTL { pairs, .. } => {
            Ok(ClientRpcRequest::BatchWrite {
                operations: batch_set_operations_from_pairs(pairs),
            })
        }
        WriteCommand::DeleteMulti { keys } => Ok(ClientRpcRequest::BatchWrite {
            operations: batch_delete_operations_from_keys(keys),
        }),
        WriteCommand::Batch { operations } => Ok(ClientRpcRequest::BatchWrite {
            operations: batch_operations_from_batch(operations),
        }),
        other => {
            debug!(
        ...
        ClientRpcResponse::CompareAndSwapResult(resp) => {
            if resp.is_success {
                ...
                Err(KeyValueStoreError::CompareAndSwapFailed {
                    key: String::new(),
                    expected: None,
                    actual: resp.actual_value.map(|value| String::from_utf8_lossy(&value).to_string()),
                })
            }
        }
    ...
    async fn get_connection(
        ...
        let connection = timeout(CONNECT_TIMEOUT, self.endpoint.connect(leader_addr.clone(), CLIENT_ALPN))
            ...
            .map_err(|_| KeyValueStoreError::Timeout {
                duration_ms: duration_ms_u64(CONNECT_TIMEOUT),
            })?
        ...
        match result {
            ...
            Err(_timeout) => {
                ...
                Err(KeyValueStoreError::Timeout {
                    duration_ms: duration_ms_u64(FORWARD_TIMEOUT),
                })
            }
        }
    }
