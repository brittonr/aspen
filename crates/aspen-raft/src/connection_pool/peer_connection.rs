...
impl PeerConnection {
    ...
    /// Check connection health before acquiring a stream.
    async fn acquire_stream_check_health(&self) -> Result<()> {
    ...
    /// Acquire a semaphore permit for the stream.
    fn acquire_stream_get_permit(&self) -> Result<tokio::sync::OwnedSemaphorePermit> {
    ...
    /// Handle stream open failure: decrement counter, update health, log transition.
    async fn acquire_stream_handle_failure(&self, err: anyhow::Error) -> Result<StreamHandle> {
        use crate::verified::transition_connection_health;
        ...
        *health = new_health;
        ...
        self.acquire_stream_log_health_transition(old_health, new_health, &err);
        Err(err)
    }

    /// Log health state transitions on stream failure.
    fn acquire_stream_log_health_transition(
        &self,
        old_health: ConnectionHealth,
        new_health: ConnectionHealth,
        err: &anyhow::Error,
    ) {
