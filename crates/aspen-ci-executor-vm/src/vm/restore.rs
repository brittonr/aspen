...
impl ManagedCiVm {
    ...
    async fn restore_inner(&self, snapshot: &GoldenSnapshot) -> Result<()> {
        ...
        *self.virtiofsd_nix_store.write().await = Some(nix_store_virtiofsd);
        ...
        // (fresh Iroh connection, not in snapshot)
        {
            let ticket_str = self.config.get_cluster_ticket().ok_or_else(|| CloudHypervisorError::RestoreFailed {
        ...
        // Remove stale API socket before restore creates it
        ...
        // We need to start cloud-hypervisor with --api-socket and then call vm.restore.
        info!(vm_id = %self.id, source = %snapshot.source_url(), "calling vm.restore");
        ...
        self.api.restore(&snapshot.source_url()).await.map_err(|e| CloudHypervisorError::RestoreFailed {
            path: snapshot.dir.clone(),
