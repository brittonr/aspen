...
fn test_three_node_trust_init_and_reconstruct() {
    ...
    let mut digests: BTreeMap<u64, ShareDigest> = BTreeMap::new();
    for (i, share) in shares.iter().enumerate() {
        digests.insert(node_ids[i], shamir::share_digest(share));
    }

    // 4. Verify each share against its stored digest
    for (i, share) in shares.iter().enumerate() {
        let computed = shamir::share_digest(share);
        let stored = digests.get(&node_ids[i]).unwrap();
    ...
    // All three
    let reconstructed_all = shamir::reconstruct_secret(&shares).unwrap();
    assert_eq!(&reconstructed_all, secret.as_bytes());

    // 6. Single share cannot reconstruct
    let wrong = shamir::reconstruct_secret(&[shares[0].clone()]).unwrap();
...
#[test]
fn test_five_node_trust_any_three_reconstruct() {
    let n: u32 = 5;
    let threshold = Threshold::default_for_cluster_size(n);
    ...
    let wrong = shamir::reconstruct_secret(&[shares[0].clone(), shares[3].clone()]).unwrap();
    assert_ne!(&wrong, secret.as_bytes());
}

#[test]
fn test_single_node_trust() {
    // K=1, N=1: single node holds the full secret
    let secret = ClusterSecret::generate();
    let mut rng = rand::rng();
    let shares = shamir::split_secret(secret.as_bytes(), 1, 1, &mut rng).unwrap();
...
#[test]
fn test_corrupted_share_detected_by_digest() {
    let secret = ClusterSecret::generate();
    let mut rng = rand::rng();
    ...
    let wrong = shamir::reconstruct_secret(&[corrupted, shares[1].clone()]).unwrap();
    assert_ne!(&wrong, secret.as_bytes());
}
