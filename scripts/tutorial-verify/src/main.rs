...
fn run_verification(cli: &Cli) -> Result<bool> {
    ...
    println!("\n{} {}: {}", Color::Cyan.bold().paint("Lab"), cli.lab, get_lab_description(cli.lab));
    ...
    for (work_dir, args) in commands {
        ...
        println!("\n{} {}", Color::Blue.bold().paint("Running:"), cmd_args.join(" "));
        ...
        if !status.success() {
            println!("\n{} Lab {} verification failed", Color::Red.bold().paint("✗"), cli.lab);
            println!("{}", Color::Yellow.paint("Hint: Check your implementation and try again."));
            return Ok(false);
        }
    }

    println!("\n{} Lab {} verified successfully!", Color::Green.bold().paint("✓"), cli.lab);
    ...
    if cli.lab < 6 {
        println!(
            ...
            Color::White
                .dimmed()
                .paint(format!("cargo run --manifest-path scripts/tutorial-verify/Cargo.toml -- {}", cli.lab + 1))
        );
    } else {
        println!("\n{} Congratulations! You've completed all labs!", Color::Green.bold().paint("🎉"));
    }
...
fn watch_loop(cli: &Cli) -> Result<()> {
    ...
    println!(
        "{} Watching for changes in {} (Ctrl+C to exit)...",
