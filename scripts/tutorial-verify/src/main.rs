...
fn run_verification(cli: &Cli) -> Result<bool> {
    ...
    for (work_dir, args) in commands {
        ...
        if !status.success() {
            ...
            println!(
                "{}",
                Color::Yellow.paint("Hint: Check your implementation and try again.")
            );
            return Ok(false);
        }
    }
    ...
    if cli.lab < 6 {
        println!(
            ...
            Color::White.dimmed().paint(format!(
                "cargo run --manifest-path scripts/tutorial-verify/Cargo.toml -- {}",
                cli.lab + 1
            ))
        );
    } else {
...
fn main() -> Result<()> {
    ...
    if !cli.watch {
        ...
        println!(
            ...
            Color::Cyan
                .bold()
                .paint("  Aspen S3 Tutorial - Lab Verifier")
        );
        println!(
            "{}",
            Color::Cyan
