#!/usr/bin/env bash
# Aspen Self-Hosting with Local Nodes (No VMs)
#
# This script runs the Aspen dogfood workflow using local processes instead
# of Cloud Hypervisor VMs. Useful for development and testing.
#
# Usage:
#   nix run .#dogfood-local               # Launch default 3-node cluster
#   ASPEN_NODE_COUNT=1 nix run .#dogfood-local  # Single node
#   nix run .#dogfood-local -- help       # Show help
#
# Environment variables:
#   ASPEN_NODE_COUNT     - Number of nodes (default: 3, max: 10)
#   ASPEN_DATA_DIR       - Data directory (default: /tmp/aspen-dogfood-local)
...
DATA_DIR="${ASPEN_DATA_DIR:-/tmp/aspen-dogfood-local}"
LOG_LEVEL="${ASPEN_LOG_LEVEL:-info}"
COOKIE="dogfood-local-$(date +%s)"
...
check_prereqs() {
    if [ -z "$ASPEN_NODE_BIN" ] || [ ! -x "$ASPEN_NODE_BIN" ]; then
        ...
        printf "Run from flake: nix run .#dogfood-local\n"
        exit 1
    fi
...
init_repo() {
    ...
    if ! output=$("$ASPEN_CLI_BIN" --quiet --ticket "$ticket" git init \
        --description "Aspen distributed systems platform (local dogfood)" \
        "aspen" 2>&1); then
...
cmd_run() {
    ...
    printf "${BLUE}============================================${NC}\n"
    printf "${GREEN}Aspen Self-Hosting (Local)${NC}\n"
    printf "${BLUE}============================================${NC}\n\n"
...
# Show help
cmd_help() {
    printf "Aspen Self-Hosting with Local Nodes\n"
    printf "\n"
    printf "Usage: nix run .#dogfood-local [-- command]\n"
    printf "\n"
    printf "Commands:\n"
    printf "  run [branch]  - ${GREEN}Full workflow${NC} (start nodes, init cluster, push, CI)\n"
    printf "  help          - Show this help message\n"
    printf "\n"
    printf "Environment variables:\n"
    printf "  ASPEN_NODE_COUNT  - Number of nodes (default: 3, max: 10)\n"
    printf "  ASPEN_DATA_DIR    - Data directory (default: /tmp/aspen-dogfood-local)\n"
    printf "  ASPEN_LOG_LEVEL   - Log level (default: info)\n"
    printf "\n"
    printf "Example:\n"
    printf "  nix run .#dogfood-local\n"
    printf "  ASPEN_NODE_COUNT=1 nix run .#dogfood-local\n"
}
