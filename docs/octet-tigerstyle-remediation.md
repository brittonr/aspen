...
Plan ref: `blake3:3fd5a986eb4653db42f4a2173e86a634fc4a1a5206349861c2bab1a1f76cadaa`

Latest Octet receipt refs: artifact import `blake3:fcaa7082c8a843e3ea4256d73347904e9d0bc932da58222f206508f9f2f2925b`; strict gate pass `blake3:8f93a1344aa4f669e49d9e9f355da1f9bc41f90ff6b2ef368d89430be5082235`; remediation plan `blake3:3fd5a986eb4653db42f4a2173e86a634fc4a1a5206349861c2bab1a1f76cadaa`.
...
| workspace/focused | `target/octet/object-corpus-receipt.json` | `blake3:3ac733d2ee6445583d02f2c0dd8e1aefb9af209b8670c76e1b9d220ae6d22570` |
...
| workspace/import | `target/octet/artifact-ledger-receipt.preserves` | `blake3:fcaa7082c8a843e3ea4256d73347904e9d0bc932da58222f206508f9f2f2925b` |
| workspace/gate | `target/octet/gate-receipt.preserves` | `blake3:8f93a1344aa4f669e49d9e9f355da1f9bc41f90ff6b2ef368d89430be5082235` |
| workspace/plan | `target/octet/remediation-plan.preserves` | `blake3:3fd5a986eb4653db42f4a2173e86a634fc4a1a5206349861c2bab1a1f76cadaa` |

Focused object corpus: object-set hash `b3:afcd32a980c36564747c94923704728b2011abd60f7c4fe7f0300a22f3eaade1`, 2863 objects, 2863 pure-cache blocked, source paths include `src/cli_artifact.rs`, `src/cli_cache.rs`, `src/cli_catalog.rs`, `src/cli_chunk.rs`, `src/cli_coordination.rs`, `src/cli_delivery.rs`, `src/cli_dogfood.rs`, `src/cli_job.rs`, `src/cli_ledger.rs`, `src/cli_node.rs`, `src/cli_nixos_vm.rs`, `src/cli_octet.rs`, `src/cli_plugin.rs`, `src/cli_receipts.rs`, `src/cli_prod_soak.rs`, `src/cli_protocol.rs`, `src/cli_provenance.rs`, `src/cli_raft.rs`, `src/cli_remote.rs`, `src/cli_replay_fixture.rs`, `src/cli_report.rs`, `src/cli_repro.rs`, `src/cli_retention.rs`, `src/cli_rewrite.rs`, `src/cli_schema.rs`, `src/cli_secrets.rs`, `src/cli_service.rs`, `src/cli_storage.rs`, `src/cli_transcript.rs`, `src/cli_upgrade.rs`, `src/cli_vat.rs`, `src/prod_soak.rs`, and `src/octet_remediation.rs` plus the critical runtime, node, job, retention, secrets, protocol, catalog, and CLI paths listed in `target/octet/object-corpus-receipt.json`.
...
Active burn-down slice: `cairn/changes/octet-source-remediated-zero` has moved Octet, Delivery, Protocol, Provenance, Retention, Job, Secrets, Plugin, Node, Repro, Catalog, Chunk, Cache, Artifact, Storage, Schema, Upgrade, Transcript, Rewrite, Remote, Ledger, Chain, Receipts, Service, Vat, Coordination, Dogfood, Raft, replay-fixture, and Report CLI command parsing out of `src/main.rs` into `src/cli_octet.rs`, `src/cli_delivery.rs`, `src/cli_protocol.rs`, `src/cli_provenance.rs`, `src/cli_retention.rs`, `src/cli_job.rs`, `src/cli_secrets.rs`, `src/cli_plugin.rs`, `src/cli_node.rs`, `src/cli_prod_soak.rs`, `src/cli_repro.rs`, `src/cli_catalog.rs`, `src/cli_chunk.rs`, `src/cli_cache.rs`, `src/cli_artifact.rs`, `src/cli_storage.rs`, `src/cli_schema.rs`, `src/cli_upgrade.rs`, `src/cli_transcript.rs`, `src/cli_rewrite.rs`, `src/cli_remote.rs`, `src/cli_ledger.rs`, `src/cli_receipts.rs`, `src/cli_service.rs`, `src/cli_vat.rs`, `src/cli_coordination.rs`, `src/cli_dogfood.rs`, `src/cli_raft.rs`, `src/cli_replay_fixture.rs`, and `src/cli_report.rs` while preserving command semantics. The broader state remains configuration-clean until disabled lint families are removed or narrowed and evidence is refreshed.
