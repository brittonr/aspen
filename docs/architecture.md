...
- `signed-evidence-receipts` — signed receipt envelopes that bind canonical evidence bytes to signer identity, key purpose, trust roots, expected subject refs, parent receipt chains, and release bundle member verification while remaining evidence-only.
