...
- `operator-dogfood-nix-release-check` — Nix release check that depends on hermetic nextest, runs `molten dogfood local-node`, preserves report/gate artifacts, and now emits verified Nix dogfood evidence binding the output path, release-gate ref, and nextest marker as evidence-only review outputs.
