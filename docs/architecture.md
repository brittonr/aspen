...
- `node-control-live-send-ux` — external `control-ingress-live-send` workflow that uses bound live tickets to join real Iroh gossip topics, publish canonical ingress bytes, and record send/transport plus live workflow runbook receipts without treating transport as authority.
