...
The first local vat fixture is available as `molten test vat run-fixture --out target/vat.preserves` and `molten test vat show target/vat.preserves`. It emits canonical `<vat-fixture-run-v1 "molten.runtime.vat-fixture-run.v1" ...>` evidence that binds Trellis runtime predicate receipts for near/far call admission, actormap commit/rollback, bounded promise pipelining, and revocation cleanup. `molten test vat snapshot-fixture --out target/vat-snapshot.preserves` emits `<vat-snapshot-v1 "molten.runtime.vat-snapshot.v1" ...>` evidence that binds snapshot authority predicates, proving a snapshot cannot claim authority refs outside the held/admitted authority graph. `molten test vat restore-fixture --out target/vat-restore.preserves` emits an admitted schema-rename upgrade recipe plus a missing-recipe denial so restored object schemas remain explicit and deterministic. `molten test vat promise-fixture --out target/vat-promise.preserves` records pending-to-resolved, pending-to-broken, cancellation, timeout, terminal-mutation denial, and stale-pipeline-cleanup evidence for far-call promises/vows. `molten test vat ambient-authority-fixture --out target/vat-authority.preserves` records deny-by-default filesystem, network, clock, process, dataspace, store, blob, consensus, choreography, and host-resource authority attempts for a newly spawned object, plus an explicit endowed/admitted clock pass. `molten test vat rights-fixture --out target/vat-rights.preserves` records private sealer/unsealer rights amplification: a matching brand can recover sealed authority, while a wrong brand or over-broad recovery is denied. `molten test vat distributed-ref-fixture --out target/vat-distributed-ref.preserves` records session-scoped far-reference lifetime evidence: live session use, disconnect cleanup, admitted handoff replacement, stale descriptor denial, and pending-call failure requirements. The fixtures are local runtime evidence only; they do not claim Goblins, OCapN, or CapTP compatibility and do not grant transport, authority, policy, resource, provenance, or persistence trust.
...
Remote SAM dataspace traffic uses canonical `<remote-dataspace-envelope-v1 "molten.remote-dataspace.envelope.v1" ...>` records for message/assert/retract/observe actions and `<remote-dataspace-transport-receipt-v1 "molten.remote-dataspace.transport-receipt.v1" ...>` evidence for publish/deliver/deny decisions. The first implementation is a deterministic `iroh-local-gossip` adapter that stores canonical envelope bytes under an Iroh-shaped local transport root, validates declared blob refs before delivery, and applies delivered envelopes through the local runtime turn boundary. Live `iroh-gossip` publication uses the same envelope and receipt boundary; recorded delivery logs (`<remote-dataspace-delivery-log-v1 ...>`) are required for deterministic replay, and pass evidence can be summarized with `<remote-dataspace-gate-receipt-v1 ...>` binding envelope, transport, bootstrap, authority, resource, and turn-journal refs. Iroh endpoint/topic membership remains transport evidence, not actor authority. Delivered remote assertions are owned by the receiving session scope: admission resolves the declared owner session against a receiver-side session registry (`<remote-dataspace-session-v1 ...>` identities bound to receiver peer, topic, and generation), applies the assertion with that session scope as its runtime owner, and records the owning session ref in a canonical `<remote-dataspace-applied-assertion-v1 ...>` record so the owner is readable from evidence. An envelope whose declared owner is unknown or belongs to a closed session denies before staging, so cleanup cannot be undone by a late delivery. Session close or disconnect runs the normal owner-scope cleanup path for the session scope (lifecycle `<lifecycle-scope-cleanup-v1 ...>` receipt with `session` entity kind): session-owned assertions retract with matching observers notified, session-owned observers and pending messages drop together, and the session identity cannot be reused afterwards. Reconnection requires the peer to assert again under a fresh session identity; replaying a closed session's delivery log stays diagnostic and never resurrects assertions. Molten makes no delivery-completeness claim: transport receipts bound published/delivered bytes, not that every admitted delivery reached every peer.

The direct Iroh protocol boundary is modeled as admitted router and framed-envelope evidence before it becomes a long-running service. `<iroh-protocol-router-receipt-v1 "molten.node.iroh-protocol-router-receipt.v1" ...>` records generationed ALPN install, replacement, removal, shutdown, stale-generation denial, and unsupported-ALPN denial decisions. `<iroh-framed-envelope-receipt-v1 "molten.node.iroh-framed-envelope-receipt.v1" ...>` records bounded bidirectional stream frame admission over canonical Preserves bytes, declared envelope refs, actual refs, peer/node ids, sequence, ALPN, and limit-profile evidence. `<iroh-stream-session-receipt-v1 "molten.node.iroh-stream-session-receipt.v1" ...>` lets local and remote unary or streaming service interactions share the same canonical request/response model while remote paths additionally bind frame receipts. These receipts follow the useful `iroh-examples` custom-router/framed-message and `irpc` interaction patterns only as references; Molten does not claim API or wire compatibility, and transport receipts do not grant authority, policy, resource, provenance, source-gate, retention, execution, or deterministic replay trust.

Network diagnostics are evidence-only observations. `<network-diagnostics-report-v1 "molten.node.network-diagnostics-report.v1" ...>`, `<network-connectivity-probe-receipt-v1 "molten.node.network-connectivity-probe-receipt.v1" ...>`, `<network-port-mapping-receipt-v1 "molten.node.network-port-mapping-receipt.v1" ...>`, `<network-watcher-snapshot-v1 "molten.node.network-watcher-snapshot.v1" ...>`, `<metrics-snapshot-receipt-v1 "molten.node.metrics-snapshot-receipt.v1" ...>`, and `<external-diagnostics-bridge-receipt-v1 "molten.node.external-diagnostics-bridge-receipt.v1" ...>` bind NAT/UDP/direct/relay observations, connectivity probes, latest-state route/interface summaries, bounded redacted metrics, deny-by-default port-mapping decisions, and optional external diagnostics bridge admissions. Live-only observations must be marked non-replayable unless separately recorded, metrics labels are bounded/redacted, port mapping mutates only with explicit authority/policy/resource/operator evidence, and external service bridges stay disabled by default.

Operator gateway readback is read-only review UX over existing content-addressed artifacts. `<operator-gateway-read-receipt-v1 "molten.operator.gateway-read-receipt.v1" ...>`, `<operator-gateway-range-receipt-v1 "molten.operator.gateway-range-receipt.v1" ...>`, and `<operator-gateway-index-receipt-v1 "molten.operator.gateway-index-receipt.v1" ...>` bind read/index/range decisions, visibility policy refs, retention/redaction/reveal refs, normalized ranges, required chunk refs, and diagnostics. Every exposed byte range is mapped to chunk manifests and verified chunk refs before bytes are returned; protected commitments deny without reveal evidence, public indexes omit or redact hidden members, and gateway receipts cannot authorize pin, unpin, delete, install, execute, policy mutation, or any other side effect. The `iroh-gateway` range/index/MIME patterns are design references only, not Molten's trust model.
...
Retention and GC are explicit evidence boundaries. `<retention-class-v1 "molten.retention.class.v1" ...>` records declare retention classes such as ephemeral cache, debug trace, replay snapshot, audit receipt, durable value, public artifact, private secret ref, upgrade rollback, and legal hold. `<retention-pin-v1 "molten.retention.pin.v1" ...>` binds object refs to sources, owners, reasons, expiry refs, policy refs, and evidence refs; mutable names or absent aliases never prove deletion eligibility. Before a destructive action, Molten builds a bounded `<retention-reference-index-v1 "molten.retention.reference-index.v1" ...>` over active pins, retained receipt dependencies, tombstones, remote cache refs, and evaluation cache refs. Operators can inspect an object with `<retention-candidate-explain-v1 "molten.retention.candidate-explain.v1" ...>`, a read-only discovery artifact that lists known pins, evidence admissions, remote clearances/imports, GC plans, applies, executions, audits, receipts, and tombstones without mutating retained content or granting deletion authority. `<retention-candidate-bundle-v1 "molten.retention.candidate-bundle.v1" ...>` packages the explain artifact plus referenced local plan/apply/execute/audit/receipt/tombstone artifacts into a review handoff directory while remaining evidence-only. `<retention-candidate-bundle-profile-v1 "molten.retention.candidate-bundle-profile.v1" ...>` records whether an export is internal full-fidelity, public deny-sensitive, or diagnostic with redacted review copies. `<retention-candidate-bundle-verify-v1 "molten.retention.candidate-bundle-verify.v1" ...>` verifies that the handoff bundle still matches its manifest and packaged artifact refs, diagnosing missing, tampered, duplicate, or unreferenced files without importing trust. Operators can first emit a dry-run `<retention-gc-plan-v1 "molten.retention.gc-plan.v1" ...>` that lists requester, policy, authority, supporting-evidence, reference-index, local-retention, remote-GC, remote-clearance, and evidence-only boundary gates without writing retention receipts, tombstones, or deleting content. Applying from a plan uses `<retention-gc-apply-v1 "molten.retention.gc-apply.v1" ...>`: the operator supplies a stored plan ref, Molten recomputes the plan from its embedded candidate/evidence before mutation, denies on drift or failed admission, and only then emits the normal retention receipt/tombstone refs for unchanged passing plans. `<retention-receipt-v1 "molten.retention.receipt.v1" ...>` denies when pins, legal holds, retained refs, missing requester/policy/authority/supporting evidence, remote uncertainty, or incomplete proof remain; passing destructive actions emit tombstone/redaction evidence through `<retention-tombstone-v1 "molten.retention.tombstone.v1" ...>` without leaking private content. Evidence-ledger GC, chunk-store GC, and evaluation-cache invalidation now take explicit requester, policy, authority, evidence, retained, remote peer, remote ref, reference-index, remote-GC, remote-clearance, reference-index-complete, and non-dry-run apply-ref inputs; they require matching local `<retention-evidence-admission-v1 "molten.retention.evidence-admission.v1" ...>` receipts for apply-mode policy/authority/support/index/remote-plan proofs, require per-remote `<retention-remote-gc-clearance-v1 "molten.retention.remote-gc-clearance.v1" ...>` receipts for every known remote/peer, emit per-candidate `<retention-gc-execute-v1 "molten.retention.gc-execute.v1" ...>` gates that bind subsystem/action/object/class/apply/receipt/tombstone scope, and bind admission summaries plus per-remote/execution diagnostics in subsystem receipts before mutating local state. Operators can emit `<retention-gc-audit-v1 "molten.retention.gc-audit.v1" ...>` from an execution gate ref to inspect the plan → apply → execute → receipt → tombstone chain and its consistency diagnostics; the audit artifact is explanatory evidence only and does not grant authority, policy, resource, provenance, transport, execution, source-gate, remote-GC, or deletion trust. Catalog and MCP read-only discovery classify retention GC plan/apply/execute/audit artifacts by stage, decision, subsystem, object ref, retention class, and chain refs; the `search_retention_gc` MCP tool only returns catalog evidence and never replaces plan/apply/execute/admission gates. Remote clearance can be produced through transport-neutral `<retention-remote-gc-clearance-request-v1 "molten.retention.remote-gc-clearance-request.v1" ...>`, `<retention-remote-gc-clearance-response-v1 "molten.retention.remote-gc-clearance-response.v1" ...>`, and `<retention-remote-gc-clearance-import-v1 "molten.retention.remote-gc-clearance-import.v1" ...>` artifacts; import stores the embedded peer clearance locally only when request, response, peer, remote, object, action, policy, authority, current/revoked state, and retained refs match and pass. `<retention-remote-gc-clearance-live-workflow-v1 "molten.retention.remote-gc-clearance-live-workflow.v1" ...>` binds the request, response, import, and node-control live transport receipt refs for deterministic loopback workflows and operator-managed multi-host request-send/response-send/import-workflow flows, but the local import gate remains the only path that stores usable peer clearance. Secret cleanup requires actual matching retention receipt values for the cleaned secret and tombstone. Denied retention decisions leave content intact. Retention explain artifacts, candidate bundles, bundle profile receipts, bundle verification receipts, retention plans, apply receipts, retention receipts, remote-clearance workflow receipts, and dogfood retention GC release refs are deletion-safety evidence only and do not grant authority, provenance, transport, policy, resource, source-gate, remote-GC, or execution trust.
...
SAM service supervision follows the same evidence-first rule. Demand/startup, failure, lifecycle, monitor notification, restart decision, cleanup, retraction, and retention records are canonical Preserves artifacts. A supervision gate receipt replays the report and binds failure/status/lifecycle/restart/monitor/cleanup evidence for operator review, but it is not authority: service names, monitor delivery, and gate receipts cannot replace explicit authority, resource, policy, provenance, or source gates before side effects.
...
- `redacted-repro-export-profiles` / `repro-reveal-encrypted-ref-binding` — explicit `deny-sensitive`, `redacted-diagnostic`, and `encrypted-private` export profiles; deterministic redaction transform manifests/receipts bind source report/suite, policy, profile, output bundle, marker coverage, and loss classification; encrypted-private unpack requires reveal receipts bound to the exact encrypted-ref ids in the bundle; diagnostic/private bundles stay out of pass gates unless future gate-preserving policy explicitly allows them.
- `signed-evidence-receipts` / `signed-receipt-keyring` / `release-promotion-gate` — signed receipt envelopes that bind canonical evidence bytes to signer identity, key purpose, trust roots, expected subject refs, parent receipt chains, release bundle member verification, and the final promotion decision signature; ledger-backed key records and revocation/rotation records make currentness auditable; the promotion gate aggregates bundle verification, current keyring evidence, and source/Octet/Cairn markers into a final evidence-only pass/deny receipt, and promotion summary readback binds the signed final decision into one compact evidence artifact.
...
- `retention-gc-pinning` — retention classes, pin sources, reference indexes, tombstones, redaction hooks, per-remote GC clearance, safe GC eligibility, and catalog/MCP discovery for retention GC audit chains.
...
- `supply-chain-provenance-builds` — provenance records, trust states, reproducible build records, Nix derivation evidence refs, build verification receipts, artifact install/remote-sync gates, sensitive-artifact stronger provenance, catalog/MCP summaries, and review/attestation receipts.
...
- `node-control-shutdown-admission` — pure shutdown admission over request evidence, startup adapter receipts, and active-lock facts; admitted plans close adapters in reverse start order, while denied requests publish diagnostic denial evidence and preserve the active lock, startup evidence, and prior shutdown receipts.
...
- `node-control-live-send-ux` / `node-control-live-send-retry-idempotency` — external `control-ingress-live-send` workflow that uses bound live tickets to join real Iroh gossip topics, publish canonical ingress bytes, guard derived operation ids, retry bounded join/publish failures with receipts, suppress duplicate state-root-bound sends, and record send/transport plus live workflow runbook receipts without treating transport as authority.
- `node-control-live-import-ux` — explicit `live-ticket-import` and `authority-grant-import` commands with canonical import receipts that validate moved ticket/admission/grant artifacts before importing them into another state root while preserving receiver-side bootstrap, authority, policy/resource, and provenance gates.
- `node-control-live-send-diagnostics` — live-send ticket expectation guards, sender-state-root imported peer/grant preflight, deterministic import guidance, and receipt check labels for address support, operation binding, sender evidence, and join/publish success.
- `node-control-live-workflow-bundle-import-export` — canonical live workflow handoff bundles containing ticket, peer admission, authority grant, and supporting receipts, plus import/export receipts that validate bindings while remaining non-authority operational evidence.
- `node-control-live-workflow-bundle-verify` — offline bundle verification receipts for member refs, expected bindings, malformed members, and supporting receipt kinds before sender-state import; verify receipts remain non-authority/non-provenance.
- `node-control-live-workflow-bundle-gate` — operator gate receipts that re-run bundle verification and optionally require a current matching verify receipt before import; gate receipts remain non-authority/non-provenance.
...
- `operator-dogfood-retention-gc-workflow` — local dogfood release evidence for retention GC plan/apply/execute/audit, candidate bundle verify, and read-only catalog/MCP discovery while keeping retention artifacts evidence-only.
- `operator-dogfood-nix-release-check` / `operator-dogfood-release-evidence-bundle` — Nix release check that depends on hermetic nextest, runs `molten dogfood local-node`, preserves report/gate artifacts, emits verified Nix dogfood evidence, exports/verifies a canonical release evidence bundle, signs and summarizes the promotion receipt, and now emits a deterministic `release-evidence.tar.zst` export plus manifest/verify receipts binding the portable evidence member refs as evidence-only review outputs.
