...
- `redacted-repro-export-profiles` — explicit `deny-sensitive`, `redacted-diagnostic`, and `encrypted-private` export profiles; deterministic redaction transform manifests/receipts bind source report/suite, policy, profile, output bundle, marker coverage, and loss classification; diagnostic/private bundles stay out of pass gates unless gate-preserving policy or reveal receipts validate.
