...
- `signed-evidence-receipts` / `signed-receipt-keyring` — signed receipt envelopes that bind canonical evidence bytes to signer identity, key purpose, trust roots, expected subject refs, parent receipt chains, and release bundle member verification; ledger-backed key records and revocation/rotation records make currentness auditable while remaining evidence-only.
