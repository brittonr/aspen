...
## Exact-source Nix preflight: no realization (2026-09-25)

The previously owner-recovered local `executable-extent-src` Git cache contains the locked commit and tree; offline Nix prefetch of that explicit revision returned the unchanged lock's NAR hash. The other three private inputs needed by the `molten-node` derivation (`basalt-src`, `cairn-src`, `kamacite-src`) likewise matched their locked revisions and NAR hashes using local Git, including a separate full Cairn clone because the existing checkout was partial. These were invocation-local transport overrides, not `flake.lock` edits or new cohort approvals. Exact identities, commands, and outcomes are retained under `.cairn/changes/node-startup-evidence/evidence/standalone-node-exact-source-preflight/`.

Offline, no-builder, no-substituter, no-IFD evaluation returned the `molten-node` derivation path. Its requisite graph contained 3,571 store paths. A dry-run reported 1,347 derivations to build, including the Stage0 bootstrap chain, so **no build was launched**. This supersedes only the earlier *local source availability* blocker: source bytes are available in the isolated cache, but the approved already-realized compatible toolchain and package output are not. The pinned Octet gate still fails on the four host false positives; no source/compiler/build/binary binding, normal startup, VM transfer, or native replay has been established.

## Standalone Nix package realized; startup still denied (2026-09-25)

The earlier no-realization observation is historical. A missing Bash development output was retrieved from the signed official cache and retained under a user-owned GC root. With the already-realized May-26 nightly Rust toolchain and GCC stdenv, an offline no-builder dry-run reduced the plan to only the two node-crate derivations; it scheduled neither Stage0 nor a compiler chain. Unit2nix package helpers now receive the same Cargo toolchain as the dependency compiler, and the two node crates receive the complete workspace source needed for their shared compiled modules. Cairn evidence and documentation are excluded from that source snapshot so recording proof cannot invalidate the package it describes. The exact preflight, invocation-local private-input overrides, derivation/output/source identities, and limitations are retained under `.cairn/changes/node-startup-evidence/evidence/standalone-node-nix-package/`.

`nix build .#packages.x86_64-linux.molten-node` with those overrides exited 0. Its measured binary digest is recorded in that evidence. The realized executable initialized isolated state and refused `run` with `node-startup-source-gate-required`, writing no startup receipt. Package availability narrows the toolchain blocker, but does not turn the previously failing pinned Octet gate into a pass or validate the complete source/compiler closure. The four host false positives, approved real cohort, normal startup, consumer VM, and replay remain unresolved. No Stage0 bootstrap, producer publication, or consumer flake integration occurred.

The ambient Cairn CLI requires a newer workflow profile than this producer's committed policy allows. A verified, already-realized pre-`outcome-machine` Cairn executable validated the repository and passed the proposal, design, and tasks **structural** gates for `node-startup-evidence`; details and exact identity are in the package evidence. No policy migration or lifecycle approval follows from those structural checks.

The lock-matched node unit graph and the realized Nix derivation's recursive references each identify 557 distinct Rust crate name/version labels, including only the three node-local crates. Retained compiler logs show the expected first-party roots and direct `--extern` counts. This is package-graph agreement, **not** an independently measured file-level source closure: those compiler commands did not emit dependency files, and no complete `build-inputs.json` has been admitted. The exact comparison and its limitations are retained under `.cairn/changes/node-startup-evidence/evidence/standalone-node-graph-closure/`.

## Owner-mainline Octet review: diagnostic only (2026-09-25)

An isolated, unpublished Octet branch based on owner main ports the focused false-positive repairs for const calls, bounded literal arithmetic, and mutating borrowed vectors; the Molten Octet pin and selected three-package/all-target deny-all hook remain unchanged. Its existing-toolchain library build and paired positive/negative controls pass. A fresh, nonincremental diagnostic using that library and the pinned CLI/hook passes the host after an internal acquired-file `size_bytes` cutover but exits 2 on **299 `molten_node_core` errors**; the runtime is not reached. Private artifact-auth naming and const-eligibility repairs passed five authorization tests, and the host passed 22 tests. Exact tool identities, status, complete lint summary, and non-approval boundary are retained under `.cairn/changes/node-startup-evidence/evidence/standalone-node-owner-mainreview/`.

Owner review must reconcile advisory naming/assertion/enum policy with fail-closed validation and deliberately exhaustive state enums; do not insert panic assertions, wildcard arms, false `const fn`, or lint allowances to manufacture a pass. This working source is not the previously realized binary's source, and neither a complete compiler-file/generated-source inventory nor an approved release/cohort exists. Startup, VM, and replay remain denied.

## Committed-source stable-const diagnostic: still denied (2026-09-25)

The isolated Octet owner-main review branch `c9c4f35997d5a1cf3dbc04a77221db321c982006` now excludes unstable const library calls, such as `Vec::push` behind `const_heap`, from `missing_const_fn`. Ten paired const-call controls (including a failing-before/passing-after `Vec::push` case) and 155 owner library tests passed. This branch remains unpublished and unapproved; the Molten Octet pin and unmodified deny-all hook are unchanged.

On committed Molten source `81a2e880e219fafbf2925c4cf8dd493d7517ea71`, the three-package all-target hook with the review library compiled `molten-node-host`, then exited 2 on **288 `molten_node_core` errors**, zero warnings. The runtime was not reached. `missing_const_fn` is absent; 103 path naming, 59 assertion-density, and 51 exhaustive-enum advisory findings are among the remaining errors. Raw status, full summary, tool digests, and claim limits are retained under `.cairn/changes/node-startup-evidence/evidence/standalone-node-owner-stableconst-81a2/`. The source repairs on that commit use named, fail-closed authority facts and private validation naming without changing the public artifact-auth field names. They do not establish a passing strict gate, a complete compiler-file inventory, or an approved release cohort. No normal startup, consumer VM, or replay is authorized.

The committed selected-source all-target suite passed 518 tests. A separately Cargo-built executable from that source initialized an isolated state directory, then denied `run` with `node-startup-source-gate-required` and no receipt. This proves the guard remains closed for the current source, not that the release package or a normal startup cohort is admitted.
