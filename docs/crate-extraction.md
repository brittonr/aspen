...
| Jobs and CI core | `service library` | `aspen-jobs-core`, `aspen-jobs-protocol`, `aspen-jobs`, `aspen-ci-core`, executors | Aspen jobs and CI maintainers | [`crate-extraction/jobs-ci-core.md`](crate-extraction/jobs-ci-core.md) | `workspace-internal` | `aspen-jobs-core` now owns the first portable scheduler/run-state contracts and `aspen-jobs` re-exports them through a compatibility shell; next action is broader consumer compatibility and readiness evidence. |
