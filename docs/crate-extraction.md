...
| Trust/crypto/secrets | `leaf type/helper` + `service library` | `aspen-trust`, `aspen-crypto`, parts of `aspen-secrets` | architecture-modularity | [`crate-extraction/trust-crypto-secrets.md`](crate-extraction/trust-crypto-secrets.md) | `workspace-internal` | First blocker complete: pure trust/property and malformed-share evidence are present, `aspen-crypto` defaults to a transport-free helper boundary, and identity lifecycle is behind `aspen-crypto/identity`; next action is aggregate split/public API ownership review before readiness promotion. |
