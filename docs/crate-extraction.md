...
Typed policy lives at `docs/crate-extraction/policy.ncl`. The deterministic checker at `scripts/check-crate-extraction-readiness.rs` verifies manifests, dependency boundaries, evidence indexes, and broader-inventory consistency for the selected family, and is invoked as:
...
| Iroh/IRPC adapter | `aspen-raft-network` | `docs/crate-extraction/aspen-raft-network.md` | `crates/aspen-raft-network` | `extraction-ready-in-workspace` | Default/minimal adapter graph compiles without `aspen-sharding`; sharding route metadata is behind the explicit `sharding` feature; publishable/repo-split blocked on license/publication policy. |
...
| --- | --- | --- | --- | --- |
| 1 | `foundational-types` | `aspen-storage-types`, `aspen-traits`, `aspen-cluster-types`, `aspen-hlc`, `aspen-time`, `aspen-constants` | Public API owner review, canonical imports, downstream fixture, no-std checker, negative boundary, and compatibility evidence pass. | Publishable/repo-split blocked on license/publication policy. |
| 2 | `auth-ticket` | `aspen-auth-core`, `aspen-auth`, `aspen-ticket`, `aspen-hooks-ticket` | Portable APIs and compatibility re-export ownership are reviewed; token/ticket serialization, malformed rejection, downstream fixture, and runtime-shell negative evidence pass. | Publishable/repo-split remains blocked on license/publication policy. |
| 3 | `jobs-ci-core` | `aspen-jobs-core`, `aspen-jobs`, `aspen-jobs-protocol`, `aspen-jobs-guest`, `aspen-jobs-worker-*`, `aspen-ci-core`, `aspen-ci`, `aspen-ci-handler`, `aspen-job-handler`, `aspen-ci-executor-*` | Owner/public API review, downstream metadata, negative boundary, compatibility, and readiness checker evidence pass. | Publishable/repo-split remains blocked on license policy. |
| 4 | `trust-crypto-secrets` | `aspen-trust`, `aspen-crypto`, `aspen-secrets-core`, runtime coverage for `aspen-secrets`/`aspen-secrets-handler` | Reusable package candidates now have checker, async/default, serialization, and compatibility evidence; runtime/service shells remain excluded from promotion. | Future work needs explicit package-level evidence before promoting `aspen-secrets`, `aspen-secrets-handler`, `aspen-crypto/identity`, or `aspen-trust/async`. |
| 5 | `testing-harness` | `aspen-testing-core`, `aspen-testing`, `aspen-testing-fixtures`, `aspen-testing-madsim`, `aspen-testing-network`, `aspen-testing-patchbay` | Canonical reusable testing-core surface, adapter ownership, positive downstream fixture, negative adapter-boundary fixture, dependency graph, and compatibility checks pass. | Publishable/repo-split remains blocked on license policy. |
...
| --- | --- | --- | --- | --- | --- | --- |
| Foundational types/helpers | `leaf type/helper` | `aspen-constants`, `aspen-hlc`, `aspen-kv-types`, `aspen-storage-types`, `aspen-cluster-types`, `aspen-traits`, `aspen-time` | Aspen foundational type maintainers | [`crate-extraction/foundational-types.md`](crate-extraction/foundational-types.md) | `extraction-ready-in-workspace` | Canonical imports, owner, downstream fixture, negative boundary checks, no-std checker, and representative compatibility checks pass; publishable/repo-split blocked on license policy. |
| Auth and tickets | `leaf type/helper` | `aspen-auth-core`, `aspen-auth`, `aspen-ticket`, `aspen-hooks-ticket` | Aspen auth and ticket maintainers | [`crate-extraction/auth-ticket.md`](crate-extraction/auth-ticket.md) | `extraction-ready-in-workspace` | Canonical portable imports, compatibility re-export ownership, token/ticket goldens, malformed-input rejection, downstream fixture, and runtime-verifier negative evidence pass; publishable/repo-split blocked on license policy. |
...
| Redb Raft KV | `storage/backend` + `service library` + `runtime adapter` | `aspen-kv-types`, `aspen-redb-storage`, `aspen-raft-kv-types`, `aspen-raft-kv`, `aspen-raft-network`, `aspen-raft` | Aspen Raft/KV maintainers | per-layer manifests | 5/6 `extraction-ready-in-workspace` | Type/storage/facade/adapter layers ready; app compatibility shell remains `workspace-internal` by design; publishable/repo-split blocked on license policy. |
| Iroh transport/RPC | `runtime adapter` + `service library` | `aspen-transport`, `aspen-rpc-core`, `aspen-client` | Aspen transport/RPC maintainers | [`crate-extraction/transport-rpc.md`](crate-extraction/transport-rpc.md) | `extraction-ready-in-workspace` | Downstream fixture tests, forbidden-boundary scans, crate checks, cargo-tree evidence, and compatibility consumers pass; publishable/repo-split blocked on license policy. |
...
| Blob/castore/cache | `service library` + `runtime adapter` | `aspen-blob`, `aspen-castore`, `aspen-cache`, `aspen-exec-cache` | Aspen storage/cache maintainers | [`crate-extraction/blob-castore-cache.md`](crate-extraction/blob-castore-cache.md) | `extraction-ready-in-workspace` | Downstream fixtures, negative boundaries, checker-required evidence, and compatibility rails pass; publishable/repo-split blocked on license policy. |
| Commit DAG / branches | `service library` | `aspen-commit-dag`, `aspen-kv-branch`, `aspen-dag` | Aspen branch/DAG maintainers | [`crate-extraction/kv-branch-commit-dag.md`](crate-extraction/kv-branch-commit-dag.md) | `extraction-ready-in-workspace` | Downstream fixture, negative boundary checks, branch/DAG package checks, docs feature compatibility, and readiness checker pass; publishable/repo-split blocked on license policy. |
| Jobs and CI core | `service library` | `aspen-jobs-core`, `aspen-jobs-protocol`, `aspen-jobs`, `aspen-jobs-guest`, `aspen-jobs-worker-*`, `aspen-ci-core`, `aspen-ci`, `aspen-ci-handler`, `aspen-job-handler`, `aspen-ci-executor-*` | Aspen jobs and CI maintainers | [`crate-extraction/jobs-ci-core.md`](crate-extraction/jobs-ci-core.md) | `extraction-ready-in-workspace` | Owner/public API review, downstream metadata, negative boundary, compatibility, and readiness checker evidence pass; publishable/repo-split blocked on license policy. |
| Trust/crypto/secrets | `leaf type/helper` + `service library` | `aspen-trust`, `aspen-crypto`, `aspen-secrets-core`, parts of `aspen-secrets` | Aspen trust/crypto/secrets maintainers | [`crate-extraction/trust-crypto-secrets.md`](crate-extraction/trust-crypto-secrets.md) | 3 package candidates `extraction-ready-in-workspace`; aggregate remains `workspace-internal` | `aspen-crypto`, default/no-default `aspen-trust`, and `aspen-secrets-core` are ready in-workspace; keep `aspen-secrets`, `aspen-secrets-handler`, `aspen-crypto/identity`, and `aspen-trust/async` internal/runtime-scoped until separate package-level evidence exists. Publishable/repo-split remains blocked on license policy. |
| Config/plugin | `protocol/wire` | `aspen-nickel`, `aspen-plugin-api` | Aspen config/plugin maintainers | [`crate-extraction/config-plugin.md`](crate-extraction/config-plugin.md) | `workspace-internal` | Manifest, owner, feature minima, and verification rails documented; add standalone examples and resolve `aspen-nickel -> aspen-core` before raising readiness. |
| Testing harness | `service library` | `aspen-testing-core`, fixtures, madsim/network/patchbay crates | Aspen testing harness maintainers | [`crate-extraction/testing-harness.md`](crate-extraction/testing-harness.md) | `extraction-ready-in-workspace` | Canonical reusable testing-core surface, adapter ownership, positive downstream fixture, negative adapter-boundary fixture, dependency graph, and madsim/network/patchbay compatibility checks pass; publishable/repo-split blocked on license policy. |
