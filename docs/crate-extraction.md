...
| 3 | `jobs-ci-core` | `aspen-jobs-core`, `aspen-jobs`, `aspen-jobs-protocol`, `aspen-jobs-guest`, `aspen-jobs-worker-*`, `aspen-ci-core`, `aspen-ci`, `aspen-ci-handler`, `aspen-job-handler`, `aspen-ci-executor-*` | Portable payload/wire/keyspace/route/status/handle contracts now live on owning surfaces; owner/public API review confirmed canonical surfaces, compatibility re-export ownership, runtime-shell exclusions, and fresh evidence; publishable/repo-split remains blocked on license policy. | Refresh manifest/policy/evidence, keep executor/handler negative checks, and rerun CI/jobs compatibility evidence. |
