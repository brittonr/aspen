...
| Trust/crypto/secrets | `leaf type/helper` + `service library` | `aspen-trust`, `aspen-crypto`, `aspen-secrets-core`, parts of `aspen-secrets` | architecture-modularity | [`crate-extraction/trust-crypto-secrets.md`](crate-extraction/trust-crypto-secrets.md) | `workspace-internal` | Owner/public API review complete; checker coverage now maps `aspen-crypto` and `aspen-secrets-core` as real reusable crate candidates, while promotion remains deferred until `aspen-trust` async/default dependency policy is explicit and trust/secrets serialization-contract evidence lands. |
