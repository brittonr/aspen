...
| Trust/crypto/secrets | `leaf type/helper` + `service library` | `aspen-trust`, `aspen-crypto`, `aspen-secrets-core`, parts of `aspen-secrets` | architecture-modularity | [`crate-extraction/trust-crypto-secrets.md`](crate-extraction/trust-crypto-secrets.md) | 3 package candidates `extraction-ready-in-workspace`; aggregate remains `workspace-internal` | `aspen-crypto`, default/no-default `aspen-trust`, and `aspen-secrets-core` are ready in-workspace; keep `aspen-secrets`, `aspen-secrets-handler`, `aspen-crypto/identity`, and `aspen-trust/async` internal/runtime-scoped until separate package-level evidence exists. Publishable/repo-split remains blocked on license policy. |
