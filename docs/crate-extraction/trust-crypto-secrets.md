...
- **Readiness state**: package-scoped promotion complete: `aspen-crypto`, default/no-default `aspen-trust`, and `aspen-secrets-core` are `extraction-ready-in-workspace`; the aggregate `trust-crypto-secrets` family remains `workspace-internal` because `aspen-secrets`, `aspen-secrets-handler`, `aspen-crypto/identity`, and `aspen-trust/async` remain runtime/service/compatibility surfaces.
...
`review-trust-crypto-secrets-public-api` records the aggregate readiness decision after the transport-free crypto, auth-runtime, KV traits, handler runtime-adapter, secrets-core type, and mount-provider boundary slices. The initial review kept the family at `workspace-internal`; the fresh promotion review now raises only the real reusable package candidates (`aspen-crypto`, default/no-default `aspen-trust`, and `aspen-secrets-core`) to `extraction-ready-in-workspace`. `complete-trust-crypto-secrets-checker-coverage` made the readiness checker validate real package candidates instead of an aggregate pseudo-candidate, and `complete-aspen-trust-async-default-policy` added `aspen-trust` to that real package coverage while keeping async service APIs behind `aspen-trust/async`.

Compatibility re-exports: none for `aspen-crypto`; `aspen-secrets` preserves compatibility re-exports for `aspen-secrets-core`.

Canonical reusable surfaces are `aspen-crypto` default/no-default helpers, `aspen-secrets-core` type/state contracts, and the default/no-default `aspen-trust` pure helper/state/wire surface. `aspen-trust` async/Tokio service APIs are explicitly opt-in through `aspen-trust/async`. `aspen-secrets` remains a service/runtime implementation crate for now, and `aspen-secrets-handler` remains a compatibility/runtime consumer. Evidence is recorded under `openspec/changes/archive/2026-05-03-review-trust-crypto-secrets-public-api/evidence/`, `openspec/changes/archive/2026-05-03-complete-trust-crypto-secrets-checker-coverage/evidence/`, and the package-scoped promotion review archive after completion.
