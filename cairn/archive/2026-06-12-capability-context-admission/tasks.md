...
- [x] [serial] r[molten.testing.capability_context.fixture_schema] Normalize explicit capability fixtures into deterministic capability context refs and track omitted fixtures separately for fail-closed mandatory-fixture gates.
...
- [x] [parallel] r[molten.testing.capability_context.validation] Emit canonical failure diagnostics for missing grants, stale capability refs, and unauthorized effect requests.
...
- [x] [serial] r[molten.testing.capability_context.deny_by_default] Add negative suites for send without grant, effect without grant, tampered grant, stale capability ref, and denied effect with response.
