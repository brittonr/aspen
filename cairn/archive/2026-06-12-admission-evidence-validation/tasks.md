...
- [x] [serial] r[molten.testing.admission_evidence.records] Make report validation reject observations with missing, duplicated, malformed, or misplaced admission decisions.
...
- [x] [parallel] r[molten.testing.admission_evidence.records.missing] Emit canonical validation/replay failure diagnostics for admission evidence failures.
...
- [x] [parallel] r[molten.testing.admission_evidence.policy_divergence] Ensure repro bundles preserve policy fixtures, admission events, and failure diagnostics needed to reproduce admission evidence failures.

## Phase 4: Tests and future policy boundary

- [x] [serial] r[molten.testing.admission_evidence.records.missing] Add negative suites for missing admission event, tampered allow/deny, denied turn with committed action, and denied effect with response.
- [x] [serial] r[molten.testing.admission_evidence.gate_checks.receipt] Add gate receipt tests proving admission checks are listed and parsed.
- [x] [parallel] r[molten.testing.admission_evidence.static_policy_boundary] Document and structure the static Preserves policy fixture so Nickel contracts and Basalt/UCAN context can replace or augment it without removing fail-closed validation.
