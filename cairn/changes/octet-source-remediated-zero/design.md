...
`src/main.rs` remains the top-level Clap shell, but its `TestCommand::Octet`, `TestCommand::Delivery`, `TestCommand::Protocol`, `TestCommand::Provenance`, `TestCommand::Retention`, `TestCommand::Job`, `TestCommand::Secrets`, and `TestCommand::Plugin` variants reference module-local command enums, and dispatch delegates to module-local runners.

### Semantic preservation

The split must preserve:

- command paths and flags for `molten test octet ...`, `molten test delivery ...`, `molten test provenance ...`, `molten test protocol ...`, `molten test retention ...`, `molten test job ...`, `molten test secrets ...`, `molten test plugin ...`, and `molten node ...`;
