...
`src/main.rs` remains the top-level Clap shell, but its `TestCommand::Octet`, `TestCommand::Delivery`, and `TestCommand::Retention` variants reference module-local command enums, and dispatch delegates to module-local runners.

### Semantic preservation

The split must preserve:

- command paths and flags for `molten test octet ...`, `molten test delivery ...`, and `molten test retention ...`;
