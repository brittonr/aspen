...
- Split Dogfood command enums, release export archive helpers, and bounded signed-member loading into child files while preserving operator dogfood command syntax and receipt behavior; the no-disabled probe now has 8205 findings, `excessive_file_length` down to 115, `non_trait_imports` down to 4603, and Molten source still clear of `module_file_count` findings.
