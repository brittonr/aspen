...
- Split low-risk file-length hotspots by moving runtime envelope tests, NixOS VM tests, the coordination bounded batch helper, and rewrite CLI input structs into child files while preserving module names and command behavior; the no-disabled probe now has `excessive_file_length` down to 129.
