...
- [x] [serial] r[molten.sam_service_supervision.spec.supervision] Replay-bound `service-supervision-gate-receipt-v1` evidence, CLI `service gate-supervision`, ledger classification, pass/deny replay coverage, and non-authority checks are implemented in `src/service_supervision.rs` and `src/main.rs`.
