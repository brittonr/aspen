...
- [ ] [serial] Connect consensus engine instances to admitted transport, durable-log, snapshot, time, entropy, membership, placement, fencing, supervision, and resource ports. r[molten.fabric_consistency.live_service_ports]
  - Progress: the pure startup plan and thin host projection require the exact active group, canonical group integrity, running service generation, admitted policy cohort, static membership, timer/resource bounds, and canonical transport/durable-log/snapshot/time/entropy/membership/placement bindings. The effect shell executes durability before transport in declared order, publishes state only after all effects pass, and retains partial evidence on failure. The concrete Iroh port sends strict canonical frames through admitted peer endpoints, derives each request identity from the bounded canonical payload before acknowledgement, and accepts sequential dynamically identified frames without a pre-known message ref. The Redb port records hard state at machine-loss durability, buffers canonical log mutations, binds a flush marker before machine-loss flush, and persists snapshots. The Tokio time port validates plans through the admitted live time profile, draws bounded election delays from the production entropy adapter, cancels superseded timers, and delivers generation-bound timer tokens. Deterministic core-owned timer tokens deny stale queued timeouts before durability or network effects. The application adapter admits contiguous schema-bound batches, the supervision adapter publishes channel-backed canonical observations, and the scoped service executes startup and bounded turns through an exact runtime port cohort. Startup remains production-denied; a full concrete cohort constructor and live membership/placement/fencing/resource lifecycle integration are still required before this task can close.
- [ ] [serial] Implement the first live Raft service profile with protocol registration, elections, replication, commit, reads, snapshots, recovery, and bounded static membership before admitting wider transitions. r[molten.fabric_consistency.live_raft]
  - Progress: a pure static three-voter transition core binds every protocol envelope to group, generation, configuration, fencing, and member identity; persists hard state and log entries before outbound effects; elects by majority; commits only current-term majority replication; suppresses duplicate requests; ignores out-of-order append failures; and keeps linearizable reads retryable without a read barrier. Strict Preserves frame encoding/decoding and one admitted live Iroh listener/client exchange now pass. Multi-message live elections, concrete durability/time execution, snapshots, recovery, and distinct-process evidence remain open.
