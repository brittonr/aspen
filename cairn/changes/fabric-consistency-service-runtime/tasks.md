...
- [ ] [serial] Implement the first live Raft service profile with protocol registration, elections, replication, commit, reads, snapshots, recovery, and bounded static membership before admitting wider transitions. r[molten.fabric_consistency.live_raft]
  - Progress: a pure static three-voter transition core binds every protocol envelope to group, generation, configuration, fencing, and member identity; persists hard state and log entries before outbound effects; elects by majority; commits only current-term majority replication; suppresses duplicate requests; ignores out-of-order append failures; and keeps linearizable reads retryable without a read barrier. Canonical frame encoding, concrete live adapter execution, snapshots, recovery, and distinct-process evidence remain open.
