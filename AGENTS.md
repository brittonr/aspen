...
- `crates/aspen-cluster-types` is still part of the alloc-only `aspen-core` dependency path and today hard-depends on `iroh` via `NodeAddress`. If `cargo tree -p aspen-core -e features` still shows `iroh` in the bare/default graph, fix `aspen-cluster-types` before blaming the new `aspen-core/std` gates.
