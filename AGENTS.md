...
- Long cargo rails launched through pueue can inherit `CARGO_INCREMENTAL=1`, which makes Aspen's `sccache` wrapper abort in detached baseline clones/worktrees with `sccache: incremental compilation is prohibited`. Unset `CARGO_INCREMENTAL` before those captures.
- In this environment, some cargo compile/test rails still hit the `sccache` wrapper even after `env -u CARGO_INCREMENTAL`. If that happens, use `env -u CARGO_INCREMENTAL RUSTC_WRAPPER= bash -lc 'cargo ...'` so `cargo` resolves `rustc` directly instead of querying the wrapper.
