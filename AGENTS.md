...
- **crates/aspen-auth-core/**: Alloc-focused auth types and pure helpers (`Capability`, `Operation`, `CapabilityToken`, `AuthError`, verified auth/credential logic)
- **crates/aspen-hooks-ticket/**: Hook trigger ticket crate (`AspenHookTicket`) split from `aspen-hooks-types`
...
- OpenSpec typed tasks only accept bare numeric IDs like `I1`, `V2`, `H1`, or `R1`; suffixed IDs like `I1a` / `V2b` fail the tasks gate. When you need decomposition, use plain bullet substeps under the typed task or add new numeric tasks instead.
...
- Root-disk free space can drop suddenly because `target/` grows into tens of GB. If `edit`/OpenSpec artifact writes hit `ENOSPC`, recover staged file contents with `git show :path/to/file`, then free space with targeted `cargo clean -p <crate>` instead of wiping the whole workspace cache.
- Large targeted cargo tests can also fail even with root-disk space left if `/tmp` tmpfs fills up; `clang`/linker errors may show `printf: write error: No space left on device`. In that case rerun with `TMPDIR=/run/user/$UID/tmp` (or another disk-backed temp dir) instead of debugging the crate.
- Aspen baseline-capture clones/worktrees must preserve sibling-repo layout. If you need to run historical compile rails for evidence, do not drop the baseline tree into a random `/tmp` path without also wiring adjacent `aspen-dns` / `aspen-wasm-plugin` siblings; otherwise workspace path deps can either fail to resolve or resolve `../aspen` back to the live repo and produce package-collision lockfile errors. Prefer a standalone baseline clone plus explicit sibling symlinks that point back to the baseline snapshot.
- Repo root `.cargo/config.toml` now sets `build.rustc-wrapper = ""` to override the global Home Manager `sccache` wrapper, because Aspen's dev shell exports `CARGO_INCREMENTAL=1` and that wrapper aborts on incremental rustc probes (`cargo clean`, `cargo check`, Tiger Style rails, etc.). If those commands start failing with `sccache: incremental compilation is prohibited`, check whether you are in an older checkout/worktree that predates this override.
- Older detached baseline clones/worktrees can still inherit the global wrapper without the repo override. In those pre-fix trees, the reliable escape hatch is `env -u CARGO_INCREMENTAL RUSTC_WRAPPER= bash -lc 'cargo ...'`.
- `scripts/run-forge-web.sh` creates its demo Git repo under `/tmp`, outside Aspen's `.cargo/config.toml`; keep demo `Cargo.lock` generation clearing `CARGO_INCREMENTAL`/`RUSTC_WRAPPER` and fail closed if `Cargo.lock` or `flake.lock` is not staged, otherwise Forge CI later fails with Nix's `Path 'Cargo.lock' does not exist in Git repository` or creates an unpinned lock during build.
- The focused Forge web demo prep check is `nix build .#checks.x86_64-linux.forge-web-demo-prep --no-link`. It must copy exact script paths into the derivation because raw flake `src` excludes `scripts/`, run the shell test via `bash` because Nix sandbox lacks `/usr/bin/env`, and keep u2n's plugin-stripped source removing `jj-native-forge` whenever it strips `plugins`.
...
- `crates/aspen-cluster-types` is still part of the alloc-only `aspen-core` dependency path and `NodeAddress` must stay alloc-safe with iroh conversion helpers behind an opt-in feature. Keep `crates/aspen-cluster-types` on `default = []`; runtime/test crates that need `NodeAddress::new`, `ClusterNode::with_iroh_addr`, or `.iroh_addr()` must opt into `features = ["iroh"]` explicitly in their dependency stanza. If `cargo tree -p aspen-core -e features` still shows `iroh` or `iroh-base` in the bare/default graph, check both `crates/aspen-cluster-types` and transitive consumers like `crates/aspen-traits`; disabling defaults on `aspen-core` alone is not enough if another direct dep re-enables `aspen-cluster-types` defaults.
- `crates/aspen-auth` is now the std/runtime shell. Portable consumers that only need token/capability types should depend on `crates/aspen-auth-core`; runtime consumers keep using `aspen-auth` for `TokenBuilder`, `TokenVerifier`, HMAC auth, and revocation storage.
- `crates/aspen-hooks-types` now carries only hook config/event schema. Consumers that need `AspenHookTicket` must depend on `crates/aspen-hooks-ticket` (or `aspen-hooks`, which re-exports both types crates).
...
- The old `aspen-storage-types` no-std seam is resolved: `KvEntry` stays alloc-safe, `SM_KV_TABLE` lives in runtime/shell Redb crates, and `scripts/check-aspen-core-no-std-boundary.py` is the source of truth for the `aspen-core --no-default-features` graph. If that checker fails after dependency updates, refresh the expected direct set, `scripts/aspen-core-no-std-transitives.txt`, and current review notes under `docs/dependency-reviews/aspen-core-no-std/` rather than re-splitting storage types. Future no-std work should inspect the live checker/tree first, then continue with `aspen-traits` alloc-safety or wire-crate `std::collections` / test-only serde baggage only if the live graph still shows those leaks.
