...
- Highest-ROI follow-up no-std seam is `crates/aspen-storage-types`: `KvEntry` is alloc-safe but `SM_KV_TABLE` keeps `redb`/`libc` in the foundational graph. Split the Redb table definition into a shell/storage crate before chasing smaller leaks. After that, make `crates/aspen-traits` alloc-safe (`alloc::sync::Arc` or shell-only blanket impls), then sweep wire crates (`aspen-client-api`, protocol crates) for `std::collections` and test-only `serde_json` / `postcard use-std` baggage.
