...
fn chunk_count_for_len(byte_len: usize, chunk_size: usize, label: &str) -> Result<usize> {
    if chunk_size == 0 {
        return Err(Failure::invalid_harness(format!("{label} chunk size must be non-zero")));
    }
...
fn chunk_size_to_usize(chunk_size: u64, label: &str) -> Result<usize> {
    usize::try_from(chunk_size)
        .map_err(|error| Failure::invalid_harness(format!("{label} is unsupported on this platform: {error}")))
}
...
fn verify_manifest_chunks_into(
    ...
    let total_len = usize::try_from(manifest.total_len).map_err(|error| {
        Failure::invalid_harness(format!("chunk manifest total length is unsupported on this platform: {error}"))
    })?;
    ...
    let actual_len = u64::try_from(bytes.item_count()).map_err(|error| {
        Failure::invalid_harness(format!("chunk manifest actual length is unsupported on this platform: {error}"))
    })?;
...
fn verify_raw_chunk_file(
    ...
    verify_raw_chunk_bytes(&bytes, chunk_ref, length, chunk_size).map_err(|error| {
        Failure::invalid_harness(format!("chunk store content path for {chunk_ref} is invalid: {error}"))
    })
}
...
fn verify_raw_chunk_bytes(bytes: &[u8], chunk_ref: &str, length: u64, chunk_size: usize) -> Result<()> {
    ...
    if bytes.len() as u64 != length {
        return Err(Failure::invalid_harness(format!("chunk bytes length {}, expected {length}", bytes.len())));
    }
    Ok(())
}
...
fn validate_fixed_chunk_lengths(total_len: u64, chunk_size: u64, chunks: &[ChunkRef]) -> Result<()> {
    ...
    if total_len == 0 {
        if !chunks.is_empty() {
            return Err(Failure::invalid_harness("empty object manifest must not contain chunks"));
        }
        return Ok(());
    }
...
fn filename_for_ref(reference: &str) -> Result<String> {
    let hex = content_ref_hex(reference)
        .map_err(|error| Failure::invalid_harness(format!("unsupported chunk-store ref {reference}: {error}")))?;
