...
mod tests {
    ...
    fn test_execution_handle_creation() {
        let vm_handle = ExecutionHandle::vm("vm-123".to_string(), "job-456".to_string());
        assert_eq!(vm_handle.id, "vm-123");
        assert_eq!(vm_handle.job_id, "job-456");
        assert_eq!(vm_handle.backend_name, "vm");
        assert!(vm_handle.is_vm());
        assert!(!vm_handle.is_flawless());
    }

    #[test]
    fn test_handle_type_display() {
        let vm_type = ExecutionHandleType::Vm("abc".to_string());
        assert_eq!(vm_type.to_string(), "vm:abc");

        let process_type = ExecutionHandleType::LocalProcess(1234);
        assert_eq!(process_type.to_string(), "process:1234");

        let custom_type = ExecutionHandleType::Custom {
            backend_type: "custom".to_string(),
            handle_id: "xyz".to_string(),
        };
        assert_eq!(custom_type.to_string(), "custom:xyz");
    }

    #[test]
    fn test_handle_serialization() {
        let handle = ExecutionHandle::mock("mock-test".to_string(), "job-test".to_string());
        let serialized = serde_json::to_string(&handle).unwrap();
        let deserialized: ExecutionHandle = serde_json::from_str(&serialized).unwrap();
