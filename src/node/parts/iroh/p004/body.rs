...
struct RouterReceiptInput<'a> {
    ...
    registry_entry_ref: Option<&'a str>,
...
fn router_receipt_value(input: RouterReceiptInput<'_>) -> crate::error::Result<preserves::IOValue> {
    Ok(crate::preserves_rail::record("iroh-protocol-router-receipt-v1", vec![
        ...
        crate::preserves_rail::record("registry-entry", vec![optional_string_value(input.registry_entry_ref)]),
        ...
        checks_value(&[
            ...
            ("alpn-registry-entry-bound", pass_fail(input.registry_entry_ref.is_some())),
