...
pub fn parse_operation_id(value: &IoValue) -> Result<OperationId> {
    let fields = value
        ...
        .ok_or_else(|| Failure::invalid_harness("expected <operation-id-v1 ...>"))?;
...
pub fn window_value(
    ...
    if lowest_retained == 0 || next_sequence == 0 || lowest_retained > next_sequence {
        return Err(Failure::invalid_harness("invalid delivery window sequence bounds"));
    }
...
pub fn parse_window(value: &IoValue) -> Result<Window> {
    let fields = value
        ...
        .ok_or_else(|| Failure::invalid_harness("expected <delivery-window-v1 ...>"))?;
    ...
    validate_scope_profile(&scope_profile)?;
    if lowest_retained == 0 || next_sequence == 0 || lowest_retained > next_sequence {
        return Err(Failure::invalid_harness("invalid parsed delivery window sequence bounds"));
    }
