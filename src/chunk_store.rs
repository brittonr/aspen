...
pub fn range_read(root: &Path, manifest_ref: &str, offset: u64, length: u64) -> Result<ChunkStoreRangeRead> {
    ...
    if length > 0 {
        ...
        let end_usize = usize::try_from(end).map_err(|error| {
            MoltenError::invalid_harness(format!("range end is unsupported on this platform: {error}"))
        })?;
        let first = offset_usize
            .checked_div(chunk_size)
            .ok_or_else(|| MoltenError::invalid_harness("range chunk size must be non-zero"))?;
        let last_exclusive = end_usize.div_ceil(chunk_size);
        for index in first..last_exclusive {
            let chunk = manifest
                .chunks
                .get(index)
                .ok_or_else(|| MoltenError::invalid_harness(format!("range maps to missing chunk index {index}")))?;
            let chunk_bytes = match read_verified_chunk(root, chunk, chunk_size) {
                Ok(bytes) => bytes,
                Err(error) => {
            ...
            let wanted_start = offset_usize.saturating_sub(chunk_start);
            let wanted_end = end_usize.saturating_sub(chunk_start).min(chunk_bytes.len());
    ...
    {
        let message = format!("range reconstruction length mismatch: got {}, expected {length}", bytes.len());
        let receipt_value =
            denial_receipt_value("range-read", Some(&manifest.manifest_ref), &chunk_refs, &message, vec![
                ("range-chunk-verification", "fail"),
...
pub fn parse_chunk_lineage_value(value: &IOValue) -> Result<ChunkLineage> {
    ...
    require_chunk_lineage_predicate(&predicates, SEGMENT_NO_GAP_PREDICATE)?;
...
fn chunk_lineage_context_refs(manifest: &ChunkManifest, receipt: &ChunkStoreReceipt) -> Result<Vec<ChainContextRef>> {
    let mut refs = vec![
        ...
        ChainContextRef::new("metadata", manifest.metadata_ref.clone()),
        ChainContextRef::new(
            "operation",
    ...
    for chunk in &manifest.chunks {
        push_bounded(
            &mut refs,
            ChainContextRef::new("chunk", chunk.chunk_ref.clone()),
...
struct DetailContextRefsInput<'a> {
    value: &'a IOValue,
    refs: &'a mut Vec<ChainContextRef>,
}
...
fn collect_detail_context_refs_push_text(input: DetailTextInput<'_>) -> Result<()> {
    if validate_content_ref(&input.text).is_ok() {
        ...
        push_bounded(
            input.refs,
            ChainContextRef::new("ticket", canonical_hash(&record("iroh-ticket", vec![string(input.text)]))?),
            MAX_CHUNK_STORE_CONTEXT_REFS,
            "chunk lineage detail refs",
        )?;
    }
...
fn require_lineage_context(context_refs: &[ChainContextRef], label: &str, expected: &str) -> Result<()> {
...
fn parse_lineage_chain_scope(value: &Value<IOValue>) -> Result<ChainScope> {
    let chain = value
