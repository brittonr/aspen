...
fn parse_chain_scope(value: &Value<IoValue>) -> Result<crate::evidence_chain::ChainScope> {
    ...
    let chain = value
        ...
        .ok_or_else(|| Failure::invalid_harness("expected chain scope record"))?;
...
fn parse_check_names(value: &Value<IoValue>) -> Result<Vec<String>> {
    ...
    let values = record[0]
        .collect_sequence()
        .ok_or_else(|| Failure::invalid_harness("expected sequence for checks"))?;
    values
        .iter()
        .map(|value| {
            let value = value_to_iovalue(value);
            ...
            if status != "pass" {
                return Err(Failure::invalid_harness(format!("check {name} status is {status}")));
            }
            Ok(name)
        })
        .collect()
...
fn require_check(checks: &[String], expected: &str) -> Result<()> {
    if checks.iter().any(|check| check == expected) {
        ...
        Err(Failure::invalid_harness(format!("chain bundle missing {expected} check")))
    }
}
...
fn blob_store_path(bundle_ref: &str) -> Result<RelativeLocator> {
    let hex = content_ref_hex(bundle_ref)
        .map_err(|error| Failure::invalid_harness(format!("unsupported Iroh bundle ref {bundle_ref}: {error}")))?;
    RelativeLocator::parse(&format!("blobs/blake3_{hex}.bin"))
}
