    ...
    refs.iter()
        .map(|verify_ref| {
            let artifact = artifacts.get(verify_ref).ok_or_else(|| {
                Failure::invalid_harness(format!("chain bundle missing verify receipt {verify_ref}"))
            })?;
...
fn primary(receipts: &[ParsedChainVerifyReceipt]) -> Result<&ParsedChainVerifyReceipt> {
    receipts
        .first()
        .ok_or_else(|| Failure::invalid_harness("chain bundle must contain a verify receipt"))
}
...
fn check_primary(input: PrimaryCheck<'_>) -> Result<()> {
    ...
    {
        return Err(Failure::invalid_harness("chain bundle verify receipt does not match requested chain range"));
    }
    ...
    if has_fork_diagnostics && !input.has_forks {
        return Err(Failure::invalid_harness("chain bundle fork diagnostics require fork evidence artifacts"));
    }
    Ok(())
}
    ...
    refs.iter()
        .map(|checkpoint_ref| {
            let artifact = artifacts.get(checkpoint_ref).ok_or_else(|| {
                Failure::invalid_harness(format!("chain bundle missing checkpoint {checkpoint_ref}"))
            })?;
...
fn validate_chain_links(
    ...
    if verify.verified_links.is_empty() {
        return Err(Failure::invalid_harness("chain bundle verify receipt contains no verified links"));
    }
    if let Some(anchor_ref) = anchor_ref
        && verify.verified_links.first().map(String::as_str) != Some(anchor_ref)
    {
        return Err(Failure::invalid_harness("chain bundle segment does not begin at requested anchor"));
    }
    if let Some(head_ref) = head_ref
        && verify.verified_links.last().map(String::as_str) != Some(head_ref)
    {
        return Err(Failure::invalid_harness("chain bundle segment does not end at requested head"));
    }
    for (position, link_ref) in verify.verified_links.iter().enumerate() {
        let link = links
            .get(link_ref)
            .ok_or_else(|| Failure::invalid_harness(format!("chain bundle missing verified link {link_ref}")))?;
        if link.chain != verify.chain {
            return Err(Failure::invalid_harness("chain bundle link belongs to a different chain scope"));
        }
        ...
        if position > 0 {
            ...
            let previous = links.get(previous_ref).ok_or_else(|| {
                Failure::invalid_harness(format!("chain bundle missing previous link {previous_ref}"))
            })?;
...
fn validate_bundle_checkpoints(
    ...
    for checkpoint in checkpoints {
        if &checkpoint.chain != chain {
            return Err(Failure::invalid_harness("chain bundle checkpoint belongs to a different chain"));
        }
        if !links.contains_key(&checkpoint.anchor_link_ref) || !links.contains_key(&checkpoint.head_ref) {
            return Err(Failure::invalid_harness("chain bundle checkpoint anchor/head links are unavailable"));
        }
        let Some(verify_artifact) = artifacts.get(&checkpoint.verify_receipt_ref) else {
            return Err(Failure::invalid_harness("chain bundle checkpoint verify receipt is unavailable"));
        };
        ...
        let Some(predicate) = predicates.get(&checkpoint.range_predicate_ref) else {
            return Err(Failure::invalid_harness("chain bundle checkpoint range predicate is unavailable"));
        };
...
fn parse_chain_bundle_artifacts(value: &Value<IoValue>) -> Result<Vec<ChainBundleArtifact>> {
    let artifacts = value
        ...
        .ok_or_else(|| Failure::invalid_harness("expected <artifacts ...> field"))?;
    let sequence = artifacts[0]
        .collect_sequence()
        .ok_or_else(|| Failure::invalid_harness("chain bundle artifacts must be a sequence"))?;
    let mut parsed = Vec::new();
    for item in sequence.iter() {
        let item = value_to_iovalue(item);
        let artifact = item
            .collect_simple_record("artifact", Some(3))
            .ok_or_else(|| Failure::invalid_harness("expected <artifact kind ref value> in chain bundle"))?;
    ...
    if parsed.is_empty() {
        return Err(Failure::invalid_harness("chain bundle must contain artifacts"));
    }
    Ok(parsed)
}
...
fn parse_chain_verify_receipt_value(value: &IoValue) -> Result<ParsedChainVerifyReceipt> {
    let receipt = value
        ...
        .ok_or_else(|| Failure::invalid_harness("expected chain verify receipt in chain bundle"))?;
...
fn parse_diagnostic_kinds(value: &Value<IoValue>) -> Result<Vec<String>> {
    let diagnostics = value
        ...
        .ok_or_else(|| Failure::invalid_harness("expected chain verify diagnostics field"))?;
    let values = diagnostics[0]
        .collect_sequence()
        .ok_or_else(|| Failure::invalid_harness("chain verify diagnostics must be a sequence"))?;
    values
        .iter()
        .map(|diagnostic| {
            ...
            let record = diagnostic
                ...
                .ok_or_else(|| Failure::invalid_harness("expected diagnostic record"))?;
