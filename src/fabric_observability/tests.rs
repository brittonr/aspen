...
fn capability_rooted_durable_scan_detects_corruption_partial_and_overbound_without_mutation() {
    ...
    let namespace =
        molten_node_host::node_state::DirectoryView::open(molten_node_host::node_state::NamespaceKind::Ledger, &workspace)
            .expect("ledger namespace");
    let path = molten_node_host::node_state::RelativePath::parse("record.bin").expect("record path");
