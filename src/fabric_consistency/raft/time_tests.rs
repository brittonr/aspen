...
impl CryptographicEntropySource for FixedEntropySource {
    ...
    fn fill_secret(&mut self, output: &mut [u8]) -> crate::fabric::FabricPortResult<()> {
        output.fill(0);
        Ok(())
    }
}
...
async fn tokio_replica_time_port_delivers_entropy_bound_election_and_heartbeat_events() {
    ...
    let timer_ref = super::tests::test_ref("election-timer-token");
    let election_evidence = port.arm_election_timer(&timer_ref).expect("election timer");
    assert!(election_evidence.starts_with("blake3:"));
    let election_event =
        tokio::time::timeout(std::time::Duration::from_millis(EVENT_TIMEOUT_MILLISECONDS), receiver.recv())
            .await
...
async fn rearming_election_timer_cancels_the_superseded_delivery() {
    ...
    port.arm_election_timer(&super::tests::test_ref("superseded-timer-token")).expect("first timer");
    let replacement_timer_ref = super::tests::test_ref("replacement-timer-token");
    port.arm_election_timer(&replacement_timer_ref).expect("replacement timer");
    let delivered = tokio::time::timeout(std::time::Duration::from_millis(EVENT_TIMEOUT_MILLISECONDS), receiver.recv())
        .await
...
) -> crate::error::Result<(TokioReplicaTimePort<FixedEntropySource>, tokio::sync::mpsc::UnboundedReceiver<ReplicaEvent>)>
{
    ...
    let config = TokioReplicaTimeConfig {
        ...
        capability_ref: super::tests::test_ref("raft-time-capability"),
        entropy_binding_ref: entropy_binding_ref(),
        tick_duration: std::time::Duration::from_millis(TICK_MILLISECONDS),
        heartbeat_ticks,
        election_min_ticks,
        election_max_ticks,
    };
...
fn entropy_binding_ref() -> String {
    super::tests::test_ref(ENTROPY_BINDING_LABEL)
}
