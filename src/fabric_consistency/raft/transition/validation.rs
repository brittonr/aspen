...
pub(super) fn validate_incoming_entries(
    ...
    if entries.len() > state.profile.max_message_entries {
        return Err(crate::error::MoltenError::invalid_harness(
            "Raft append entries exceed the admitted message bound",
        ));
    }
    let mut expected_index = prev_log_index
        .checked_add(NEXT_LOG_INDEX_STEP)
        .ok_or_else(|| crate::error::MoltenError::invalid_harness("Raft append index overflow"))?;
    let mut request_refs = std::collections::BTreeSet::new();
...
pub(super) fn validate_message_envelope(
    state: &ReplicaState,
    ...
    if envelope.group_binding_ref != state.profile.group_binding_ref {
        return Err(crate::error::MoltenError::invalid_harness("Raft message uses a substituted group binding"));
    }
    if envelope.service_generation != state.profile.service_generation {
        return Err(crate::error::MoltenError::invalid_harness("Raft message uses a stale service generation"));
    }
    if envelope.to != state.node_id || envelope.from == state.node_id {
        return Err(crate::error::MoltenError::invalid_harness("Raft message endpoint identity is invalid"));
    }
    if !state.membership.voters.contains(&envelope.from) || !state.membership.voters.contains(&envelope.to) {
        return Err(crate::error::MoltenError::invalid_harness("Raft message endpoint is outside admitted membership"));
    }
    if envelope.message.term() == 0 {
        return Err(crate::error::MoltenError::invalid_harness("Raft protocol message cannot use term zero"));
    }
    if envelope.message.config_epoch() != state.membership.config_epoch
        || envelope.message.fencing_epoch() != state.profile.fencing_epoch
    {
        return Err(crate::error::MoltenError::invalid_harness(
            "Raft message uses a stale configuration or fencing epoch",
        ));
    }
...
pub(super) fn validate_replica_state(state: &ReplicaState) -> crate::error::Result<()> {
    if state.membership.voters.len() != STATIC_VOTER_COUNT || !state.membership.voters.contains(&state.node_id) {
        return Err(crate::error::MoltenError::invalid_harness("Raft state has invalid static membership"));
    }
    if state.log.len() > state.profile.max_log_entries {
        return Err(crate::error::MoltenError::invalid_harness("Raft state exceeds its admitted log bound"));
    }
    ...
    if state.last_applied > state.commit_index || state.commit_index > last {
        return Err(crate::error::MoltenError::invalid_harness(
            "Raft applied or committed index exceeds the local log",
        ));
    }
    validate_role(state)?;
    ...
    if state.voted_for.as_ref().is_some_and(|voter| !state.membership.voters.contains(voter)) {
        return Err(crate::error::MoltenError::invalid_harness("Raft vote target is outside admitted membership"));
    }
    Ok(())
}

fn validate_completed_requests(state: &ReplicaState) -> crate::error::Result<()> {
    for (request_ref, index) in &state.completed_requests {
        ...
        if *index == INITIAL_COMMIT_INDEX || *index > state.commit_index {
            return Err(crate::error::MoltenError::invalid_harness(
                "Raft completed request index is outside the committed range",
            ));
        }
    }
    for entry in state.log.iter().filter(|entry| entry.index <= state.commit_index) {
        if state.completed_requests.get(&entry.request_ref) != Some(&entry.index) {
            return Err(crate::error::MoltenError::invalid_harness(
                "Raft committed log entry is absent from idempotency state",
            ));
        }
    }
    Ok(())
...
fn validate_pending_reads(state: &ReplicaState) -> crate::error::Result<()> {
    if state.pending_reads.len() > MAX_PENDING_REPLICA_READS {
        return Err(crate::error::MoltenError::invalid_harness("Raft pending reads exceed their static bound"));
    }
    if !state.pending_reads.is_empty() && state.role != ReplicaRole::Leader {
        return Err(crate::error::MoltenError::invalid_harness("only a Raft leader may retain pending reads"));
    }
    for (request_ref, pending) in &state.pending_reads {
        ...
        {
            return Err(crate::error::MoltenError::invalid_harness("Raft pending read state is inconsistent"));
        }
    }
    Ok(())
}

fn validate_snapshot(state: &ReplicaState) -> crate::error::Result<()> {
    ...
    {
        return Err(crate::error::MoltenError::invalid_harness("Raft snapshot binding or identity mismatch"));
    }
    for (request_ref, index) in &snapshot.completed_requests {
        ...
        if *index == INITIAL_COMMIT_INDEX || *index > snapshot.last_included_index {
            return Err(crate::error::MoltenError::invalid_harness(
                "Raft snapshot completed request index is out of range",
            ));
        }
        if state.completed_requests.get(request_ref) != Some(index) {
            return Err(crate::error::MoltenError::invalid_harness(
                "Raft snapshot idempotency state is not retained locally",
            ));
        }
    }
    if snapshot.last_included_index == INITIAL_COMMIT_INDEX || snapshot.last_included_index > state.commit_index {
        return Err(crate::error::MoltenError::invalid_harness(
            "Raft snapshot boundary must be positive and no newer than commit",
        ));
    }
    Ok(())
}

fn validate_election_timer(state: &ReplicaState) -> crate::error::Result<()> {
    if state.election_timer_sequence == 0 {
        return Err(crate::error::MoltenError::invalid_harness("Raft election timer sequence must be positive"));
    }
    ...
    if state.active_election_timer_ref != expected {
        return Err(crate::error::MoltenError::invalid_harness("Raft active election timer ref does not match state"));
    }
    Ok(())
}

pub(super) fn validate_transition(transition: &ReplicaTransition) -> crate::error::Result<()> {
    validate_replica_state(&transition.next)?;
    if transition.effects.len() > transition.next.profile.max_effects_per_step {
        return Err(crate::error::MoltenError::invalid_harness("Raft transition exceeds its admitted effect bound"));
    }
    Ok(())
}

pub(super) fn ensure_running(state: &ReplicaState) -> crate::error::Result<()> {
    if state.lifecycle != ReplicaLifecycle::Running {
        return Err(crate::error::MoltenError::invalid_harness("Raft event requires a running replica"));
    }
    Ok(())
}

pub(super) fn validate_content_ref(value: &str, label: &str) -> crate::error::Result<()> {
...
fn validate_incoming_entry<'a>(
    ...
    if entry.index != expected_index || entry.term == 0 || entry.term > leader_term {
        return Err(crate::error::MoltenError::invalid_harness(
            "Raft append entries are not contiguous or use an invalid term",
        ));
    }
    ...
    if !request_refs.insert(entry.request_ref.as_str()) {
        return Err(crate::error::MoltenError::invalid_harness("Raft append entries contain a duplicate request ref"));
    }
    if state
        .log
        .iter()
    ...
    {
        return Err(crate::error::MoltenError::invalid_harness(
            "Raft append reuses a request ref at a different index",
        ));
    }
    Ok(())
}

fn validate_message_sender(envelope: &ReplicaMessageEnvelope) -> crate::error::Result<()> {
    ...
    if embedded_sender != &envelope.from {
        return Err(crate::error::MoltenError::invalid_harness(
            "Raft message sender does not match its canonical envelope",
        ));
    }
    Ok(())
}

fn validate_message_request_ref(message: &RaftMessage) -> crate::error::Result<()> {
    match message {
...
fn validate_log(state: &ReplicaState) -> crate::error::Result<()> {
    let mut expected = match &state.snapshot {
        Some(snapshot) => snapshot
            .last_included_index
            .checked_add(NEXT_LOG_INDEX_STEP)
            .ok_or_else(|| crate::error::MoltenError::invalid_harness("Raft snapshot log index overflow"))?,
    ...
    for entry in &state.log {
        ...
        expected = expected
            .checked_add(NEXT_LOG_INDEX_STEP)
            .ok_or_else(|| crate::error::MoltenError::invalid_harness("Raft log index overflow"))?;
    }
    Ok(())
}
