...
pub struct IrohReplicaTransportPort {
    protocol_ref: String,
    peers: std::collections::BTreeMap<String, crate::fabric_transport::IrohCrossProcessClientInput>,
...
pub trait ReplicaListenerSource {
    fn admitted_listener(&mut self) -> crate::error::Result<&mut crate::fabric_transport::IrohCrossProcessListener>;
}

impl ReplicaListenerSource for crate::fabric_transport::IrohCrossProcessListener {
    fn admitted_listener(&mut self) -> crate::error::Result<&mut crate::fabric_transport::IrohCrossProcessListener> {
        Ok(self)
    }
}

impl ReplicaListenerSource for Option<crate::fabric_transport::IrohCrossProcessListener> {
    fn admitted_listener(&mut self) -> crate::error::Result<&mut crate::fabric_transport::IrohCrossProcessListener> {
        self.as_mut()
            .ok_or_else(|| crate::error::MoltenError::invalid_harness("live Raft listener is detached from its owner"))
    }
}
...
pub struct IrohReplicaIngressPump {
    ...
    task: Option<tokio::task::JoinHandle<crate::error::Result<crate::fabric_transport::IrohCrossProcessListener>>>,
}
    ...
    pub fn new(
        ...
        if peers.is_empty() {
            return Err(crate::error::MoltenError::invalid_harness(
                "live Raft Iroh transport requires at least one admitted peer",
            ));
        }
        if timeout.is_zero() {
            return Err(crate::error::MoltenError::invalid_harness(
                "live Raft Iroh transport timeout must be positive",
            ));
        }
...
impl IrohReplicaIngressPump {
    pub fn spawn(
        ...
        config: IrohReplicaIngressConfig,
    ) -> crate::error::Result<Self> {
                ...
                tokio::select! {
                    ...
                    result = receive_replica_event(&mut listener, &config.session_ref, config.accept_timeout) => {
                        ...
                        event_sender
                            .send(event)
                            .await
                            .map_err(|_| crate::error::MoltenError::invalid_harness("live Raft ingress consumer closed"))?;
                    }
                }
            }
            Err(crate::error::MoltenError::invalid_harness("live Raft ingress exhausted its delivery limit"))
        });
        Ok(Self {
            events,
            cancel: Some(cancel),
    ...
    pub async fn next(&mut self) -> crate::error::Result<ReceivedReplicaEvent> {
        self.events.recv().await.ok_or_else(|| {
            crate::error::MoltenError::invalid_harness("live Raft ingress pump terminated before delivery")
        })
    }

    pub async fn shutdown(mut self) -> crate::error::Result<crate::fabric_transport::IrohCrossProcessListener> {
        let cancel = self.cancel.take().ok_or_else(|| {
            crate::error::MoltenError::invalid_harness("live Raft ingress cancellation handle is absent")
        })?;
        let _cancel_result = cancel.send(());
        let task = self
            .task
            .take()
            .ok_or_else(|| crate::error::MoltenError::invalid_harness("live Raft ingress task handle is absent"))?;
        task.await.map_err(|error| {
            crate::error::MoltenError::invalid_harness(format!("live Raft ingress task failed: {error}"))
        })?
    }
}
...
impl ReplicaTransportEffects for IrohReplicaTransportPort {
    fn send<'a>(&'a mut self, envelope: &'a ReplicaMessageEnvelope) -> ReplicaTransportFuture<'a> {
        ...
        Box::pin(async move {
            ...
            let evidence =
                crate::fabric_transport::exchange_cross_process_frame(input, &message.bytes, timeout).await?;
            if evidence.delivery != crate::fabric_transport::DeliveryOutcome::Delivered {
                return Err(crate::error::MoltenError::invalid_harness(
...
pub async fn receive_replica_event<L: ReplicaListenerSource>(
    ...
    session_ref: &str,
    timeout: std::time::Duration,
) -> crate::error::Result<ReceivedReplicaEvent> {
    ...
    let payload_bytes = u64::try_from(received.payload.len())
        .map_err(|_| crate::error::MoltenError::invalid_harness("live Raft received payload length exceeds u64"))?;
    if received.evidence.request_ref != refs.request_ref || received.evidence.payload_bytes != payload_bytes {
        return Err(crate::error::MoltenError::invalid_harness(
            "live Raft received payload shape does not match transport evidence",
        ));
    }
...
pub fn replica_transport_refs(envelope: &ReplicaMessageEnvelope) -> crate::error::Result<ReplicaTransportRefs> {
...
fn prepare_send(
    ...
    crate::fabric_transport::IrohCrossProcessClientInput,
    CanonicalReplicaMessage,
    std::time::Duration,
)> {
    let message = canonical_replica_message(envelope)?;
    let mut input = peers.get(&envelope.to).cloned().ok_or_else(|| {
        crate::error::MoltenError::invalid_harness("live Raft Iroh transport has no admitted endpoint for peer")
    })?;
    input.request_ref.clone_from(&message.envelope_ref);
    Ok((input, message, timeout))
}

fn derive_replica_request_ref(payload: &[u8]) -> crate::error::Result<String> {
    let envelope = parse_canonical_replica_message(payload)?;
    Ok(canonical_replica_message(&envelope)?.envelope_ref)
}

pub(super) fn validate_ingress_config(config: &IrohReplicaIngressConfig) -> crate::error::Result<()> {
    crate::preserves_rail::validate_content_ref(&config.session_ref)?;
    if config.accept_timeout.is_zero() {
        return Err(crate::error::MoltenError::invalid_harness("live Raft ingress timeout must be positive"));
    }
    if config.event_capacity == 0 || config.event_capacity > MAX_REPLICA_INGRESS_EVENTS {
        return Err(crate::error::MoltenError::invalid_harness(
            "live Raft ingress capacity is outside its static bound",
        ));
    }
    if config.delivery_limit == 0 || config.delivery_limit > MAX_REPLICA_INGRESS_DELIVERIES {
        return Err(crate::error::MoltenError::invalid_harness(
            "live Raft ingress delivery limit is outside its static bound",
        ));
    }
    Ok(())
}

fn validate_peer_id(peer: &str) -> crate::error::Result<()> {
    if peer.is_empty()
        || !peer.bytes().all(|byte| byte.is_ascii_alphanumeric() || matches!(byte, b'-' | b'_' | b'.' | b':'))
    {
        return Err(crate::error::MoltenError::invalid_harness("live Raft Iroh peer id is empty or malformed"));
    }
    Ok(())
}
