...
fn read_endpoints(run_directory: &Path) -> Result<std::collections::BTreeMap<String, CanonicalCrossProcessEndpoint>> {
...
async fn propose(node: &mut crate::fabric_consistency::raft::live_cluster::LiveNode) -> Result<()> {
    require_applied(
        node.service
...
async fn begin_read(node: &mut crate::fabric_consistency::raft::live_cluster::LiveNode) -> Result<()> {
    require_applied(
        node.service
...
async fn begin_snapshot_catch_up(node: &mut crate::fabric_consistency::raft::live_cluster::LiveNode) -> Result<()> {
    require_applied(
        node.service
