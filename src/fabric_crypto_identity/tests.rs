...
fn adapter<'a>(namespace: &'a molten_node_host::node_state::DirectoryView) -> IrohEd25519FileAdapter<'a> {
...
fn production_file_key_is_random_persisted_restricted_and_restart_stable() {
    ...
    let namespace =
        molten_node_host::node_state::DirectoryView::open(molten_node_host::node_state::NamespaceKind::Identity, &workspace)
    ...
    let secret_record = namespace.read(&key_path, molten_node_host::node_state::MAX_NODE_SECRET_BYTES).expect("secret record");
    let status = adapter
...
fn production_sign_verify_binds_domain_and_denies_wrong_purpose_or_payload() {
    ...
    let namespace =
        molten_node_host::node_state::DirectoryView::open(molten_node_host::node_state::NamespaceKind::Secrets, &workspace)
    ...
    let other_namespace = molten_node_host::node_state::DirectoryView::open(
...
fn fixture_profile_and_missing_or_unsafe_keys_fail_closed() {
    ...
    let malformed_namespace = molten_node_host::node_state::DirectoryView::open(
        molten_node_host::node_state::NamespaceKind::Secrets,
        &malformed_workspace,
    )
    .expect("malformed secrets namespace");
    let malformed_path = molten_node_host::node_state::RelativePath::parse("crypto-authority.key").expect("authority key path");
    malformed_namespace
...
fn artifact_auth_shell_signs_and_verifies_exact_statement_without_admitting_authority() {
    ...
    let secret_path = molten_node_host::node_state::RelativePath::parse("crypto-evidence-signing.key").expect("secret path");
    let secret_record = namespace.read(&secret_path, molten_node_host::node_state::MAX_NODE_SECRET_BYTES).expect("secret record");
...
fn artifact_auth_operational_receipt_survives_restart_and_rejects_rotation() {
    ...
    let root = molten_node_host::node_state::Root::open(&workspace).expect("node state root");
    ...
    let reopened_root = molten_node_host::node_state::Root::open_existing(&workspace).expect("reopen node state");
