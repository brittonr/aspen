...
mod tests {
    ...
    fn capability_root_reads_and_writes_valid_relative_paths() {
        ...
        let path = RelativeLocator::parse("chunks/ok.bin").expect("relative path");
        ...
        artifact_root
            .root()
            .write(&RelativeLocator::parse("objects/value.preserves").expect("artifact path"), b"artifact")
            .expect("write artifact");
        retention_root
            .root()
            .write(&RelativeLocator::parse("pins/pin.preserves").expect("retention path"), b"pin")
            .expect("write retention");
        dataspace_root
            .root()
            .write(&RelativeLocator::parse("envelopes/message.preserves").expect("dataspace path"), b"message")
            .expect("write dataspace");
        let chunks_path = RelativeLocator::parse("chunks").expect("chunks path");
        ...
        assert_eq!(derived.root().kind(), Category::Exchange);
        assert_eq!(
            derived
                .root()
                .read(&RelativeLocator::parse("ok.bin").expect("derived path"))
    ...
    fn capability_roots_deny_invalid_local_locators_and_missing_authority() {
        ...
        assert!(
            RelativeLocator::parse("../escape")
                .expect_err("parent traversal denied")
                .to_string()
                .contains("parent")
        );
        assert!(RelativeLocator::parse("/tmp/escape").expect_err("absolute denied").to_string().contains("relative"));
        assert!(
            RelativeLocator::parse("iroh://peer/blob")
                .expect_err("remote locator denied")
                .to_string()
                .contains("local filesystem")
        );
        assert!(
            RelativeLocator::parse("blake3:abc")
                .expect_err("content ref denied")
                .to_string()
                .contains("local filesystem")
        );
        assert!(
            RelativeLocator::parse(r"C:\escape")
                .expect_err("drive-prefixed path denied")
                .to_string()
                .contains("platform-prefixed")
        );
        assert!(
            RelativeLocator::parse(r"\\server\share")
    ...
    fn capability_root_denies_symlink_escape() {
        ...
        let error = root
            .root()
            .read(&RelativeLocator::parse("entries/escape-link").expect("symlink path"))
        ...
        let intermediate_error = root
            .root()
            .read(&RelativeLocator::parse("entries/escape-directory/secret.txt").expect("intermediate path"))
            .expect_err("intermediate symlink escape denied");
        assert!(!intermediate_error.to_string().is_empty());
        let entries = root
            .root()
            .list_entries(&RelativeLocator::parse("entries").expect("entry directory"))
            .expect("list logical entries");
        assert_eq!(entries.len(), 3);
        assert_eq!(entries[0].name, "escape-directory");
        assert_eq!(entries[0].kind, ObjectKind::Symlink);
        assert_eq!(entries[1].name, "escape-link");
        assert_eq!(entries[1].kind, ObjectKind::Symlink);
        ...
        let replacement_error = root
            .root()
            .read(&RelativeLocator::parse("entries/replace-me").expect("replacement path"))
    ...
    fn typed_roots_do_not_substitute_same_relative_handle_across_authorities() {
        ...
        let locator = RelativeLocator::parse("shared/value.bin").expect("shared locator");
    ...
    fn database_handle_rejects_symlink_and_non_regular_leaf() {
        ...
        let symlink_error = root
            .root()
            .open_database_file(&RelativeLocator::parse("symlink.redb").expect("symlink path"))
            .expect_err("database symlink denied");
        assert!(symlink_error.to_string().contains("regular file"));
        let directory_error = root
            .root()
            .open_database_file(&RelativeLocator::parse("directory.redb").expect("directory path"))
    ...
    fn open_capability_survives_ambient_root_replacement_without_switching_authority() {
        ...
        let logical_path = RelativeLocator::parse("bound.txt").expect("bound path");
