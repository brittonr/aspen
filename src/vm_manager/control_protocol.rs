...
mod tests {
    ...
    #[tokio::test]
    async fn test_control_protocol() {
        ...
        tokio::spawn(async move {
            let _ = server_protocol.start_server().await;
        });
        ...
        // Test ping
        let client = ControlClient::new(vm_id, socket_path.clone());
        match client.ping().await {
            ...
            Err(e) => {
                // Server might not be ready in test environment
                eprintln!("Ping test skipped: {}", e);
            }
        }
    }
}