//! VmManagement trait - Abstract interface for VM lifecycle operations
//!
//! This trait defines the core operations needed for VM management,
//! allowing domain services to depend on abstractions rather than
...
use super::{
    VmConfig, VmInstance, VmState, VmStats, VmAssignment, JobResult,
...
pub trait VmManagement: Send + Sync {
    ...
    async fn submit_job_to_vm(&self, vm_id: Uuid, job: crate::Job) -> Result<JobResult>;

    // === VM Queries ===

    /// Get a VM instance by ID
    async fn get_vm(&self, vm_id: Uuid) -> Result<Option<Arc<tokio::sync::RwLock<VmInstance>>>>;
