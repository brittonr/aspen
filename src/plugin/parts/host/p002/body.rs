...
pub fn plugin_host_abi_result_value(input: &HostAbiResultInput<'_>) -> Result<IoValue> {
    ...
    if input.status == "ok" && input.error.is_some() {
        return Err(Failure::invalid_harness("successful plugin ABI result must not carry an error"));
    }
    if input.status == "error" && input.error.is_none() {
        return Err(Failure::invalid_harness("error plugin ABI result requires an error message"));
    }
