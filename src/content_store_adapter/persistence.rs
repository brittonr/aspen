...
pub fn persist_partial_state(
    ...
    if !issues.is_empty() {
        return Err(Failure::invalid_harness(format!("partial state persistence denied: {issues:?}")));
    }
    let encoded = encode_partial_state(state);
    let encoded_length = u64::try_from(encoded.len())
        .map_err(|_| Failure::invalid_harness("partial state length does not fit u64"))?;
    if encoded_length > MAX_PARTIAL_STATE_BYTES {
        return Err(Failure::invalid_harness("partial state exceeds persistence bound"));
    }
...
pub fn load_partial_state(
    ...
    let bytes = match namespace.observe_file(&path)? {
        FileObservation::Missing => return Ok(None),
        FileObservation::Regular(file) => file.read_bounded(MAX_PARTIAL_STATE_BYTES)?,
        FileObservation::NonRegular(kind) => {
            return Err(Failure::invalid_harness(format!("partial state must be a regular file, got {kind:?}")));
        }
    };
    let text = std::str::from_utf8(&bytes).map_err(|_| Failure::invalid_harness("partial state is not UTF-8"))?;
    let state = decode_partial_state(text)?;
    if state.operation_ref != operation_ref {
        return Err(Failure::invalid_harness("partial state operation binding mismatch"));
    }
    let issues = validate_partial_state(profile, manifest, &state);
    if !issues.is_empty() {
        return Err(Failure::invalid_harness(format!("persisted partial state denied: {issues:?}")));
    }
    Ok(Some(state))
}

pub fn remove_partial_state(namespace: &DirectoryView, operation_ref: &str) -> Result<()> {
    let path = partial_state_path(operation_ref)?;
    match namespace.observe_file(&path)? {
        FileObservation::Missing => Ok(()),
        FileObservation::Regular(_) => namespace.remove_regular_file(&path),
        FileObservation::NonRegular(kind) => {
            Err(Failure::invalid_harness(format!("partial state removal requires a regular file, got {kind:?}")))
        }
    }
}
...
fn decode_partial_state(text: &str) -> Result<ContentPartialState> {
    ...
    if lines.next().is_some() {
        return Err(Failure::invalid_harness("partial state has trailing fields"));
    }
...
fn partial_state_path(operation_ref: &str) -> Result<RelativePath> {
    let hex = operation_ref
        .strip_prefix("blake3:")
        .filter(|value| value.len() == BLAKE3_HEX_LENGTH)
        .ok_or_else(|| Failure::invalid_harness("partial state operation ref is malformed"))?;
    RelativePath::parse(&format!("content-partial-{hex}.state"))
}
...
fn require_line<'a>(lines: &mut impl Iterator<Item = &'a str>, field: &str, expected: &str) -> Result<()> {
    ...
    if actual == expected {
        ...
        Err(Failure::invalid_harness(format!("{field} mismatch")))
    }
}

fn next_line<'a>(lines: &mut impl Iterator<Item = &'a str>, field: &str) -> Result<&'a str> {
    lines.next().ok_or_else(|| Failure::invalid_harness(format!("partial state lacks {field}")))
}
...
fn parse_u64(value: &str, field: &str) -> Result<u64> {
    value.parse().map_err(|_| Failure::invalid_harness(format!("{field} is not u64")))
}

fn parse_usize(value: &str, field: &str) -> Result<usize> {
    value.parse().map_err(|_| Failure::invalid_harness(format!("{field} is not usize")))
}
...
fn parse_terminal(value: &str) -> Result<ContentTerminal> {
    match value {
        ...
        _ => Err(Failure::invalid_harness("partial state terminal is unsupported")),
    }
}
...
fn parse_optional_failure(value: &str) -> Result<Option<ContentFailure>> {
    ...
    let failure = match value {
        ...
        _ => return Err(Failure::invalid_harness("partial state failure is unsupported")),
    };
    Ok(Some(failure))
}
