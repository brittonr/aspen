...
pub fn minimal_plugin_fixture(root: &Path) -> Result<PluginFixtureRun> {
    ...
    let policy_ref = plugin_ref("policy")?;
    let resource_ref = plugin_ref("resource")?;
    let schema_ref = plugin_ref("schema")?;
    let effect_manifest_ref = plugin_ref("effect-manifest")?;
    let supply_chain_ref = plugin_ref("supply-chain")?;
    let authority_ref = plugin_ref("authority")?;
    let executor_ref = plugin_ref("executor-preflight")?;
    let effect_receipt_ref = plugin_ref("effect-receipt")?;
    let hostcall_ref = storage_read_hostcall_ref()?;
    let installed = artifacts::install_artifact(&registry, &artifacts::ArtifactInstallInput {
        kind: "plugin-executor".to_string(),
        payload: record("reviewed-plugin-executor", vec![string("minimal")]),
        schema_refs: vec![schema_ref.clone()],
        dependency_refs: Vec::new(),
        effect_manifest_ref: Some(effect_manifest_ref.clone()),
        policy_refs: vec![policy_ref.clone()],
        evidence_refs: vec![supply_chain_ref.clone()],
        installer_ref: authority_ref.clone(),
        capability_refs: vec![authority_ref.clone()],
    })?;
    let manifest_value = plugin_manifest_value(&PluginManifestInput {
        ...
        effect_manifest_refs: std::slice::from_ref(&effect_manifest_ref),
        hostcall_refs: std::slice::from_ref(&hostcall_ref),
        schema_refs: std::slice::from_ref(&schema_ref),
        policy_refs: std::slice::from_ref(&policy_ref),
        resource_refs: std::slice::from_ref(&resource_ref),
        supply_chain_refs: std::slice::from_ref(&supply_chain_ref),
    })?;
    let manifest = parse_plugin_manifest(&manifest_value)?;
    let install = install_plugin(&registry, &manifest_value)?;
    let permission_value = plugin_permission_receipt_value(&PermissionReviewInput {
        manifest_value: &manifest_value,
        authority_refs: std::slice::from_ref(&authority_ref),
        policy_refs: std::slice::from_ref(&policy_ref),
        resource_refs: std::slice::from_ref(&resource_ref),
        effect_receipt_refs: std::slice::from_ref(&effect_receipt_ref),
        supply_chain_refs: std::slice::from_ref(&supply_chain_ref),
    })?;
    let permission = parse_plugin_permission_receipt(&permission_value)?;
    let init_value = plugin_lifecycle_receipt_value(&LifecycleReceiptInput {
        operation: "init",
        manifest_value: &manifest_value,
    ...
    let init = parse_plugin_lifecycle_receipt(&init_value)?;
    let start_value = plugin_lifecycle_receipt_value(&LifecycleReceiptInput {
        operation: "start",
        manifest_value: &manifest_value,
        permission_receipt_ref: &permission.receipt_ref,
        executor_receipt_ref: &executor_ref,
    ...
    let start = parse_plugin_lifecycle_receipt(&start_value)?;
    let hostcall_value = plugin_hostcall_receipt_value(&HostcallReceiptInput {
        manifest_value: &manifest_value,
        operation: "storage.read",
        hostcall_ref: &hostcall_ref,
        executor_receipt_ref: &executor_ref,
        effect_receipt_ref: &effect_receipt_ref,
        authority_refs: std::slice::from_ref(&authority_ref),
        resource_refs: std::slice::from_ref(&resource_ref),
    })?;
    let hostcall = parse_plugin_hostcall_receipt(&hostcall_value)?;
    let service_ref = plugin_ref("service-supervision")?;
    let health_value = plugin_health_receipt_value(&HealthReceiptInput {
        manifest_value: &manifest_value,
        lifecycle_receipt_ref: &start.receipt_ref,
        service_refs: std::slice::from_ref(&service_ref),
        health_status: "healthy",
        diagnostics: &[],
    })?;
    let health = parse_plugin_health_receipt(&health_value)?;
    let stop_value = plugin_lifecycle_receipt_value(&LifecycleReceiptInput {
        operation: "stop",
        manifest_value: &manifest_value,
        permission_receipt_ref: &permission.receipt_ref,
        executor_receipt_ref: &executor_ref,
    ...
    let stop = parse_plugin_lifecycle_receipt(&stop_value)?;
    let remove_lifecycle_value = plugin_lifecycle_receipt_value(&LifecycleReceiptInput {
        operation: "remove",
        manifest_value: &manifest_value,
        permission_receipt_ref: &permission.receipt_ref,
        executor_receipt_ref: &executor_ref,
    ...
    let remove_lifecycle = parse_plugin_lifecycle_receipt(&remove_lifecycle_value)?;
    ...
    let removal_value = plugin_removal_receipt_value(&RemovalReceiptInput {
        manifest_value: &manifest_value,
        lifecycle_receipt_ref: &remove_lifecycle.receipt_ref,
    ...
    let removal = parse_plugin_removal_receipt(&removal_value)?;
    let upgraded = artifacts::install_artifact(&registry, &artifacts::ArtifactInstallInput {
        kind: "plugin-executor".to_string(),
        payload: record("reviewed-plugin-executor", vec![string("minimal-v2")]),
        schema_refs: vec![schema_ref.clone()],
        dependency_refs: Vec::new(),
        effect_manifest_ref: Some(effect_manifest_ref.clone()),
        policy_refs: vec![policy_ref.clone()],
    ...
    let upgraded_manifest_value = plugin_manifest_value(&PluginManifestInput {
        plugin_id: "plugin:minimal",
        artifact_ref: &upgraded.artifact_ref,
        abi: PLUGIN_HOST_ABI_VERSION,
        lifecycle_callbacks: &string_vec(&["init", "start", "health", "stop", "remove"]),
        effect_manifest_refs: std::slice::from_ref(&effect_manifest_ref),
        hostcall_refs: std::slice::from_ref(&hostcall_ref),
    ...
    let upgrade_value = plugin_upgrade_receipt_value(&UpgradeReceiptInput {
        old_manifest_value: &manifest_value,
        new_manifest_value: &upgraded_manifest_value,
        rollback_ref: &plugin_ref("rollback")?,
        cleanup_refs: std::slice::from_ref(&removal.receipt_ref),
        diagnostics: &[],
    })?;
    let upgrade = parse_plugin_upgrade_receipt(&upgrade_value)?;
    let evidence_values = vec![
        manifest_value.clone(),
        install.value.clone(),
        permission_value.clone(),
        init_value,
        start_value.clone(),
        hostcall_value.clone(),
        health_value.clone(),
        stop_value.clone(),
        remove_lifecycle_value,
        removal_value.clone(),
    ...
    let report_value = plugin_fixture_report_value(&PluginFixtureReportInput {
        ...
        hostcall_receipt_ref: &hostcall.receipt_ref,
        health_receipt_ref: &health.receipt_ref,
        stop_receipt_ref: &stop.receipt_ref,
    ...
    let decision = if [
        ...
        init.decision.as_str(),
        start.decision.as_str(),
        hostcall.decision.as_str(),
        health.decision.as_str(),
        stop.decision.as_str(),
        removal.decision.as_str(),
        upgrade.decision.as_str(),
    ]
    .iter()
    .all(|decision| *decision == "pass")
    {
        "pass".to_string()
    } else {
        "deny".to_string()
