...
fn validate_actor_executor_preflight(actor: &ActorDecl, preflight: &ExecutorPreflightEvidence) -> Result<()> {
    ...
    match (&actor.kind, &actor.executor) {
        ...
        (ActorKind::Steel, None) => Err(crate::error::Failure::invalid_harness(format!(
            "steel actor {} missing reviewed Steel executor preflight fixture",
            actor.id
        ))),
        (ActorKind::Wasm, None) => Err(crate::error::Failure::invalid_harness(format!(
            "wasm actor {} missing Wasm executor preflight fixture",
            actor.id
        ))),
        (ActorKind::Adapter | ActorKind::RemoteProxy, _) => Err(crate::error::Failure::invalid_harness(format!(
            "executor kind {} requires executor adapter preflight and remains disabled in local harness",
            actor.kind.as_str()
        ))),
        (ActorKind::Steel, Some(_)) | (ActorKind::Wasm, Some(_)) => Err(crate::error::Failure::invalid_harness(format!(
            "actor {} kind {} has mismatched executor preflight fixture",
            actor.id,
            actor.kind.as_str()
        ))),
        (ActorKind::Native, Some(_)) => Err(crate::error::Failure::invalid_harness(format!(
...
fn validate_steel_preflight(
    ...
    let review = preflight.steel_review.as_ref().ok_or_else(|| {
        crate::error::Failure::invalid_harness(format!("Steel executor preflight missing review receipt for actor {}", actor.id))
    })?;
...
fn parse_optional_steel_review_receipt(receipts: &[IoValue]) -> Result<Option<SteelReviewReceipt>> {
    ...
    for receipt in receipts {
        if receipt.collect_simple_record("steel-review-receipt-v1", None).is_some() {
            if parsed.is_some() {
                return Err(crate::error::Failure::invalid_harness("duplicate Steel review receipt in executor preflight"));
            }
...
fn parse_steel_review_receipt(value: &IoValue) -> Result<SteelReviewReceipt> {
    ...
    if decision != "pass" {
        return Err(crate::error::Failure::invalid_harness(format!("unsupported Steel review receipt decision {decision}")));
    }
...
fn parse_optional_wasm_inspection_receipt(receipts: &[IoValue]) -> Result<Option<WasmInspectionReceipt>> {
    ...
    for receipt in receipts {
        if receipt.collect_simple_record("wasm-inspection-receipt-v1", None).is_some() {
            if parsed.is_some() {
                return Err(crate::error::Failure::invalid_harness("duplicate Wasm inspection receipt in executor preflight"));
            }
