...
fn validated_step_ref(position: usize, step: &super::core::CoreStep, observation: &Observation) -> Result<String> {
    ...
    if observation.step_ref != step_ref {
        return Err(crate::error::Failure::invalid_harness(format!("hostcall step ref mismatch at observation {position}")));
    }
    Ok(step_ref)
}
...
fn actor_output_slot(position: usize, observation: &Observation) -> Result<(usize, &IoValue)> {
    ...
    let Some(actor_output_event) = observation.events.as_slice().get(actor_output_index) else {
        return Err(crate::error::Failure::invalid_harness(format!("missing final actor output at observation {position}")));
    };
...
fn validate_steel_execution_receipt(
    ...
    let receipt_value = value
        ...
        .ok_or_else(|| crate::error::Failure::invalid_harness("expected <steel-execution-receipt-v1 ...>"))?;
    ...
    let ActorExecutorConfig::Steel(config) = actor.executor.as_ref().ok_or_else(|| {
        crate::error::Failure::invalid_harness(format!("Steel actor {} missing Steel executor config", actor.id))
    })?
    else {
        return Err(crate::error::Failure::invalid_harness(format!("Steel actor {} has non-Steel executor config", actor.id)));
    };
...
fn validate_steel_execution_resources(value: &Value<IoValue>) -> Result<()> {
    ...
    if fuel_remaining > fuel_limit {
        return Err(crate::error::Failure::invalid_harness("Steel execution remaining fuel exceeds limit"));
    }
    ...
    if hostcall_count > hostcall_limit {
        return Err(crate::error::Failure::invalid_harness("Steel execution hostcall count exceeds limit"));
    }
    Ok(())
}
