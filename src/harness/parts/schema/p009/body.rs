...
fn validate_wasm_execution_receipt(
    ...
    let receipt_value = value
        ...
        .ok_or_else(|| crate::error::Failure::invalid_harness("expected <wasm-execution-receipt-v1 ...>"))?;
    ...
    let ActorExecutorConfig::Wasm(config) = actor
        .executor
        .as_ref()
        .ok_or_else(|| crate::error::Failure::invalid_harness(format!("Wasm actor {} missing Wasm executor config", actor.id)))?
    else {
        return Err(crate::error::Failure::invalid_harness(format!("Wasm actor {} has non-Wasm executor config", actor.id)));
    };
...
fn validate_wasm_execution_resources(receipt: &Record<Value<IoValue>>) -> Result<()> {
    ...
    if fuel_remaining > fuel_limit {
        return Err(crate::error::Failure::invalid_harness("Wasm execution remaining fuel exceeds limit"));
    }
...
pub fn parse_admission_decision_event(value: &IoValue) -> Result<AdmissionDecisionEvent> {
    let admission = value
        ...
        .ok_or_else(|| crate::error::Failure::invalid_harness("expected <admission-decision-v1 ...>"))?;
