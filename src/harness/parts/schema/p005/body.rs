...
pub fn parse_report(report_value: &IoValue) -> Result<Report> {
    let report = report_value
        ...
        .ok_or_else(|| crate::error::Failure::invalid_harness("expected <harness-report-v1 ...>"))?;
...
fn parsed_header(report: &Record<Value<IoValue>>) -> Result<ParsedHeader> {
    ...
    if status != "pass" {
        return Err(crate::error::Failure::invalid_harness(format!("evidence-bearing report status must be pass, got {status}")));
    }
    ...
    if !matches!(replay_status.as_str(), "deterministic" | "replay" | "record") {
        return Err(crate::error::Failure::invalid_harness(format!("unsupported evidence replay status {replay_status}")));
    }
...
fn registry_after_gates(
    ...
    if actors != suite.actors {
        return Err(crate::error::Failure::invalid_harness("report actor registry does not match embedded suite actor registry"));
    }
    Ok((cursor + 1, actors))
}
...
fn log_and_budget(
    ...
    if budget.limits != suite.budget {
        return Err(crate::error::Failure::invalid_harness("report budget limits do not match embedded suite budget"));
    }
...
pub fn parse_budget_gate(value: &IoValue) -> Result<BudgetGateEvidence> {
    ...
    if decision != "pass" {
        return Err(crate::error::Failure::invalid_harness(format!("unsupported budget gate decision {decision}")));
    }
    ...
    if nickel_source.budget_ref != budget_ref {
        return Err(crate::error::Failure::invalid_harness("Nickel resource policy budget ref does not match budget gate ref"));
    }
