...
pub fn export_world_replay_capsule(
    ...
    for member in &request.capsule.members {
        let payload = payloads
            ...
            .ok_or_else(|| Failure::invalid_harness("world replay export payload is missing"))?;
        ...
        if observation.object_ref != member.object_ref {
            return Err(Failure::invalid_harness("world replay exchange substituted the exported member identity"));
        }
...
pub fn fetch_world_replay_capsule(
    ...
    if locator_hints.len() != request.capsule.members.len() {
        return Err(Failure::invalid_harness("world replay locator set does not match the capsule member set"));
    }
    let mut payloads = Vec::with_capacity(request.capsule.members.len());
    for member in &request.capsule.members {
        let locator_hint = locator_hints
            .get(&member.object_ref)
            .ok_or_else(|| Failure::invalid_harness("world replay member locator is missing"))?;
        if locator_hint.is_empty() || locator_hint.len() > MAX_WORLD_REPLAY_TEXT_BYTES {
            return Err(Failure::invalid_harness("world replay member locator is empty or overbound"));
        }
    ...
    for object_ref in locator_hints.keys() {
        if !request.capsule.members.iter().any(|member| member.object_ref == *object_ref) {
            return Err(Failure::invalid_harness("world replay locator set contains an undeclared member"));
        }
    }
    Ok(payloads)
}
