...
pub(super) fn payload_map(payloads: &[WorldReplayMemberPayload]) -> Result<BTreeMap<&str, &WorldReplayMemberPayload>> {
    ...
    for payload in payloads {
        if map.insert(payload.object_ref.as_str(), payload).is_some() {
            return Err(Failure::invalid_harness("world replay payload set contains a duplicate member"));
        }
    }
    Ok(map)
}
...
pub(super) fn core_issues(issues: Vec<WorldReplayIssue>) -> Failure {
    Failure::invalid_harness(format!("world replay denied: {issues:?}"))
}
