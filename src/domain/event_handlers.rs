...
mod tests {
    ...
    struct TestEventHandler {
        name: String,
        handled_events: Arc<RwLock<Vec<DomainEvent>>>,
    }
    ...
    async fn test_event_bus_multiple_handlers() {
        ...
        let handled1 = Arc::new(RwLock::new(Vec::new()));
        let handled2 = Arc::new(RwLock::new(Vec::new()));

        bus.register_handler(Arc::new(TestEventHandler {
            name: "handler1".to_string(),
            handled_events: handled1.clone(),
        }))
        .await;

        bus.register_handler(Arc::new(TestEventHandler {
            name: "handler2".to_string(),
            handled_events: handled2.clone(),
        }))
        .await;
        ...
        assert_eq!(handled1.read().await.len(), 1);
        assert_eq!(handled2.read().await.len(), 1);
    }
    ...
    async fn test_event_bus_metrics() {
        ...
        assert_eq!(metrics.events_handled.get("test"), Some(&5));
    }
}