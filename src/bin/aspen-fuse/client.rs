...
use aspen::client_rpc::{
    ClientRpcRequest, ClientRpcResponse, DeleteResultResponse, MAX_CLIENT_MESSAGE_SIZE,
    ReadResultResponse, ScanResultResponse, WriteResultResponse,
};
use iroh::{Endpoint, EndpointAddr, SecretKey};
use tokio::runtime::Runtime;
use tracing::{debug, warn};

use crate::constants::{CONNECTION_TIMEOUT, READ_TIMEOUT, WRITE_TIMEOUT};
...
impl FuseSyncClient {
    ...
    pub fn from_ticket(ticket: &str) -> Result<Self> {
        ...
        let parsed =
            AspenClusterTicket::deserialize(ticket).context("failed to parse cluster ticket")?;
    ...
    async fn send_rpc_inner(&self, request: ClientRpcRequest) -> Result<ClientRpcResponse> {
        ...
        let connection = tokio::time::timeout(
            CONNECTION_TIMEOUT,
            self.endpoint.connect(self.target_addr.clone(), CLIENT_ALPN),
        )
        .await
