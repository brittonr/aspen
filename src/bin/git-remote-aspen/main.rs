...
mod tests {
    ...
    // connections and bi-streams but never writes a response. The client
    // runs the exact same production exchange pattern (connect → open_bi →
    // write → finish → read) and we verify the read-stage deadline fires.
    ...
    async fn full_exchange_times_out_when_peer_never_replies() {
        ...
        let connection =
            tokio::time::timeout(Duration::from_secs(5), client_ep.connect(server_addr, aspen::CLIENT_ALPN))
                .await
        ...
        let auth_request = AuthenticatedRequest::unauthenticated(ClientRpcRequest::Ping);
        let request_bytes = postcard::to_stdvec(&auth_request).expect("serialize");
        tokio::time::timeout(
            remaining_timeout_io(deadline).expect("deadline should still be ahead"),
        ...
        let read_bounded = match remaining_timeout_io(deadline) {
            Ok(remaining) => {
                tokio::time::timeout(remaining, recv.read_to_end(MAX_GIT_FETCH_RESPONSE_SIZE)).await.is_err()
            }
