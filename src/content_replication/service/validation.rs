...
pub(super) fn require_active(instance: &ServiceInstance) -> crate::error::Result<()> {
    if instance.state != LifecycleState::Active {
        return Err(crate::error::MoltenError::invalid_harness(
            "content-replication reconcile requires an active instance",
        ));
    }
    Ok(())
}

pub(super) fn validate_authority(manifest: &Manifest, observation: &AuthorityObservation) -> crate::error::Result<()> {
    ...
    {
        return Err(crate::error::MoltenError::invalid_harness(
            "content-replication authority observation denied or drifted",
        ));
    }
    Ok(())
}

pub(super) fn validate_identity(manifest: &Manifest, observation: &IdentityObservation) -> crate::error::Result<()> {
    ...
    {
        return Err(crate::error::MoltenError::invalid_harness(
            "content-replication identity observation is stale or mismatched",
        ));
    }
    Ok(())
}
...
pub(super) fn validate_membership(
    ...
    if !observation.current || observation.membership_epoch != manifest.membership_epoch {
        return Err(crate::error::MoltenError::invalid_harness("content-replication membership observation is stale"));
    }
    Ok(())
}
...
pub(super) fn validate_placement(manifest: &Manifest, observation: &PlacementObservation) -> crate::error::Result<()> {
    ...
    {
        return Err(crate::error::MoltenError::invalid_harness("content-replication placement observation is stale"));
    }
    Ok(())
}
...
pub(super) fn validate_resources(
    ...
    if !observation.admitted || observation.plan_ref != plan.plan_ref || observation.generation != manifest.generation {
        return Err(crate::error::MoltenError::invalid_harness(
            "content-replication resource reservation denied or drifted",
        ));
    }
    Ok(())
}
...
pub(super) fn validate_pin(
    ...
    if !observation.admitted
        ...
        || observation.generation != manifest.generation
    {
        return Err(crate::error::MoltenError::invalid_harness("content-replication retention pin denied or drifted"));
    }
    Ok(())
}
...
pub(super) fn validate_cleanup(
    ...
    {
        return Err(crate::error::MoltenError::invalid_harness(
            "content-replication cleanup admission denied or drifted",
        ));
    }
    Ok(())
}
...
pub(super) fn validate_envelope(
    ...
    {
        return Err(crate::error::MoltenError::invalid_harness(
            "content-replication transfer envelope is stale or mismatched",
        ));
    }
    Ok(())
}
...
pub(super) fn validate_verification(
    ...
    {
        return Err(crate::error::MoltenError::invalid_harness(
            "content-replication verification is incomplete or stale",
        ));
    }
    Ok(())
}

pub(super) fn validate_ref(reference: &str, field: &str) -> crate::error::Result<()> {
    if !valid_ref(reference) {
        return Err(crate::error::MoltenError::invalid_harness(format!("{field} is not a canonical BLAKE3 reference")));
    }
    Ok(())
}
