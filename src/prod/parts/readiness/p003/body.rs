...
fn validate_profile_metadata(input: &DeploymentProfileInput<'_>) -> Result<()> {
    ...
    if input.profile_identity != input.profile_name {
        return Err(Failure::invalid_harness("production profile identity must match profile name"));
    }
    validate_content_ref(input.profile_ref)
        .map_err(|error| Failure::invalid_harness(format!("invalid production profile ref: {error}")))
}

fn validate_text_field(label: &str, value: &str) -> Result<()> {
    if value.trim().is_empty() {
        Err(Failure::invalid_harness(format!("production readiness {label} must not be empty")))
    } else {
        Ok(())
    }
}
...
fn validate_ref_slice(label: &str, refs: &[String]) -> Result<()> {
    ...
    for reference in refs {
        validate_content_ref(reference).map_err(|error| {
            Failure::invalid_harness(format!("invalid production readiness {label} ref {reference}: {error}"))
        })?;
    }
    Ok(())
}
