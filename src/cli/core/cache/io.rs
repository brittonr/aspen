pub(super) fn cli_cache_ref(kind: &str, label: &str) -> molten::error::Result<String> {
    molten::preserves_rail::canonical_hash(&molten::preserves_rail::record("eval-cache-cli-ref", vec![
        molten::preserves_rail::string(kind),
        molten::preserves_rail::string(label),
    ]))
}

pub(super) fn preserves_sequence_strings(values: &[String]) -> preserves::IOValue {
    molten::preserves_rail::sequence(values.iter().map(molten::preserves_rail::string).collect())
}
