...
pub(super) fn run(command: Command) -> Outcome<()> {
    match command {
        command @ Command::PlanNameMove { .. } => plan_name_move(command),
        Command::Create {
            plan,
            store,
            receipt_out,
        } => create(plan, store, receipt_out),
        Command::SetName {
            store,
            name,
            artifact_ref,
            receipt_out,
        } => set_name(store, name, artifact_ref, receipt_out),
        Command::RunTask {
            store,
            ledger,
            plan_ref,
            task_id,
            receipt_out,
        } => run_task(store, ledger, plan_ref, task_id, receipt_out),
        Command::Rollback {
            store,
            plan_ref,
            task_id,
            receipt_out,
        } => rollback(store, plan_ref, task_id, receipt_out),
        Command::Status { store, plan_ref } => status(store, plan_ref),
        Command::CleanupCheck {
            ...
            artifact_ref,
            receipt_out,
        } => cleanup_check(store, ledger, registry, artifact_ref, receipt_out),
    }
}

fn plan_name_move(command: Command) -> Outcome<()> {
    let Command::PlanNameMove {
        ledger,
        registry,
        session_id,
        name,
...
fn create(plan: FilePath, store: FilePath, receipt_out: Option<FilePath>) -> Outcome<()> {
...
fn set_name(store: FilePath, name: String, artifact_ref: String, receipt_out: Option<FilePath>) -> Outcome<()> {
...
fn rollback(store: FilePath, plan_ref: String, task_id: String, receipt_out: Option<FilePath>) -> Outcome<()> {
...
fn status(store: FilePath, plan_ref: String) -> Outcome<()> {
