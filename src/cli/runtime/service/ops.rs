...
pub(super) fn run(command: Command) -> Outcome<()> {
    match command {
        Command::Run { suite, out } => run_runtime_suite(suite, out),
        Command::RunTwoService { out } => run_two_service(out),
        Command::Supervise { suite, out } => supervise_suite(suite, out),
        Command::RunSupervisionFixture { out } => run_supervision_fixture(out),
        Command::Show { report } => show_runtime(report),
        Command::ShowSupervision { report } => show_supervision(report),
        Command::GateSupervision { report, receipt_out } => gate_supervision(report, receipt_out),
        Command::Replay { report } => replay_runtime(report),
        Command::ReplaySupervision { report } => replay_supervision(report),
    }
}
...
fn supervise_suite(suite: FilePath, out: FilePath) -> Outcome<()> {
    let suite_value = super::io::read_preserves_file(&suite)?;
...
fn run_supervision_fixture(out: FilePath) -> Outcome<()> {
...
fn show_supervision(report: FilePath) -> Outcome<()> {
    let value = super::io::read_preserves_file(&report)?;
    println!("{}", molten::service_supervision::service_supervision_summary(&value)?);
    Ok(())
}

fn gate_supervision(report: FilePath, receipt_out: Option<FilePath>) -> Outcome<()> {
    let value = super::io::read_preserves_file(&report)?;
...
fn replay_supervision(report: FilePath) -> Outcome<()> {
    let value = super::io::read_preserves_file(&report)?;
