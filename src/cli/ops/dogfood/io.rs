...
pub(super) fn read_preserves_file(path: &std::path::Path) -> molten::error::Result<preserves::IOValue> {
    ...
    file.read_to_string(&mut text).map_err(molten::error::Failure::from)?;
...
pub(super) fn write_file(path: &std::path::Path, contents: &str) -> molten::error::Result<()> {
    ...
    file.write_all(contents.as_bytes()).map_err(molten::error::Failure::from)?;
    file.flush().map_err(molten::error::Failure::from)
}
