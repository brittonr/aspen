...
pub(crate) fn run() -> molten::error::Result<()> {
    ...
    match cli.command {
        ...
        Some(command::Top::FabricSimulation { command }) => {
            crate::cli_fabric_simulation::run_fabric_simulation_command(command)
        }
        Some(command::Top::SystemExtension { command }) => {
            crate::cli_system_extension::run_system_extension_command(command)
        }
        Some(command::Top::World { command }) => crate::cli_world_operator::run_world_command(command),
        Some(command::Top::WorldCommit { command }) => crate::cli_world_commit::run_world_commit_command(command),
        Some(command::Top::WorldSnapshot { command }) => crate::cli_world_snapshot::run_world_snapshot_command(command),
        Some(command::Top::WorldAuthority { command }) => {
            crate::cli_world_authority::run_world_authority_command(command)
        }
...
pub(crate) fn run_test_command(command: command::Test) -> molten::error::Result<()> {
    match command {
        ...
        command::Test::Drift { command } => crate::cli_drift::run_drift_command(command),
        ...
        command::Test::Storage { command } => crate::cli_storage::run(command),
        command::Test::Artifact { command } => crate::cli_artifact::run(command),
        command::Test::Schema { command } => crate::cli_schema::run(command),
        command::Test::Cache { command } => crate::cli_cache::run(command),
        command::Test::Upgrade { command } => crate::cli_upgrade::run_upgrade_command(command),
        command::Test::Transcript { command } => crate::cli_transcript::run(command),
        command::Test::Rewrite { command } => crate::cli_rewrite::run_rewrite_command(command),
        command::Test::Catalog { command } => crate::cli_catalog::run(command),
