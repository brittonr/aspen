...
fn test_purge_log_same_leader_as_prev_last_purged() -> anyhow::Result<()> {
    ...
    assert_eq!(vec![Command::PurgeLog { upto: log_id(2, 1, 3) }], lh.output.take_commands());
...
fn test_purge_log_to_last_key_log() -> anyhow::Result<()> {
    ...
    assert_eq!(vec![Command::PurgeLog { upto: log_id(4, 1, 4) }], lh.output.take_commands());
...
fn test_purge_log_go_pass_last_key_log() -> anyhow::Result<()> {
    ...
    assert_eq!(vec![Command::PurgeLog { upto: log_id(4, 1, 5) }], lh.output.take_commands());
...
fn test_purge_log_to_last_log_id() -> anyhow::Result<()> {
    ...
    assert_eq!(vec![Command::PurgeLog { upto: log_id(4, 1, 6) }], lh.output.take_commands());
...
fn test_purge_log_go_pass_last_log_id() -> anyhow::Result<()> {
    ...
    assert_eq!(vec![Command::PurgeLog { upto: log_id(4, 1, 7) }], lh.output.take_commands());
...
fn test_purge_log_to_higher_leader_lgo() -> anyhow::Result<()> {
    ...
    assert_eq!(vec![Command::PurgeLog { upto: log_id(5, 1, 7) }], lh.output.take_commands());
