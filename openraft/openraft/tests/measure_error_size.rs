...
fn measure_storage_error_size() {
    ...
    println!("\n--- Components ---");
    println!("ErrorSubject<TestConfig>:         {} bytes", size_of::<ErrorSubject<TestConfig>>());
    println!("ErrorVerb:                        {} bytes", size_of::<ErrorVerb>());
    println!("Box<anyerror::AnyError>:          {} bytes", size_of::<Box<anyerror::AnyError>>());
    ...
    println!("LogId<TestConfig>:                {} bytes", size_of::<LogId<TestConfig>>());
    println!("Option<LogId<TestConfig>>:        {} bytes", size_of::<Option<LogId<TestConfig>>>());
    println!("SnapshotSignature<TestConfig>:    {} bytes", size_of::<SnapshotSignature<TestConfig>>());
    println!(
    ...
    println!("Option<Box<LogId<TestConfig>>>:   {} bytes", size_of::<Option<Box<LogId<TestConfig>>>>());
    ...
    println!("RaftError<TestConfig, ()>:        {} bytes", size_of::<RaftError<TestConfig, ()>>());
    println!("Fatal<TestConfig>:                {} bytes", size_of::<Fatal<TestConfig>>());

    println!("\n=== Target ===");
    println!("Current:                          {storage_error_size} bytes");
    println!("Target:                           < 32 bytes");
    println!("Reduction needed:                 {} bytes\n", storage_error_size.saturating_sub(32));
