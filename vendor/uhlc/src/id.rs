...
impl ID {
    ...
    ///
    /// When the `rand` feature is disabled, this falls back to a fixed non-zero
    /// identifier so no-std consumers can compile without pulling in OS-backed
    /// randomness. Callers that need unique generated IDs should enable `rand`
    /// or provide explicit IDs.
    #[inline]
    pub fn rand() -> Self {
        #[cfg(feature = "rand")]
        {
        ...
        {
            const FALLBACK_ID_RAW: u128 = 1;
            Self(FALLBACK_ID_RAW.to_le_bytes())
        }
    }
}
