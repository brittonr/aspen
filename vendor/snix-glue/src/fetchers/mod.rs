...
pub enum Fetch {
    ...
    /// Fetch a git repository by cloning and checking out a specific revision.
    /// The checkout directory is ingested as a store path (like `builtins.fetchGit`).
    Git {
        /// The URL to clone from.
        url: Url,
        /// The expected commit revision (40-char hex SHA).
        rev: Option<String>,
        /// The git ref to fetch (branch or tag name).
        r#ref: Option<String>,
        ...
        all_refs: bool,
        /// The expected NAR SHA256 hash of the checked-out contents.
        exp_nar_sha256: Option<[u8; 32]>,
    },
}
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Fetch::Git {
                ...
                write!(
                    f,
                    "Git [url: {}, rev: {:?}, ref: {:?}, shallow: {}, submodules: {}, all_refs: {}",
                    url, rev, r#ref, shallow, submodules, all_refs
                )?;
                if let Some(exp_nar_sha256) = exp_nar_sha256 {
                    write!(
                        f,
    ...
    pub async fn ingest(&self, fetch: Fetch) -> Result<(Node, CAHash, u64), FetcherError> {
        match fetch {
            ...
            Fetch::Git {
                ...
                // Clone the repo via git subprocess in a blocking task.
                let clone_url = url.clone();
                let clone_rev = rev.clone();
                let clone_ref = r#ref.clone();
                let checkout_dir = tokio::task::spawn_blocking(move || {
                    let temp_dir = tempfile::tempdir().map_err(FetcherError::Io)?;
                    ...
                    if submodules {
                        ...
                        cmd.args(["clone", "--recurse-submodules", "--depth=1"]);
                        if let Some(ref r) = clone_ref {
                            cmd.args(["--branch", r]);
                        }
                        cmd.arg(clone_url.as_str()).arg(&checkout);
                        let output = cmd.output().map_err(FetcherError::Io)?;
                        if !output.status.success() {
                            return Err(FetcherError::Io(std::io::Error::new(
                                std::io::ErrorKind::Other,
                                format!(
                                    "git clone --recurse-submodules failed: {}",
                                    String::from_utf8_lossy(&output.stderr)
                                ),
                            )));
                        }
                        // Check out specific rev if requested
                        if let Some(ref rev) = clone_rev {
                        ...
                        // Bare clone + git archive extraction
                        let mut cmd = std::process::Command::new("git");
                        cmd.args(["clone", "--bare"]);
                        if !all_refs {
                        ...
                        // Resolve revision
                        let rev_to_export = if let Some(ref rev) = clone_rev {
                            rev.clone()
                        } else {
                        ...
                        std::fs::create_dir_all(&checkout).map_err(FetcherError::Io)?;
                        let git_archive = std::process::Command::new("git")
                            .args(["archive", "--format=tar", &rev_to_export])
                            .current_dir(&bare_dir)
                        ...
                        let tar_output = std::process::Command::new("tar")
                            .args(["xf", "-", "-C"])
                            .arg(&checkout)
                            .stdin(git_archive.stdout.expect("piped stdout"))
                            .output()
                            .map_err(FetcherError::Io)?;
                    ...
                    // Remove .git directory from checkout if present (for submodule path)
                    let git_dir = checkout.join(".git");
                    if git_dir.exists() {
                        std::fs::remove_dir_all(&git_dir).map_err(FetcherError::Io)?;
                    }

                    // Keep temp_dir alive so files persist until castore ingest completes
                    let path = checkout.to_path_buf();
                    let _ = temp_dir.keep();
                    Ok::<_, FetcherError>(path)
                })
                .await
                .map_err(|e| FetcherError::Io(std::io::Error::other(e)))??;

                // Ingest the checkout directory into the castore
                let node = snix_castore::import::fs::ingest_path::<_, _, _, &str>(
                    self.blob_service.clone(),
                    self.directory_service.clone(),
                    checkout_dir,
                    None,
...
mod tests {
    mod fetch {
        ...
        #[case::git_with_nar_sha256(
            Fetch::Git{
                url: Url::parse("https://github.com/nix-systems/default.git").unwrap(),
                rev: Some("da67096a3b9bf56a91d16901293e51ba5b49a27e".to_string()),
                r#ref: None,
                shallow: false,
                submodules: false,
                all_refs: false,
                // Same hash as fetch_tarball_store_path test below — verifies Git uses CAHash::Nar
                exp_nar_sha256: Some(nixbase32::decode_fixed("1hf6cgaci1n186kkkjq106ryf8mmlq9vnwgfwh625wa8hfgdn4dm").unwrap()),
            },
            Some(StorePathRef::from_bytes(b"7adgvk5zdfq4pwrhsm3n9lzypb12gw0g-source").unwrap()),
            "source"
        )]
        #[case::git_no_hash(
            Fetch::Git{
