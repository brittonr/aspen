# jobs-ci-core-extraction Specification

## Purpose

The jobs/CI core extraction defines reusable scheduler, CI config, run-state, and jobs protocol contracts that can be consumed without Aspen runtime shells, handlers, concrete worker/executor adapters, or node/bootstrap integration.
...
- AND the classification SHALL be saved as checked-in evidence under `openspec/changes/extract-jobs-ci-core/evidence/`
