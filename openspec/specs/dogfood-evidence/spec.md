...
VM-CI dogfood readiness MUST distinguish base node health, VM pool bootstrap, direct guest routing, direct TAP privilege requirements, and helper-backed TAP lifecycle requirements, and MUST emit bounded receipt evidence when any VM-CI-specific boundary is unavailable.
...
#### Scenario: Node health before VM pool warmup [r[dogfood-evidence.vmci-worker-readiness.node-health-before-pool]]
...
#### Scenario: VM pool bootstrap failure remains visible [r[dogfood-evidence.vmci-worker-readiness.pool-bootstrap-visible]]
...
#### Scenario: Local direct health checks avoid external discovery [r[dogfood-evidence.vmci-worker-readiness.local-direct-health]]
...
#### Scenario: VM worker guest ticket uses only the host bridge address [r[dogfood-evidence.vmci-worker-readiness.guest-ticket-bridge-only]]
...
#### Scenario: Host setup admits guest Iroh UDP through the effective firewall [r[dogfood-evidence.vmci-worker-readiness.firewall-bridge-ingress]]
...
#### Scenario: Host client ticket behavior remains unfiltered [r[dogfood-evidence.vmci-worker-readiness.host-ticket-unfiltered]]
