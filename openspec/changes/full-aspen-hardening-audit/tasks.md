...
- [x] Verify SNIX, Forge, federation, deploy, jobs/CI, secrets/trust, and admin operations use domain-specific capabilities rather than generic prefixes. Evidence: `evidence/domain-capability-audit.md`, `evidence/domain-capability-audit.json`; audited `to_operation/*_ops.rs` generic internal-domain mappings now report zero remaining.
  ...
  - Observability slice captured in `evidence/domain-capability-audit.md`, `evidence/domain-capability-audit.json`; traces, metrics, and alerts now require `ObservabilityRead`/`ObservabilityWrite` instead of generic `_sys:traces:`, `_sys:metrics:`, or `_sys:alerts:` data prefixes.
...
- [x] Audit CLI output, logs, errors, debug formatting, receipts, test fixtures, and docs for credential leakage using synthetic secret-like fixtures only. Evidence: `evidence/cli-log-output-redaction.md`, `evidence/cli-log-output-redaction.json`; audited representative operator output/log/error/debug/docs surfaces with synthetic markers only and hardened Forge web clone-command rendering so repository overview pages display a `<cluster-ticket>` placeholder instead of the live ticket value.
- [x] Verify dogfood, CI, deploy, and diagnostic receipts identify artifacts/runs without log scraping or credential values. Evidence: `evidence/dogfood-receipt-redaction.md`, `evidence/dogfood-receipt-redaction.json`; audited dogfood receipt artifacts and diagnostic flows, preserved CI run ids/cluster receipt keys as operator-safe identifiers, and hardened start logs, git push failure capture, persisted failure summaries, and receipt show/diagnose output so `aspen://.../...` credentials render as `aspen://<cluster-ticket>/...`.
