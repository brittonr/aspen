...
- [ ] Verify SNIX, Forge, federation, deploy, jobs/CI, secrets/trust, and admin operations use domain-specific capabilities rather than generic prefixes.
  ...
  - Automerge slice captured in `evidence/domain-capability-audit.md`, `evidence/domain-capability-audit.json`; document create/read/update/delete/merge/list and sync message exchange now require `AutomergeRead`/`AutomergeWrite` instead of generic `_automerge:` data prefixes. Remaining generic internal-domain mappings stay open for follow-up slices.
