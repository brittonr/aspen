...
- Changed file: `openspec/changes/prepare-crate-extraction/evidence/checker-negative-mutations.md`
...
- Changed file: `openspec/changes/prepare-crate-extraction/evidence/openspec-gate-tasks-v2.txt`
- Changed file: `openspec/changes/prepare-crate-extraction/evidence/openspec-gate-tasks-v2-corrected.txt`
- Changed file: `openspec/changes/prepare-crate-extraction/evidence/openspec-gate-tasks-v11.txt`
- Changed file: `openspec/changes/prepare-crate-extraction/evidence/openspec-gate-tasks-v11-pass.txt`
- Changed file: `openspec/changes/prepare-crate-extraction/evidence/openspec-preflight-v2.txt`
- Changed file: `openspec/changes/prepare-crate-extraction/evidence/openspec-preflight-v11.txt`
- Changed file: `openspec/changes/prepare-crate-extraction/evidence/openspec-preflight-i7-i8-checked.txt`
- Changed file: `openspec/changes/prepare-crate-extraction/evidence/openspec-preflight-i9-checked.txt`
- Changed file: `openspec/changes/prepare-crate-extraction/evidence/readiness-checker-run.txt`
- Changed file: `openspec/changes/prepare-crate-extraction/evidence/runtime-networking-policy.md`
- Changed file: `openspec/changes/prepare-crate-extraction/evidence/runtime-networking-policy-source-audit.txt`
...
- [ ] V10 If any compatibility rail fails from known unrelated parser or workspace breakage, save a triage artifact that identifies the unrelated failing file/error, reruns or isolates the extraction-relevant compile slice when possible, and avoids counting unrelated failures as extraction-boundary regressions; if all compatibility rails pass without unrelated workspace breakage, save a no-triage-needed note in `verification.md` that names the passing compatibility artifacts and marks this task not applicable for failure triage. [covers=architecture.modularity.binaries-are-thin-imperative-shells-over-libraries.aspen-compatibility-consumers-stay-verified-during-migration]
  - Evidence pending until V7/V8/V9 are valid post-migration compatibility rails: `openspec/changes/prepare-crate-extraction/evidence/openspec-gate-tasks-v11.txt`
...
I7 evidence now records the reusable `aspen-raft-kv-types` boundary and the explicit legacy `aspen-raft-types` package/API transition plan, including concrete subsystem owner status and per-consumer alias/re-export decisions. I8 evidence now exposes and tests pre-storage `aspen-raft-kv` facade operations for KV and cluster-control trait surfaces without concrete Redb or iroh endpoint construction. I7 and I8 are checked for the current checkpoint after the task gate accepted the task plan and existing prepared evidence was linked from this verification index. V2 evidence is prepared but intentionally unchecked after the task gate flagged it as premature until `I7`-`I12` complete the full type/facade/network/storage migration. V11 is checked for this intermediate checkpoint: verification, preflight, readiness-checker, gate transcripts, and human/oracle escalation evidence are synchronized for the currently checked tasks. I9 is checked for the adapter-boundary checkpoint: default storage/type/facade graphs do not reach concrete iroh or transport dependencies, runtime networking remains iroh-only, and `aspen-raft-network` remains the explicit adapter. The partial V1 feature matrix records reusable/core compile rails plus prepared V7/V8/V9 compatibility consumer rails, but those compatibility tasks remain unchecked because they are post-migration verification rails for I10-I12. V1 remains unchecked until the newly required core UI rail and future named storage/facade features from I10-I12 are covered or explicitly triaged.
