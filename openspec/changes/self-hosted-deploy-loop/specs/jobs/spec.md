...
The system SHALL support multiple specialized worker types, each handling a specific category of jobs. The deploy job type SHALL be handled by the deployment coordinator on the Raft leader, not by regular worker nodes.
...
- AND the deployment coordinator on the Raft leader SHALL handle the deploy job directly
