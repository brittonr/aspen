...
Auth/ticket serialization goldens and malformed-input coverage are in place; the remaining blocker is to decide the stable reusable API and keep runtime verifier shells explicit. This change is intentionally spec-only at creation time: it defines the implementation/evidence rail without changing Rust code yet.

## Goals / Non-Goals

**Goals:**
- Define the smallest evidence-backed implementation slice for `review-auth-ticket-public-api`.
...
**Choice:** Implementation tasks must save command transcripts, fixture metadata, negative checks, and readiness/doc diffs under `openspec/changes/review-auth-ticket-public-api/evidence/` or documented repo-local equivalents before tasks are checked off.
