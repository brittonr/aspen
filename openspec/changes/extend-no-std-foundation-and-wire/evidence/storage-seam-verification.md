...
- `python3 scripts/check-aspen-core-no-std-surface.py --crate-dir crates/aspen-core/src --output-dir openspec/changes/extend-no-std-foundation-and-wire/evidence`
  - Result: success; generated `surface-inventory.md`, `export-map.md`, and `source-audit.txt`

## Durable artifacts

- UI fixture index: `openspec/changes/extend-no-std-foundation-and-wire/evidence/ui-fixtures.txt`
- UI stderr snapshots:
  - `openspec/changes/extend-no-std-foundation-and-wire/evidence/ui-app-registry-no-std.stderr`
  - `openspec/changes/extend-no-std-foundation-and-wire/evidence/ui-network-transport-no-std.stderr`
  - `openspec/changes/extend-no-std-foundation-and-wire/evidence/ui-simulation-artifact-no-std.stderr`
  - `openspec/changes/extend-no-std-foundation-and-wire/evidence/ui-content-discovery-std-without-global-discovery.stderr`
  - `openspec/changes/extend-no-std-foundation-and-wire/evidence/ui-directory-layer-std-without-layer.stderr`
  - `openspec/changes/extend-no-std-foundation-and-wire/evidence/ui-storage-table-no-std.stderr`
- Boundary inventory: `openspec/changes/extend-no-std-foundation-and-wire/evidence/surface-inventory.md`
- Export map: `openspec/changes/extend-no-std-foundation-and-wire/evidence/export-map.md`
- Source audit: `openspec/changes/extend-no-std-foundation-and-wire/evidence/source-audit.txt`
