...
- **BREAKING** Replace the unsigned ticket's public runtime transport metadata with alloc-safe types: transport-neutral `NodeAddress` bootstrap peers and a crate-local alloc-safe topic identifier instead of concrete iroh gossip/runtime types. Previously serialized unsigned cluster tickets will need regeneration after this schema change.
...
- **Direct consumers to audit**: exhaustive set discovered from `rg -n 'aspen-ticket\\s*=\\s*\\{' . -g 'Cargo.toml'`: root `Cargo.toml`, `crates/aspen-ci-executor-vm`, `crates/aspen-cluster-handler`, `crates/aspen-cluster`, `crates/aspen-rpc-handlers`, `crates/aspen-client`, and `crates/aspen-ci`.
...
- **Testing**: targeted `cargo tree`, `cargo check`, and ticket roundtrip/regression rails plus consumer compile proofs for each changed manifest edge, including distinct saved evidence for current alloc-safe unsigned roundtrip success, legacy unsigned payload rejection, default-vs-`--no-default-features` equivalence, and signed malformed-input rejection.
