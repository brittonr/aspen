...
- [x] [depends:tests] Run focused dogfood/client tests, formatting, OpenSpec validation, and a live `nix run .#dogfood-local-vmci -- full` retry or save a classified failure bundle. Evidence: focused `aspen-client` direct route tests passed; `cargo test -p aspen-dogfood` passed; `nix run .#rustfmt`, `git diff --check`, and `openspec validate fail-fast-direct-only-route-loss --strict` passed; live `nix run .#dogfood-local-vmci -- full` saved classified VM-CI readiness failure receipt at `/home/brittonr/.cargo-target/aspen-dogfood-vmci-receipts/dogfood-20260523T034701Z.json`.
