...
run_checker_expect_failure() {
  ...
  local expected="$2"
  local policy_path="$3"
  local report_json="$evidence/v4-negative-$name.json"
  local report_md="$evidence/v4-negative-$name.md"
  local stdout="$evidence/v4-negative-$name.stdout"
  local stderr="$evidence/v4-negative-$name.stderr"
  local summary="$evidence/v4-negative-$name-summary.txt"
  set +e
  "$checker" \
    --policy "$policy_path" \
    --inventory "$inventory" \
...
if edit_kind == "missing-owner":
    text = text.replace(
        'dependency_path = "aspen-commit-dag -> aspen-traits",\n          owner = "Aspen branch/DAG maintainers"',
        'dependency_path = "aspen-commit-dag -> aspen-traits",\n          owner = "owner needed"',
        1,
    )
elif edit_kind == "invalid-readiness":
    text = re.sub(
        r'(aspen_commit_dag = \{.*?readiness_state = )"workspace-internal"',
        r'\1"publishable from monorepo"',
        text,
        count=1,
        flags=re.S,
    )
...
run_forbidden_dependency_mutation() {
  local backup
  backup=$(mktemp)
  cp crates/aspen-commit-dag/Cargo.toml "$backup"
  trap 'cp "$backup" crates/aspen-commit-dag/Cargo.toml; rm -f "$backup"' RETURN
  python - <<'PY'
from pathlib import Path
path = Path('crates/aspen-commit-dag/Cargo.toml')
text = path.read_text()
needle = '[dependencies]\n'
...
path.write_text(text)
PY
  run_checker_expect_failure forbidden-raft 'aspen_commit_dag: direct forbidden dependency `aspen-raft`' "$policy"
}
...
EOF
  ...
  "$checker" \
    ...
    > "$evidence/v4-negative-missing-downstream.stdout" \
    2> "$evidence/v4-negative-missing-downstream.stderr"
  local code=$?
  set -e
  {
  ...
  } > "$evidence/v4-negative-missing-downstream-summary.txt"
  rm -rf "$tmp_change"
  if [[ "$code" -eq 0 ]]; then
    echo "negative mutation missing-downstream unexpectedly passed" >> "$evidence/v4-negative-missing-downstream-summary.txt"
    return 1
  fi
  rg -n --fixed-strings "missing downstream fixture evidence" "$evidence/v4-negative-missing-downstream-summary.txt" >/dev/null
}

run_forbidden_dependency_mutation
with_temp_policy missing-owner 'aspen_commit_dag: exception `aspen-commit-dag -> aspen-traits` has unassigned owner' missing-owner
with_temp_policy invalid-readiness 'aspen_commit_dag: forbidden readiness state `publishable from monorepo`' invalid-readiness
run_missing_downstream_evidence_mutation

cat > "$evidence/v4-negative-mutations-summary.txt" <<'EOF'
Negative mutation checks passed:
- forbidden aspen-raft dependency injection failed readiness checking
