...
- [x] 2.6 Create `src/store.rs` with `ContactStore` struct: CRUD operations via `KeyValueStore` trait, KV key construction, JSON serialization
- [x] 2.7 Implement `create_book`, `delete_book`, `list_books` on `ContactStore`
- [x] 2.8 Implement `create_contact` (parse vCard → Contact struct → JSON → KV write), `get_contact`, `update_contact`, `delete_contact`
- [x] 2.9 Implement `list_contacts` with prefix scan + pagination
...
- [x] 3.7 Create `src/store.rs` with `CalendarStore` struct: CRUD via `KeyValueStore` trait
- [x] 3.8 Implement `create_calendar`, `delete_calendar`, `list_calendars`
- [x] 3.9 Implement `create_event` (parse iCal → CalendarEvent → JSON → KV write), `get_event`, `update_event`, `delete_event`
...
- [x] 4.1 Create `crates/aspen-contacts-handler/Cargo.toml` with deps: `aspen-rpc-core`, `aspen-contacts`, `aspen-client-api`, `async-trait`, `anyhow`, `tracing`
- [x] 4.2 Create `src/handler.rs` with `ContactsHandler` implementing `RequestHandler`: `can_handle()` for all 16 Contacts variants, `handle()` dispatch
- [x] 4.3 Create `src/lib.rs` with `ContactsHandlerFactory` implementing `HandlerFactory` (priority 350, app_id "contacts"), `submit_handler_factory!`
- [x] 4.4 Implement handler functions for book operations: `handle_create_book`, `handle_delete_book`, `handle_list_books`
- [x] 4.5 Implement handler functions for contact CRUD: `handle_create_contact`, `handle_get_contact`, `handle_update_contact`, `handle_delete_contact`
- [x] 4.6 Implement handler functions for list/search: `handle_list_contacts`, `handle_search_contacts`
- [x] 4.7 Implement handler functions for import/export: `handle_import_vcard`, `handle_export_vcard`
- [x] 4.8 Implement handler functions for groups: `handle_create_group`, `handle_delete_group`, `handle_list_groups`, `handle_add_to_group`, `handle_remove_from_group`
- [x] 4.9 Add to workspace `Cargo.toml` members
- [x] 4.10 Write `can_handle` unit tests
- [x] 4.11 Verify `cargo nextest run -p aspen-contacts-handler` passes

## 5. Calendar Handler Crate (`aspen-calendar-handler`)

- [x] 5.1 Create `crates/aspen-calendar-handler/Cargo.toml` with deps: `aspen-rpc-core`, `aspen-calendar`, `aspen-client-api`, `async-trait`, `anyhow`, `tracing`
- [x] 5.2 Create `src/handler.rs` with `CalendarHandler` implementing `RequestHandler`: `can_handle()` for all 14 Calendar variants, `handle()` dispatch
- [x] 5.3 Create `src/lib.rs` with `CalendarHandlerFactory` implementing `HandlerFactory` (priority 360, app_id "calendar"), `submit_handler_factory!`
- [x] 5.4 Implement handler functions for calendar CRUD: `handle_create_calendar`, `handle_delete_calendar`, `handle_list_calendars`
- [x] 5.5 Implement handler functions for event CRUD: `handle_create_event`, `handle_get_event`, `handle_update_event`, `handle_delete_event`
- [x] 5.6 Implement handler functions for queries: `handle_list_events`, `handle_search_events`, `handle_free_busy`, `handle_expand_recurrence`
- [x] 5.7 Implement handler functions for import/export: `handle_import_ical`, `handle_export_ical`
- [x] 5.8 Add to workspace `Cargo.toml` members
- [x] 5.9 Write `can_handle` unit tests
- [x] 5.10 Verify `cargo nextest run -p aspen-calendar-handler` passes

## 6. Wire Up Feature Flags & Registry

- [x] 6.1 Add `contacts` and `calendar` features to `aspen-rpc-handlers/Cargo.toml` with optional deps on handler crates
- [x] 6.2 Add `contacts` and `calendar` features to root `Cargo.toml` (node binary) feature list
- [x] 6.3 Verify `cargo build --features contacts,calendar` compiles
- [x] 6.4 Verify handlers auto-register via `submit_handler_factory!` (linked via `pub use` in `handlers/mod.rs`)

## 7. CLI Commands

- [x] 7.1 Create `crates/aspen-cli/src/commands/contacts.rs` with subcommands: `create-book`, `list-books`, `add`, `get`, `list`, `search`, `import`, `export`, `delete`, `create-group`, `list-groups`
- [x] 7.2 Create `crates/aspen-cli/src/commands/calendar.rs` with subcommands: `create`, `list`, `add-event`, `get-event`, `list-events`, `search`, `import`, `export`, `free-busy`, `delete-event`
- [x] 7.3 Wire into CLI main command enum
- [x] 7.4 Verify `cargo build -p aspen-cli` compiles
- [x] 7.5 Write CLI parse tests for new subcommands

## 8. Integration Tests

- [x] 8.1 Write end-to-end contacts test: create book → add contacts (vCard) → list → search by email → export vCard → verify roundtrip
- [x] 8.2 Write end-to-end calendar test: create calendar → add events (iCal) → list by time range → free/busy query → export iCal → verify roundtrip
- [x] 8.3 Write RRULE integration test: create recurring event → expand recurrence → verify instance count and dates (including EXDATE exclusion)
- [x] 8.4 Write contact groups test: create group → add members → list group → remove member
- [x] 8.5 Verify `cargo nextest run --features contacts,calendar` passes with no regressions (813 tests, 0 failures)
