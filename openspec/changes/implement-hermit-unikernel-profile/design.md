...
Aspen now has an accepted runtime host-loading baseline and an active runtime-service-core implementation OpenSpec. The Hermit unikernel profile change owns one focused implementation seam so the runtime does not collapse distinct host boundaries into a vague generic runner.
