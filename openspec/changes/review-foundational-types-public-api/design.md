...
Aspen foundational type crates are already split far enough that stale storage/Redb blockers are resolved, but the family still needs an owner-grade public API review before readiness can be raised or rejected with evidence. This change is intentionally spec-only at creation time: it defines the implementation/evidence rail without changing Rust code yet.
