...
- [x] 1.1 Create `crates/aspen-net/Cargo.toml` with dependencies: `aspen-core`, `aspen-auth`, `aspen-constants`, `aspen-client-api`, `aspen-proxy` (for DownstreamProxy re-export), `aspen-dns` (git dep to sibling repo), `iroh`, `iroh-proxy-utils`, `tokio`, `serde`, `serde_json`, `snafu`, `tracing`
- [x] 1.2 Create `crates/aspen-net/src/lib.rs` with module declarations: `registry`, `resolver`, `socks5`, `forward`, `auth`, `daemon`, `types`, `constants`, `verified`
- [x] 1.3 Add `net` feature flag to workspace root `Cargo.toml` that enables `aspen-net` and wires through `aspen-rpc-handlers/net`
- [x] 1.4 Add `aspen-net` to workspace `members` list
- [x] 1.5 Verify `cargo check -p aspen-net` compiles with empty module stubs

## 2. Types and Constants

- [x] 2.1 Create `crates/aspen-net/src/types.rs` with `ServiceEntry` struct: `name: String`, `endpoint_id: String`, `port: u16`, `proto: String`, `tags: Vec<String>`, `hostname: Option<String>`, `published_at_ms: u64`. Derive `Serialize`, `Deserialize`, `Clone`, `Debug`.
- [x] 2.2 Add `NodeEntry` struct: `endpoint_id: String`, `hostname: String`, `tags: Vec<String>`, `services: Vec<String>`, `last_seen_ms: u64`
- [x] 2.3 Create `crates/aspen-net/src/constants.rs` with Tiger Style bounds:
...
- [x] 3.1 Create `crates/aspen-net/src/registry.rs` with `ServiceRegistry` struct holding a KV store reference (generic over `KeyValueStore` trait)
- [x] 3.2 Implement `publish(&self, entry: ServiceEntry) -> Result<()>`: validate name (regex `^[a-z0-9][a-z0-9.-]{0,252}$`), validate tag count, serialize to JSON, write to `/_sys/net/svc/{name}`, update node entry
- [x] 3.3 Implement `unpublish(&self, name: &str) -> Result<()>`: delete `/_sys/net/svc/{name}`, update node entry
- [x] 3.4 Implement `lookup(&self, name: &str) -> Result<Option<ServiceEntry>>`: read `/_sys/net/svc/{name}`, deserialize
- [x] 3.5 Implement `list(&self, tag_filter: Option<&str>) -> Result<Vec<ServiceEntry>>`: scan `/_sys/net/svc/` prefix, optionally filter by tag
- [x] 3.6 Implement `list_nodes(&self) -> Result<Vec<NodeEntry>>`: scan `/_sys/net/node/` prefix
- [x] 3.7 Add service name validation function in `verified/`: `is_valid_service_name(name: &str) -> bool`
- [x] 3.8 Unit tests: publish, lookup, unpublish, list, list-by-tag, name validation, duplicate name overwrite, resource bounds

## 4. Name Resolver

- [x] 4.1 Create `crates/aspen-net/src/resolver.rs` with `NameResolver` struct holding a `ServiceRegistry` reference and a local cache (`HashMap<String, ServiceEntry>`)
- [x] 4.2 Implement `resolve(&self, name: &str) -> Result<Option<(EndpointId, u16)>>`: strip `.aspen` suffix, lookup in cache, fallback to registry, parse endpoint_id string to `iroh::EndpointId`
- [x] 4.3 Implement `refresh_cache(&self) -> Result<()>`: list all services from registry, replace local cache
- [x] 4.4 Add cache TTL: entries expire after `NET_REGISTRY_POLL_INTERVAL_SECS`, trigger refresh on next resolve
- [x] 4.5 Unit tests: resolve known service, resolve unknown, cache hit, cache miss triggers refresh, `.aspen` suffix stripping

## 5. Capability Token Integration (aspen-auth)

- [x] 5.1 Add `NetConnect { service_prefix: String }` variant to `Capability` enum in `crates/aspen-auth/src/capability.rs`
- [x] 5.2 Add `NetPublish { service_prefix: String }` variant to `Capability` enum
- [x] 5.3 Add `NetAdmin` variant to `Capability` enum
- [x] 5.4 Add `NetConnect { service: String, port: u16 }` variant to `Operation` enum
- [x] 5.5 Add `NetPublish { service: String }` variant to `Operation` enum
- [x] 5.6 Add `NetUnpublish { service: String }` variant to `Operation` enum
- [x] 5.7 Add `NetAdmin { action: String }` variant to `Operation` enum
- [x] 5.8 Implement `authorizes()` match arms in `Capability`:
...
- [x] 5.9 Implement `contains()` match arms for delegation:
...
- [x] 5.10 Add `Display` impl for new `Operation` variants
- [x] 5.11 Create `crates/aspen-net/src/auth.rs` with `NetAuthenticator` struct holding `TokenVerifier` and `CapabilityToken`:
...
- [x] 6.1 Create `crates/aspen-net/src/socks5.rs` with `Socks5Server` struct holding: `DownstreamProxy`, `NameResolver`, `NetAuthenticator`, bind address, active connection counter (`Arc<AtomicU32>`)
- [x] 6.2 Implement SOCKS5 greeting/auth negotiation: read version (0x05), method count, methods. Reply with no-auth (0x00). Reject if version != 5.
- [x] 6.3 Implement SOCKS5 CONNECT request parsing: read version, cmd (must be 0x01 = CONNECT), reserved, address type. Support only type 0x03 (domain name). Parse domain name and port.
- [x] 6.4 Implement name resolution: strip `.aspen` suffix from domain, call `resolver.resolve()`. Return SOCKS5 error 0x04 (host unreachable) if not found.
- [x] 6.5 Implement token authorization check: call `authenticator.check_connect(service_name, port)`. Return SOCKS5 error 0x05 (connection refused) if token doesn't grant `NetConnect` for this service.
- [x] 6.6 Implement tunnel creation: call `downstream_proxy.create_tunnel(EndpointAuthority { endpoint_id, authority: Authority::new("localhost", port) })`. Return SOCKS5 error 0x05 on tunnel failure.
- [x] 6.7 Implement SOCKS5 success response: send version (0x05), reply (0x00 = success), reserved (0x00), addr type (0x01), bind addr (0.0.0.0), bind port (0). Then bidirectionally copy bytes between TCP stream and tunnel streams.
- [x] 6.8 Implement connection limit enforcement: increment counter on accept, decrement on close. Reject with SOCKS5 error 0x01 (general failure) if at `MAX_SOCKS5_CONNECTIONS`.
- [x] 6.9 Implement handshake timeout: wrap greeting + request parsing in `tokio::time::timeout(SOCKS5_HANDSHAKE_TIMEOUT_SECS)`.
- [x] 6.10 Implement `Socks5Server::run(&self, listener: TcpListener)` main accept loop with per-connection task spawning and cancellation token for shutdown.
- [x] 6.11 Unit tests: greeting parsing, CONNECT parsing, domain extraction. Integration tests: tunnel through SOCKS5 to a local TCP echo server via loopback iroh endpoints.

## 7. Port Forwarding

- [x] 7.1 Create `crates/aspen-net/src/forward.rs` with `PortForward` struct wrapping `DownstreamProxy`
- [x] 7.2 Implement `PortForward::start(local_addr: SocketAddr, service_name: &str, remote_port: Option<u16>, resolver: &NameResolver, auth: &NetAuthenticator) -> Result<()>`:
...
- [x] 7.3 Parse forward spec string: `[bind_addr:]local_port:service[:remote_port]` → `(SocketAddr, String, Option<u16>)`
- [x] 7.4 Unit tests: spec string parsing. Integration tests: forward local port, connect through it.
