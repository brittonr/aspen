...
fn fuzz_client_rpc_roundtrip() {
    check!().with_type::<Vec<u8>>().for_each(|data| {
        ...
        if let Ok(request) = postcard::from_bytes::<ClientRpcRequest>(data) {
            let reserialized =
                postcard::to_stdvec(&request).expect("re-serialization of valid request must succeed");
            ...
            assert_eq!(
                ...
                request2.variant_name(),
                "Round-trip must preserve request variant"
            );
            ...
            assert_eq!(
                ...
                "Serialization must be a fixed point (idempotent)"
            );
        }
        ...
        if let Ok(response) = postcard::from_bytes::<ClientRpcResponse>(data) {
            let reserialized =
                postcard::to_stdvec(&response).expect("re-serialization of valid response must succeed");
