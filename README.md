...
The strict gate is fail-closed: `warning-only` denies even when `cargo-octet` exits `0`, and `command.txt`, `status.json`, `summary.txt`, structured finding keys, object corpus receipts, and fingerprint evidence are bound by canonical refs in the Octet receipt.

At `d98f7ff97024c3b8e16fca46f4a4a716e4304615`, the public strict gate rejects 6,787 warnings and 330 critical findings.
[`evidence/octet/native-hash-candidate/validation.md`](evidence/octet/native-hash-candidate/validation.md) preserves that executed denial.
The combined native-build and retry candidate also fails the public strict gate, with 6,810 warnings and 330 critical findings.
