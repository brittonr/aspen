...
- [komora-io `marble`](https://github.com/komora-io/marble) and [`art`](https://github.com/spacejam/art) — Apache-2.0 selected components of the 2026-09-09 komora-io audit, pinned at crates.io `marble =16.0.2` and `art =1.0.0` behind the optional `marble-store` feature for the bounded object-store pilot. The spike records measurements only; it changes no runtime semantics and does not replace the default storage path. See [`docs/marble-object-store-pilot.md`](docs/marble-object-store-pilot.md).
