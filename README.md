...
The strict gate is fail-closed: `warning-only` denies even when `cargo-octet` exits `0`, and `command.txt`, `status.json`, `summary.txt`, structured finding keys, object corpus receipts, and fingerprint evidence are bound by canonical refs in the Octet receipt. The dev shell provides `cargo-octet` from the catalogued `octet-toolchain` flake input (`OnixResearch/octet` `fc38f59330b626961d166febfdf1a5aa6575460f`, toolchain `nightly-2026-03-21`); an ambient `cargo-octet` elsewhere on `PATH` is not Octet evidence. Current state (2026-09-24, `origin/molten` `4e31cee55`, pinned Octet, `dylint.toml` `disabled_lints = []`): the configured workspace run is `warning-only` with 3627 findings (2016 distinct lint sites, all in tracked Molten source: `path_segment_repetition` 1877, `excessive_file_length` 550, `borrowed_argument_types` 368, `unbounded_collection_growth` 214, `function_length` 204, `too_many_parameters` 143, `no_unwrap` 80, `non_trait_imports` 34, `usize_in_public_api` 32, `explicit_defaults` 31, `underscore_in_module_filename` 31, `module_file_count` 20, `ambient_clock` 17, `ambient_env` 14, `unbounded_channel` 9, `no_recursion` 2, `no_panic` 1), and the `-p molten --lib` run is `warning-only` with 1515 findings. The strict gate therefore denies (`status-config-current` and `status-profile-current` pass; `strict-status-clean` and `no-critical-findings` fail with 312 unreviewed critical findings). There is no current clean Octet claim: the 2026-07-02 clean receipt predates later source growth and a pinned tool, and most current findings are in files added or changed since then. The repair plan is a burn-down series modelled on the archived `octet-burndown-*` packages: import hygiene (`non_trait_imports`, `explicit_defaults`), safety polish (the critical families plus `usize_in_public_api`, `unbounded_channel`, `ambient_env`, `no_recursion`), size shape (`excessive_file_length`, `function_length`, `too_many_parameters`, `borrowed_argument_types`), and path shape (`path_segment_repetition`, `underscore_in_module_filename`, `module_file_count`). Hook or CI enforcement that denies `warning-only` lands after the series reaches zero. The quarantine flow below is burn-down tooling only; its example expiry dates have passed, and no baseline or review receipt currently admits these findings. During warning burn-down only, use the explicit quarantine flow:
