...
The independent `molten-node` executable compiles seven existing pure decision-module trees through `molten-node-core`, without depending on the broad legacy `molten-core` package or the root `molten` library. The source files remain shared, not copied; [`docs/node-startup-evidence.md`](docs/node-startup-evidence.md) defines the exact source/build-input binding and the still-denied startup route.
...
Normal `molten-node run` and protected content serving currently fail closed. Real startup evidence is not yet integrated. The portable [startup evidence verifier](docs/node-startup-evidence.md) checks explicit snapshots without an ambient workspace. Its reports are verification-only and cannot authorize startup. The standalone `molten-node` binary is built from `molten-node-runtime`, `molten-core`, and `molten-node-host`; it does not link the root `molten` library. The lifecycle examples below do not establish production readiness.

The `molten-node` executable owns the node lifecycle. The root `molten` CLI retains independent cluster planning and review tools, not a second node-start route. For source-tree development, specify the binary with `cargo run --bin molten-node --`; `nix develop` provides tools but does not install a freshly built binary by itself.
...
```sh
cargo run --bin molten -- cluster harness-run --node-binary target/debug/molten-node --fixture tests/fixtures/cluster-harness/two-node.cluster --state-root target/cluster-harness-state --run-dir target/cluster-harness-run
cargo run --bin molten -- cluster harness-verify --run-dir target/cluster-harness-run
```
...
nix run \
  ...
  .#molten-node -- status --state-root target/node
```

The durable local node boundary is exposed by the separate `molten-node` executable:
...
The strict gate is fail-closed: `warning-only` denies even when `cargo-octet` exits `0`; a clean status from a synthetic snapshot does not prove execution or authorize startup. The earlier module-file-count burndown and dogfood receipts were scoped to their historical build. A later unchanged pinned gate on the root `molten` executable reported 14,529 errors across 892 paths; its raw evidence and root-vs-node boundary analysis are retained under `.cairn/changes/node-startup-evidence/evidence/naming-review/`. The extracted `molten-node` package requires its own complete source/build/binary cohort and independent approval. No current strict-clean normal-node gate, listener, VM transfer, or release claim follows from the historical receipts. During historical warning burn-down only, the quarantine flow was:
