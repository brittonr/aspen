...
cargo octet object corpus receipt \
  ...
  src/remote/dataspace.rs src/retention/mod.rs src/runtime/envelope/mod.rs src/runtime/envelope/tests.rs src/runtime/predicates/mod.rs \
...
The strict gate is fail-closed: `warning-only` denies even when `cargo-octet` exits `0`, and `command.txt`, `status.json`, `summary.txt`, structured finding keys, object corpus receipts, and fingerprint evidence are bound by canonical refs in the Octet receipt. Current remediation snapshot: workspace and lib-only Octet are `clean` with 0 findings, 0 warnings, and 0 errors; focused object corpus has 2979 objects (`b3:1792bb7f5a0206602037a48fc23a56c5f0ef88a1d011aa702dbb7f261eb632d8`); latest artifact import receipt is `blake3:12f2bba423be64e0eaf639ad63bcf812aec0368f6472ced02ad7b350ee24d370`, latest strict pass receipt is `blake3:d690c4ef74704a4be8b48fea810703730b5cf9077e70f38e5b64fc3de5a91195`, and latest remediation plan receipt is `blake3:ca203ff50463d0672da700a33a2c9c90aaab85414f269202eee0d5c9c9172f68`. Caveat: this is configuration-clean with the remaining broad high-noise lint families explicitly disabled in `dylint.toml`; `underscore_in_module_filename` has been removed from the disabled set, Molten source now has no `module_file_count` findings, and the latest Job helper split lowered the no-disabled probe to 8117 warnings: `non_trait_imports` 4569, `path_segment_repetition` 2992, `function_length` 430, and `excessive_file_length` 109. Source-remediated zero for the remaining disabled families and external `module_file_count` handling remains separate follow-up work. Nix/dogfood was not rerun for this partial disabled-lint burn-down slice. During warning burn-down only, use the explicit quarantine flow:
