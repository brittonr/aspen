...
async fn test_dns_crud_single_node() {
    ...
    print_success(&format!(
        "Node 1 started with endpoint: {}",
        service.endpoint_addr().id
    ));
    ...
    dns_store
        .set_record(a_record.clone())
        .await
        .expect("failed to set A record");
    ...
    dns_store
        .set_record(aaaa_record.clone())
        .await
        .expect("failed to set AAAA record");
    ...
    dns_store
        .set_record(mx_record.clone())
        .await
        .expect("failed to set MX record");
    ...
    dns_store
        .set_record(txt_record.clone())
        .await
        .expect("failed to set TXT record");
    ...
    dns_store
        .set_record(srv_record.clone())
        .await
        .expect("failed to set SRV record");
    ...
    dns_store
        .set_record(wildcard_record.clone())
        .await
        .expect("failed to set wildcard record");
    ...
    let fetched = dns_store
        .get_record("api.example.com", RecordType::A)
        .await
    ...
    let all_records = dns_store
        .get_records("api.example.com")
        .await
    ...
    let resolved = dns_store
        .resolve("random.example.com", RecordType::A)
        .await
        .expect("failed to resolve wildcard");
    ...
    let resolved_exact = dns_store
        .resolve("api.example.com", RecordType::A)
        .await
        .expect("failed to resolve exact");
    ...
    if let Some(record) = resolved_exact.first() {
        if let DnsRecordData::A { addresses } = &record.data {
    ...
    let deleted = dns_store
        .delete_record("api.example.com", RecordType::AAAA)
        .await
        .expect("failed to delete record");
    assert!(deleted);
    print_success("AAAA record deleted");

    let after_delete = dns_store
        .get_record("api.example.com", RecordType::AAAA)
        .await
    ...
    let ticket =
        DnsClientTicket::new("test-cluster", "namespace123", vec![service.endpoint_addr()])
            .with_priority(5)
...
async fn test_dns_replication_three_nodes() {
    ...
    dns_store_1
        ...
        .await
        .expect("failed to set record on leader");
    print_record(&record);
