...
use support::bolero_generators::{ValidClusterId, ValidTimeoutMs};
...
fn test_config_toml_roundtrip() {
    check!()
        .with_iterations(30)
        .with_type::<(u32, ValidTimeoutMs)>()
        .for_each(|(node_id, heartbeat)| {
            // Use u32 to ensure it fits in i64 range for TOML
            let node_id = 1 + (*node_id % 1_000_000) as u64;
            ...
            let parsed: NodeConfig =
                toml::from_str(&reserial).expect("Failed to parse re-serialized config from TOML");
