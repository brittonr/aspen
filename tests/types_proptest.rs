...
use aspen::raft::types::AppResponse;
use aspen::raft::types::NodeId;
use aspen::raft::types::RaftMemberInfo;
use bolero::check;
use support::bolero_generators::BalancedAppRequest;
use support::bolero_generators::InvalidNodeIdString;
use support::bolero_generators::ValidNodeIdString;

/// NodeId Display -> FromStr roundtrip should be identity.
#[test]
fn test_node_id_display_fromstr_roundtrip() {
    check!().with_iterations(1000).with_type::<u64>().for_each(|id| {
...
fn test_node_id_ordering() {
    check!().with_iterations(1000).with_type::<(u64, u64)>().for_each(|(a, b)| {
...
fn test_node_id_valid_string_parsing() {
    check!().with_iterations(1000).with_type::<ValidNodeIdString>().for_each(|s| {
...
fn test_node_id_invalid_string_parsing() {
    check!().with_iterations(1000).with_type::<InvalidNodeIdString>().for_each(|s| {
        let result = NodeId::from_str(&s.0);
        assert!(result.is_err(), "Invalid string '{}' should fail parsing", s.0);
    });
}
    ...
    check!().with_iterations(1000).with_type::<BalancedAppRequest>().for_each(|request| {
        ...
        match &request.0 {
            AppRequest::Set { key, value } => {
                ...
                assert!(displayed.contains("Set"), "Display should contain variant name");
            }
            AppRequest::SetMulti { pairs } => {
                assert!(displayed.contains("SetMulti"), "Display should contain variant name");
            ...
            AppRequest::Delete { key } => {
                assert!(displayed.contains(key), "Display should contain key");
                assert!(displayed.contains("Delete"), "Display should contain variant name");
            }
            AppRequest::DeleteMulti { keys } => {
                assert!(displayed.contains("DeleteMulti"), "Display should contain variant name");
            ...
            AppRequest::CompareAndSwap { key, new_value, .. } => {
                assert!(displayed.contains(key), "Display should contain key");
                assert!(displayed.contains(new_value), "Display should contain new_value");
                assert!(displayed.contains("CompareAndSwap"), "Display should contain variant name");
            }
            AppRequest::CompareAndDelete { key, expected } => {
                assert!(displayed.contains(key), "Display should contain key");
                assert!(displayed.contains(expected), "Display should contain expected");
                assert!(displayed.contains("CompareAndDelete"), "Display should contain variant name");
            }
            AppRequest::Batch { operations } => {
                assert!(displayed.contains("Batch"), "Display should contain variant name");
            ...
            AppRequest::ConditionalBatch { conditions, operations } => {
                assert!(displayed.contains("ConditionalBatch"), "Display should contain variant name");
            ...
            AppRequest::SetWithTTL { key, value, .. } => {
                assert!(displayed.contains(key), "Display should contain key");
                assert!(displayed.contains(value), "Display should contain value");
                assert!(displayed.contains("SetWithTTL"), "Display should contain variant name");
            }
            AppRequest::SetMultiWithTTL { .. } => {
                assert!(displayed.contains("SetMultiWithTTL"), "Display should contain variant name");
            }
            // Lease variants - not generated by BalancedAppRequest
            AppRequest::SetWithLease { key, value, .. } => {
                assert!(displayed.contains(key), "Display should contain key");
                assert!(displayed.contains(value), "Display should contain value");
                assert!(displayed.contains("SetWithLease"), "Display should contain variant name");
            }
            AppRequest::SetMultiWithLease { .. } => {
                assert!(displayed.contains("SetMultiWithLease"), "Display should contain variant name");
            }
            AppRequest::LeaseGrant { lease_id, ttl_seconds } => {
                assert!(displayed.contains("LeaseGrant"), "Display should contain variant name");
            ...
            AppRequest::LeaseRevoke { lease_id } => {
                assert!(displayed.contains("LeaseRevoke"), "Display should contain variant name");
                assert!(displayed.contains(&lease_id.to_string()));
            }
            AppRequest::LeaseKeepalive { lease_id } => {
                assert!(displayed.contains("LeaseKeepalive"), "Display should contain variant name");
                assert!(displayed.contains(&lease_id.to_string()));
            }
            AppRequest::Transaction {
                ...
                assert!(displayed.contains("Transaction"), "Display should contain variant name");
            ...
            AppRequest::OptimisticTransaction { read_set, write_set } => {
                assert!(displayed.contains("OptimisticTransaction"), "Display should contain variant name");
            ...
            // Shard topology operations - not generated by BalancedAppRequest
            AppRequest::ShardSplit { source_shard, .. } => {
                assert!(displayed.contains("ShardSplit"), "Display should contain variant name");
                assert!(displayed.contains(&source_shard.to_string()));
            }
            AppRequest::ShardMerge { source_shard, .. } => {
                assert!(displayed.contains("ShardMerge"), "Display should contain variant name");
                assert!(displayed.contains(&source_shard.to_string()));
            }
...
fn test_app_response_serde_json_roundtrip() {
    check!().with_iterations(1000).with_type::<(Option<String>, Option<bool>, Option<bool>)>().for_each(
        |(value, deleted, cas_succeeded)| {
...
mod unit_tests {
    ...
    fn test_app_request_display_formatting() {
        ...
        let delete = AppRequest::Delete { key: "baz".to_string() };
