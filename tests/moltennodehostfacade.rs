...
fn accepts_new_path_type(_: &molten_node_host::node_state::RelativePath) {}
fn accepts_old_path_type(_: &molten::node_state::RelativePath) {}
...
fn root_facades_preserve_exact_node_host_types_and_behavior() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let path = molten_node_host::node_state::RelativePath::parse("receipts/marker.bin")?;
    accepts_new_path_type(&path);
    accepts_old_path_type(&path);

    let directory = cap_tempfile::tempdir(cap_tempfile::ambient_authority())?;
    let root = molten::node_state::Root::from_dir(directory.try_clone()?);
    root.create_layout()?;
...
fn old_and_new_paths_deny_the_same_invalid_locator_class() -> Result<(), Box<dyn std::error::Error>> {
    // r[verify molten.node_host.facade_compatibility]
    let new_error = match molten_node_host::node_state::RelativePath::parse("../escape") {
        Ok(_) => return Err(std::io::Error::other("new path admitted parent traversal").into()),
        Err(error) => error,
    };
    let old_error = match molten::node_state::RelativePath::parse("../escape") {
