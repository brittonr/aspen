...
use support::bolero_generators::ValidIpAddr;

// Test 1: TUI RPC Request serialization roundtrip
#[test]
fn test_tui_request_roundtrip_simple_variants() {
    check!().with_iterations(100).with_type::<u8>().for_each(|seed| {
        ...
        // Deserialize
        let roundtripped: ClientRpcRequest = postcard::from_bytes(&bytes).expect("Failed to deserialize request");

        // Re-serialize and compare bytes (idempotency)
        let bytes2 = postcard::to_stdvec(&roundtripped).expect("Failed to re-serialize request");
...
fn test_tui_request_read_key_roundtrip() {
    check!().with_iterations(50).with_type::<ValidApiKey>().for_each(|key| {
        ...
        match roundtripped {
            ClientRpcRequest::ReadKey { key: roundtripped_key } => {
...
fn test_tui_request_write_key_roundtrip() {
    check!().with_iterations(50).with_type::<(ValidApiKey, Vec<u8>)>().for_each(|(key, value)| {
        ...
        let bytes = postcard::to_stdvec(&request).expect("Failed to serialize WriteKey request");
...
fn test_tui_request_add_learner_roundtrip() {
    check!().with_iterations(50).with_type::<(u64, ValidIpAddr)>().for_each(|(node_id, addr)| {
        ...
        let bytes = postcard::to_stdvec(&request).expect("Failed to serialize AddLearner request");
...
fn test_tui_request_change_membership_roundtrip() {
    check!().with_iterations(50).with_type::<MembershipList>().for_each(|members| {
        let request = ClientRpcRequest::ChangeMembership {
            members: members.0.clone(),
        };

        let bytes = postcard::to_stdvec(&request).expect("Failed to serialize ChangeMembership request");

        let roundtripped: ClientRpcRequest =
            postcard::from_bytes(&bytes).expect("Failed to deserialize ChangeMembership request");
...
fn test_tui_response_pong_roundtrip() {
    check!().with_iterations(50).with_type::<u8>().for_each(|seed| {
        ...
        let roundtripped: ClientRpcResponse = postcard::from_bytes(&bytes).expect("Failed to deserialize response");

        // Re-serialize to verify idempotency
        let bytes2 = postcard::to_stdvec(&roundtripped).expect("Failed to re-serialize response");
...
fn test_serialized_size_bounded() {
    check!().with_iterations(30).with_type::<(ValidApiKey, Vec<u8>)>().for_each(|(key, value)| {
        let value: Vec<u8> = value.iter().take(1000).copied().collect();
...
fn test_empty_value_handling() {
    check!().with_iterations(10).with_type::<ValidApiKey>().for_each(|key| {
        let request = ClientRpcRequest::WriteKey {
            key: key.0.clone(),
            value: vec![], // Empty value
        };

        let bytes = postcard::to_stdvec(&request).expect("Failed to serialize request with empty value");

        let roundtripped: ClientRpcRequest =
            postcard::from_bytes(&bytes).expect("Failed to deserialize request with empty value");
...
fn test_large_membership_list() {
    check!().with_iterations(10).with_type::<u8>().for_each(|num_members| {
        ...
        let bytes = postcard::to_stdvec(&request).expect("Failed to serialize large membership");
...
fn test_binary_value_special_bytes() {
    check!().with_iterations(30).with_type::<ValidApiKey>().for_each(|key| {
        ...
        for value in special_values {
            ...
            let bytes = postcard::to_stdvec(&request).expect("Failed to serialize special bytes");

            let roundtripped: ClientRpcRequest =
                postcard::from_bytes(&bytes).expect("Failed to deserialize special bytes");

            match roundtripped {
                ClientRpcRequest::WriteKey { value: rt_value, .. } => {
