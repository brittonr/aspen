...
fn test_eventual_consistency_multi_node() {
    check!()
        ...
        .for_each(|writes| {
            ...
            rt.block_on(async {
                let (router, initial_log_index) = init_three_node_cluster()
                    .await
...
fn test_at_most_one_leader() {
    check!()
        ...
        .for_each(|num_writes| {
            ...
            let rt = tokio::runtime::Runtime::new().unwrap();
            rt.block_on(async {
                let (router, _initial_log_index) = init_three_node_cluster()
                    .await
                    .expect("Failed to init cluster");
                ...
                for i in 0..num_writes {
                    ...
                    for node_id in 0..3 {
                        let node_handle =
                            router.get_raft_handle(node_id).expect("failed to get node");
    ...
    check!()
        ...
        .for_each(|(key, v1, v2, v3, v4)| {
            ...
            rt.block_on(async {
                ...
                for value in &values {
                    router
                        .write(leader, key.0.clone(), value.clone())
                        .await
                        .expect("write failed");
                }
    ...
    check!()
        ...
        .for_each(|writes| {
            ...
            rt.block_on(async {
                ...
                let leader_node = router
                    .get_raft_handle(leader)
    ...
    check!()
        ...
        .for_each(|(num_writes_before, num_writes_after)| {
            ...
            rt.block_on(async {
                ...
                let (mut router, initial_log_index) = init_three_node_cluster()
                    .await
                    .expect("Failed to init cluster");
                ...
                for i in 0..num_writes_before {
                    ...
                    router
                        ...
                        .await
                        .expect("pre-failure write failed");
                }
...
fn test_network_partition_recovery() {
    check!()
        .with_iterations(20)
        .with_type::<u8>()
        .for_each(|num_writes_during_partition| {
            ...
            rt.block_on(async {
                ...
                let minority_node: NodeId =
                    (0..3).map(NodeId::from).find(|n| *n != leader).unwrap();
