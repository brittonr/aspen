...
async fn test_five_node_cluster_with_concurrent_failures_seed_42() {
    let seed = 42_u64;
    let mut artifact =
        SimulationArtifactBuilder::new("madsim_5node_concurrent_failures", seed).start();
    ...
    let raft4 = create_raft_node(4, router.clone(), injector.clone()).await;
    let raft5 = create_raft_node(5, router.clone(), injector.clone()).await;
    ...
    router
        .register_node(4, "127.0.0.1:26004".to_string(), raft4.clone())
        .expect("failed to register node 4");
    router
        .register_node(5, "127.0.0.1:26005".to_string(), raft5.clone())
    ...
    assert!(
        ...
        "leader should have applied entries"
    );
...
async fn test_rolling_failures_seed_123() {
    ...
    let has_leader = metrics1.current_leader.is_some()
...
async fn test_asymmetric_partition_seed_456() {
    ...
    injector.set_message_drop(1, 2, true);
    injector.set_message_drop(2, 1, true);
...
async fn test_concurrent_writes_complex_failures_seed_789() {
    let seed = 789_u64;
    let mut artifact =
        SimulationArtifactBuilder::new("madsim_concurrent_writes_complex", seed).start();
    ...
    injector.set_network_delay(1, 2, 500);
    injector.set_network_delay(2, 1, 500);
    injector.set_network_delay(2, 3, 500);
    injector.set_network_delay(3, 2, 500);
