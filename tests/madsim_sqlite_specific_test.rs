...
async fn test_sqlite_restart_recovery_seed_42() {
    ...
    let raft1 =
        create_raft_node_sqlite(1, "restart_seed_42", router.clone(), injector.clone()).await;
    let raft2 =
        create_raft_node_sqlite(2, "restart_seed_42", router.clone(), injector.clone()).await;
    let raft3 =
        create_raft_node_sqlite(3, "restart_seed_42", router.clone(), injector.clone()).await;
    ...
    madsim::time::sleep(std::time::Duration::from_millis(2000)).await;
    router.mark_node_failed(1, false);
...
async fn test_sqlite_wal_checkpoint_seed_123() {
    ...
    assert!(
        ...
        "leader should have applied all writes"
    );
    assert!(
...
async fn test_sqlite_large_dataset_seed_456() {
    ...
    assert!(
        ...
        "node should have applied large dataset"
    );
    assert!(
    ...
    artifact = artifact.add_event(format!(
        "validation: SQLite handled {} entries efficiently",
        DATASET_SIZE
    ));
...
async fn test_sqlite_database_isolation_seed_789() {
    ...
    let raft1 =
        create_raft_node_sqlite(1, "isolation_seed_789", router.clone(), injector.clone()).await;
    let raft2 =
        create_raft_node_sqlite(2, "isolation_seed_789", router.clone(), injector.clone()).await;
    let raft3 =
        create_raft_node_sqlite(3, "isolation_seed_789", router.clone(), injector.clone()).await;
    ...
    assert!(
        metrics1_final.last_applied.is_some(),
        "node 1 should have applied entries to its SQLite db"
    );
    assert!(
        metrics2_final.last_applied.is_some(),
        "node 2 should have applied entries to its SQLite db"
    );
    assert!(
        metrics3_final.last_applied.is_some(),
        "node 3 should have applied entries to its SQLite db"
    );
