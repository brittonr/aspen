...
async fn test_follower_clear_restart_recover_seed_2001() {
    let seed = 2001_u64;
    let mut artifact = SimulationArtifactBuilder::new("madsim_follower_clear_restart_recover", seed).start();
    ...
    router
        .register_node(NodeId::from(0), "127.0.0.1:27000".to_string(), raft0.clone())
        .expect("failed to register node 0");
    router
        .register_node(NodeId::from(1), "127.0.0.1:27001".to_string(), raft1.clone())
        .expect("failed to register node 1");
    router
        .register_node(NodeId::from(2), "127.0.0.1:27002".to_string(), raft2.clone())
    ...
    raft0.initialize(nodes).await.expect("failed to initialize cluster");
    ...
    artifact = artifact.add_event(format!("metrics: node 1 has log_index={:?} before shutdown", log_index_before));
    ...
    let raft1_new = create_raft_node(NodeId::from(1), router.clone(), injector.clone(), config.clone()).await;
    ...
    router
        .register_node(NodeId::from(1), "127.0.0.1:27001".to_string(), raft1_new.clone())
    ...
    artifact = artifact
        .add_event(format!("validation: node 1 has empty log after restart, log_index={:?}", log_index_after_restart));
    assert_eq!(log_index_after_restart, None, "node 1 should have empty log after restart");

    artifact = artifact.add_event("trigger: heartbeat from node 0 to discover state loss");
    raft0.trigger().heartbeat().await.expect("heartbeat trigger should succeed");
    ...
    artifact = artifact.add_event(format!("validation: node 1 recovered log, log_index={:?}", log_index_recovered));

    assert!(log_index_recovered.is_some(), "node 1 should have recovered log entries");
    assert_eq!(log_index_recovered, log_index_before, "node 1 should have same log index as before restart");
...
async fn test_append_entries_backoff_seed_2002() {
    let seed = 2002_u64;
    let mut artifact = SimulationArtifactBuilder::new("madsim_append_entries_backoff", seed).start();
    ...
    artifact = artifact.add_event("register: all nodes with router");
    router
        .register_node(NodeId::from(0), "127.0.0.1:29000".to_string(), raft0.clone())
        .expect("failed to register node 0");
    router
        .register_node(NodeId::from(1), "127.0.0.1:29001".to_string(), raft1.clone())
        .expect("failed to register node 1");
    router
        .register_node(NodeId::from(2), "127.0.0.1:29002".to_string(), raft2.clone())
        .expect("failed to register node 2");
    ...
    for i in 0..10 {
        ...
        assert!(result.is_ok(), "write {} should succeed with majority available", i);
    }
    ...
    assert!(log_index_0.is_some() && log_index_0.unwrap() >= 10, "node 0 should have >= 10 log entries");
    assert!(log_index_1.is_some() && log_index_1.unwrap() >= 10, "node 1 should have >= 10 log entries");
    ...
    artifact = artifact.add_event("success: backoff mechanism allows cluster to function despite unreachable node");
