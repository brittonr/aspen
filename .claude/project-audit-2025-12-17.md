...
**Location**: `src/bin/aspen-node.rs:539-563`

All HTTP endpoints are world-accessible if listening on non-loopback interface:
- POST `/cluster/init` - Initialize cluster (critical operation)
- POST `/cluster/add-learner` - Add nodes
- POST `/cluster/change-membership` - Modify cluster topology
- POST `/kv/write` / `/kv/delete` - Modify data

**Current Mitigation**: Default binding is `127.0.0.1:8080` (safe). HTTP API is documented as deprecated in favor of Iroh P2P.

**Recommendation**: Add bearer token auth or mTLS if exposing beyond loopback.
...
| src/bin/ | 4,434 | A- | HTTP API needs auth if exposed, good TUI implementation |
...
2. **No Rate Limiting on HTTP API** - `src/bin/aspen-node.rs:540-563` - Large write operations could exhaust consensus bandwidth
3. **No CORS Headers** - `src/bin/aspen-node.rs:88-99` - Browser-based attacks possible if API exposed publicly
...
- Default loopback binding for HTTP API
...
1. **Add authentication** if HTTP API exposed beyond loopback
2. **Change default cookie** enforcement from warning to error
3. **Sign gossip messages** with Iroh secret key

### Short-Term (Next Release)
4. Implement generic error responses for HTTP API
...
8. Consider rate limiting for HTTP API
...
- **Public Networks**: Requires authentication implementation for HTTP API and gossip message signing

The main areas for improvement are:
1. Authentication for exposed APIs
