...
fn bench_parallel_writers(c: &mut Criterion) {
    ...
    for num_writers in [2usize, 4, 8] {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(num_writers), &num_writers, |b, &writers| {
            b.to_async(&rt).iter(|| {
                    ...
                    for _writer_id in 0..writers {
                        ...
                        handles.push(tokio::spawn(async move {
                            for _ in 0..OPS_PER_TASK {
                                let key = format!("concurrent-{:012}", counter.fetch_add(1, Ordering::Relaxed));
            ...
            b.to_async(&rt).iter(|| {
                ...
                async move {
                    ...
                    for _reader_id in 0..readers {
                        ...
                        handles.push(tokio::spawn(async move {
                            for _ in 0..OPS_PER_TASK {
                                let idx = counter.fetch_add(1, Ordering::Relaxed) % KEY_SPACE as u64;
...
fn bench_mixed_concurrent(c: &mut Criterion) {
    ...
    for (readers, writers) in [(4, 2), (2, 4), (4, 4)] {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(&config_name), &(readers, writers), |b, &(r, w)| {
            b.to_async(&rt).iter(|| {
                let router = router.clone();
                ...
                async move {
                    ...
                    for _ in 0..w {
                        ...
                        handles.push(tokio::spawn(async move {
                            for _ in 0..OPS_PER_TASK {
                                let key = format!("mixed-{:012}", counter.fetch_add(1, Ordering::Relaxed));
                                router.write(leader, key, value.clone()).await.unwrap();
                            }
                        }));
                    }
...
criterion_group!(concurrency_benches, bench_parallel_writers, bench_parallel_readers, bench_mixed_concurrent,);
