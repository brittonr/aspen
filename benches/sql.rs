...
//!
//! NOTE: This benchmark is only compiled when the `sql` feature is enabled.
//! Without it, a stub main function is provided.

// When sql feature is enabled, include the full benchmark suite
#[cfg(feature = "sql")]
mod sql_benchmarks {
    use std::sync::Arc;
    ...
    pub fn bench_sql_select_all(c: &mut Criterion) {
    ...
    pub fn bench_sql_scan_sizes(c: &mut Criterion) {
        let rt = tokio::runtime::Builder::new_multi_thread().enable_all().build().expect("runtime");
        ...
        for size in [100, 1000, 5000, 10000] {
            ...
            group.bench_with_input(BenchmarkId::from_parameter(size), &size, |b, _| {
                b.to_async(&rt).iter(|| {
                    let exec = &executor;
                    async move {
                        exec.execute("SELECT * FROM kv", &[], Some(10000), Some(30000)).await.expect("query failed")
                    }
                })
            });
        }
    ...
    pub fn bench_sql_exact_key(c: &mut Criterion) {
        let rt = tokio::runtime::Builder::new_multi_thread().enable_all().build().expect("runtime");
    ...
    pub fn bench_sql_prefix_scan(c: &mut Criterion) {
        let rt = tokio::runtime::Builder::new_multi_thread().enable_all().build().expect("runtime");
    ...
    pub fn bench_sql_range_query(c: &mut Criterion) {
        let rt = tokio::runtime::Builder::new_multi_thread().enable_all().build().expect("runtime");
    ...
    pub fn bench_sql_count(c: &mut Criterion) {
        let rt = tokio::runtime::Builder::new_multi_thread().enable_all().build().expect("runtime");
    ...
    pub fn bench_sql_count_filtered(c: &mut Criterion) {
        let rt = tokio::runtime::Builder::new_multi_thread().enable_all().build().expect("runtime");
    ...
    pub fn bench_sql_projection(c: &mut Criterion) {
        let rt = tokio::runtime::Builder::new_multi_thread().enable_all().build().expect("runtime");
    ...
    pub fn bench_sql_order_by(c: &mut Criterion) {
        let rt = tokio::runtime::Builder::new_multi_thread().enable_all().build().expect("runtime");
    ...
    pub fn bench_sql_limit(c: &mut Criterion) {
        let rt = tokio::runtime::Builder::new_multi_thread().enable_all().build().expect("runtime");
    ...
    pub fn bench_sql_vs_raw_redb(c: &mut Criterion) {
        let rt = tokio::runtime::Builder::new_multi_thread().enable_all().build().expect("runtime");
...
// When sql feature is enabled, use the benchmark main
#[cfg(feature = "sql")]
criterion::criterion_main!(sql_benchmarks::sql_benches);

// When sql feature is disabled, provide a stub main
#[cfg(not(feature = "sql"))]
fn main() {
    eprintln!("SQL benchmarks are disabled. Enable the 'sql' feature to run them.");
    eprintln!("Run with: cargo bench --bench sql --features sql");
}
