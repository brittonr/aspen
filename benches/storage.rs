...
use common::{generate_value, setup_sqlite_state_machine, setup_sqlite_state_machine_with_pool};
...
fn populate_sqlite_prefix_data(
...
fn bench_sqlite_scan(c: &mut Criterion) {
    ...
    for prefix_id in [0, 5, 9] {
        ...
        group.bench_with_input(
            BenchmarkId::new("prefix", prefix_id),
            &prefix,
            |b, prefix| {
                b.to_async(&rt).iter(|| {
                    let sm = sm.clone();
