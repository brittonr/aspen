# NixOS VM integration test for Aspen Federation Blob Transfer.
#
# Two-cluster test: Alice creates a repo with git objects, federates it.
# Bob discovers via sync, fetches refs + git objects, verifying that
# a mirror repo is created and objects are imported. Then Bob does an
# incremental pull after Alice adds a second commit.
#
# Run:
#   nix build .#checks.x86_64-linux.federation-blob-transfer-test
{
  ...
  aliceSecretKey = "a11ce00000000003a11ce00000000003a11ce00000000003a11ce00000000003";
  bobSecretKey = "b0b0000000000004b0b0000000000004b0b0000000000004b0b0000000000004";

  aliceCookie = "alice-blob-xfer-test";
  bobCookie = "bob-blob-xfer-test";
...
in
  pkgs.testers.nixosTest {
    ...
    testScript = ''
      ...
      # ── alice creates a repo and pushes a commit ─────────────────────

      with subtest("alice creates repo"):
          out = cli(alice, "git init blobtest --description 'Blob transfer test'")
      ...
      with subtest("alice pushes initial commit"):
          alice.succeed(
              "mkdir -p /tmp/blobtest && cd /tmp/blobtest && git init && "
              "git config user.email 'alice@test' && "
              "git config user.name 'Alice' && "
              "echo 'hello federation blobs' > README.md && "
              "git add . && "
              "git commit -m 'initial commit with blob content'"
          )
      ...
      # ── bob syncs and fetches with git objects ───────────────────────

      with subtest("bob discovers alice's resources"):
          sync_result = cli(bob,
          ...
          assert len(resources) >= 1, f"expected resources: {resources}"

          alice_fed_id = resources[0].get("fed_id")
          assert alice_fed_id, f"no fed_id: {resources[0]}"
          bob.log(f"Alice's fed_id: {alice_fed_id}")

      with subtest("bob fetches refs and git objects"):
          ...
          bob.log(f"Fetched: {fetched}, already_present: {fetch_result.get('already_present', 0)}")
          # With git-bridge enabled, fetched should include refs + git objects
          assert fetched >= 1, f"expected at least 1 object fetched, got {fetched}"

      # ── verify mirror metadata in bob's KV ──────────────────────────
      ...
      with subtest("bob has mirror metadata"):
          ...
          if isinstance(scan_result, dict):
              ...
              # Should have at least one _fed:mirror: entry (the metadata)
              assert len(entries) >= 1, \
                  f"expected mirror metadata entries, got {len(entries)}"

      # ── done ─────────────────────────────────────────────────────────
      alice.log("Federation blob transfer test passed!")
      bob.log("Federation blob transfer test passed!")
    '';
  }
