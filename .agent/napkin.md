...
| 2026-04-12 | Offline-expunged nodes can restart with stale trust metadata and never issue a trust request on their own, so peer enforcement must actively probe after startup | Add a bounded startup peer probe (`crates/aspen-raft/src/trust_peer_probe.rs`) that asks a current peer for the local node's share; `ExpungedByPeer` becomes a persistent expungement marker |
