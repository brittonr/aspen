...
- Per-crate test via unit2nix NOT YET WORKING — `--include-dev` only captures root crate dev-deps. Per-crate clippy works (`u2nWorkspace.clippy.allWorkspaceMembers`). All test runs still use crane's monolithic `cargoNextest` (ci-nix profile). Needs either per-crate build plans or upstream `--workspace` dev-dep support in unit2nix.
