...
| 2026-03-30 | Federation sync completes (66K objects) but `handle_git_bridge_fetch` returns 0 objects — mirror SHA-1 lookup fails for HEAD commit | **Open**: federation_import_objects builds SHA-1→BLAKE3 mappings but `translate_ref_hashes` may not map HEAD commit SHA-1 to the correct mirror BLAKE3. The `sha1_to_local_blake3` in import stats uses import-time SHA-1 (from re-parsed content), which differs from the original git SHA-1 that git-remote-aspen sends as `want` |
