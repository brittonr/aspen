...
| 2026-02-24 | self | WASM coordination plugin fails to load: "GuestError: Host function vector parameter missing length" | Pre-existing hyperlight-wasm ABI compatibility issue. Plugin installs fine (KV write works), but hyperlight can't instantiate the WASM module. Native handler tests all pass. |
