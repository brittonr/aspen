...
| 2026-03-12 | self | Deploy executor resolves `output_paths[0]` from build job, which can be `cowsay-3.8.4-man` (man pages multi-output) instead of the actual binary | Deploy artifact resolution needs to iterate outputs and probe for `bin/aspen-node` or use the un-suffixed output. Same bug as the 2026-03-10 cowsay ci-dogfood-test fix — deploy executor has its own separate artifact lookup. |
| 2026-03-12 | self | Deploy coordinator loses KV write race after leadership transfer — transfers leadership to node 2, transfer succeeds, but coordinator on node 1 tries to update deploy status in KV and gets "not leader" error, failing the deployment | Deploy coordinator should either: (1) detect post-transfer leader and forward the status update, or (2) have the new leader's deploy monitor pick up in-progress deployments and finalize them. The `found in-progress deployment` codepath exists but didn't trigger in serial-dogfood timing. |
