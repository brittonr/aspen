...
- [ ] [parallel] Add positive atomic promotion, complete reservation, safe retry, denied-after-promotion, and follow-up observation-commit fixtures. r[molten.world_promotion.verification]
  - Implemented: atomic promotion, complete reservations, safe retry, denied-after-promotion, receipt-last sidecars.
  - Blocked: the exact follow-up observation-commit compatibility fixture requires the published Weft effect-log cohort.
- [ ] [parallel] Add negative stale head, incomplete intent closure, reservation mismatch, simulated branch, denied capability, crash before commit, unknown commit, crash after dispatch, lost acknowledgment, duplicate response, conflicting observation, and exactly-once-overclaim fixtures. r[molten.world_promotion.verification]
  - Implemented: stale head, incomplete closure, reservation mismatch, simulation denial, capability denial, unknown commit/readback, lost acknowledgment, duplicate-risk acknowledgment, conflict, and overclaim denial.
  - Blocked: old-to-new Weft parity and the shared crash-boundary corpus cannot run before `weft-replay` exists at an immutable revision.
...
- [ ] [depends:world-promotion-verification] Run focused tests, Weft and reconciliation compatibility fixtures, Octet, Clippy with warnings denied, Cairn gates, lifecycle checks, and relevant Nix checks. r[molten.world_promotion.verification]
  - Passed: six pure-core tests, six shell tests, two CLI parser tests, strict Clippy, focused zero-finding Octet, source admission, release dependency, generated-plan drift, and Nix evaluation.
  - Blocked: Weft `main` at `8a89ffa9` has no runtime implementation or `weft-replay` crate. Its effect-log extraction task remains unchecked.
  - Bounded: full `nix flake check -L` also retains the inherited Cairn policy export drift already present on `origin/molten`.
