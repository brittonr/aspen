...
## Complete-cohort boundary review

The operator requested a review of a complete startup-relevant source gate, **not** permission to exclude the failing root crate. The normal `molten node run` and `molten node serve` commands are branches of the root `molten` binary (`src/main.rs`, `src/main/root.rs`, `src/cli/ops/node.rs`). The Nix package is `ws.workspaceMembers."molten".build` (`flake.nix`), not the separately packaged host library. That binary compiles a wide root CLI and `src/lib.rs` module graph, including the node, Octet, fabric, harness, WASM, and world source; it depends on both `molten-core` and `molten-node-host`. Runtime reachability of one node subcommand, or linker elimination of unused functions, is not proof that other compiled root source is outside this binary's input closure.

A read-only comparison of retained Rust compiler `.d` manifests from the cutover check found **1,048 distinct root `.rs` paths** across the normal `molten` library (834) and binary (214), plus 277 `molten-core` and 15 host `.rs` paths: 1,340 distinct first-party paths in those selected manifests. This is an observed compilation-input sample, **not** a complete release-build attestation or a claim about which symbols survive linking. The pure startup inventory fixture lists only 13 required Rust anchors plus five context members (`crates/molten-core/src/node_startup/tests.rs::source_fixture`); all 13 anchors occur in the observed 1,340 paths, leaving 1,327 observed paths outside that fixed list. The existing `source_context_and_raw_inventory_are_bound` test passes with those 13 anchors (`../final-checks.log`); a direct negative coverage comparison confirms that even `src/lib.rs` is absent. Successful fixture validation proves an exact selected inventory's shape and anchors, **not** equality to compiler inputs. The snapshot requires literal object-corpus source paths for its fixed anchors but likewise checks subset membership, not exact build closure (`src/octet/startup_snapshot.rs`). A new unlisted compiled module would not be caught by the fixed anchor list alone.

Octet delegates selection to `cargo check` with `-p molten -p molten-node-host --all-targets` and `DYLINT_NO_DEPS=1`. The separately compiled `molten-core` is a dependency, not a selected linted package; `--all-targets` may include test/fixture source absent from the normal executable and does not imply every feature, target, or build input completed after Cargo exited 101. Octet config/profile hashes bind `Cargo.toml`, `dylint.toml`, and effective arguments, but not source bytes, completed targets, build-script inputs, or the actual binary. The startup verifier checks the selected descriptor/member hashes and measures `/proc/self/exe`; it explicitly does **not** prove that Octet executed on the listed bytes or that those bytes built that executable (`src/node/startup_evidence.rs:67-74`). Moreover, the pure cohort pins Octet revision `c9b06bcf565c51d4a77d210e61b69ae51db9df25`, whereas the independently inspected local Octet worktree is `e4ecf888a0b419d0175d2fe1d748c24322ecce89`; this investigation does not identify the installed lint binary as the approved cohort.

**Boundary decision:** no sound narrower check exists for the **unchanged** normal-node executable. A complete future route must derive and bind the exact build's selected units, cfg/features/target, local source files (including `include!` and generated inputs), dependencies and build scripts; require the independently approved inventory to cover every relevant compiler input with byte identities; record completed checked units and checker/tool identities from actual strict execution; then bind the resulting build and measured executable digest. Missing/extra units or files, changed bytes, stale checker/build identities, or cold-versus-warm discrepancies must deny. Structural zero-count receipts and the 13 anchors are not that proof.

The two architectural routes left for owner decision are (1) keep the root `molten` binary and remediate its strict source findings while extending independently measured closure to the `molten-core` dependency, or (2) create a genuinely different node executable/package that does **not** compile the legacy root library, then prove the new binary's complete source/build/Octet closure and migrate actual run/serve consumers. Merely checking `-p molten-node-host`, changing `Cargo.toml` scope, or presenting a smaller source inventory against the old binary would be a gate bypass. The existing `src/octet/startup_snapshot.rs::explicit_metadata` also requires the original exact package/target arrays, so any valid policy change needs a reviewed code/spec/test cutover and a newly selected cohort. Neither route is implemented, passing, or approved; normal startup, VMs, and replay remain denied.
