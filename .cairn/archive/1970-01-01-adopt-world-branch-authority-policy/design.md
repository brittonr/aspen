...
- `promotion_gated` requires a complete committed reservation set and never authorizes dispatch.
...
### Decision: Promotion reservation admission is not dispatch

**Choice:** `PromotionReservationPort` supplies a typed observation from the Molten promotion boundary. The port has no effect-dispatch operation.

A pure adapter checks the exact promotion plan, complete committed reservation set, selected reservation, candidate head, and release branch class. The core binds those facts to the branch-authority plan and capability.

Any missing, crossed, incomplete, uncommitted, or dispatch-authorizing observation denies activation. Receipts retain identity metadata only.

**Rationale:** A policy receipt cannot become effect authority. The promotion transaction owns reservation admission, while the dispatcher retains a separate current-admission boundary.
