...
- [x] [serial] r[molten.evidence.chain_hashing.link_identity.stable] Compute link identity only from canonical Preserves bytes and keep payload refs unchanged by linking.
...
- [x] [parallel] r[molten.evidence.chain_hashing.append_receipts] Integrate signed receipt envelopes so append/verify receipts can be signed and links can payload signed receipt refs without changing subject hashes.
...
- [x] [parallel] r[molten.evidence.chain_hashing.verify_receipts.fork_diagnostic] Represent fork evidence and make production profiles reject unexpected forks while diagnostic profiles can retain fork artifacts.
...
- [x] [parallel] r[molten.evidence.chain_hashing.no_global_chain.concurrent_turns] Add optional actor/session turn-journal chains that bind input, admission, effect-log, trace, and before/after state refs without introducing a global actor-message head.
- [x] [parallel] r[molten.evidence.chain_hashing.ledger_index.rebuild] Add artifact/chunk publication lineage chains for manifests, catalog entries, and remote sync receipts.
- [x] [parallel] r[molten.evidence.chain_hashing.verify_receipts] Allow Iroh exchange of chain segments and checkpoints with local verification before ledger import.

## Phase 5: Tests

- [x] [serial] r[molten.evidence.chain_hashing.link_model.preserve_payload] Add tests that identical canonical links produce stable refs and that payload refs are preserved.
- [x] [serial] r[molten.evidence.chain_hashing.verify_receipts.prev_tamper] Add tests rejecting changed previous refs, sequence gaps, mismatched payload refs, stale heads, and unavailable payloads.
- [x] [serial] r[molten.evidence.chain_hashing.verify_receipts.fork] Add tests that two children from the same parent are detected and rejected under no-fork policy.
- [x] [parallel] r[molten.evidence.chain_hashing.control_plane_checkpoints.descends] Add tests for accepted anchors/checkpoints, checkpoint freshness, and Raft/Trellis control-plane checkpoint receipts.
- [x] [parallel] r[molten.evidence.chain_hashing.trellis_append_predicates.append] Add Hegel property tests over bounded chain segments for append determinism, no-gap validation, fork detection, and anchor descent.
