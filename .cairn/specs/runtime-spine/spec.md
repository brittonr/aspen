...
### Requirement: Blob-ref job submissions reject inline large bytes
...
### Requirement: Blob-ref content is verified before execution
...
### Requirement: Blob-ref executables carry provenance and policy refs
...
#### Scenario: Executable ref lacks provenance or policy refs
- GIVEN a ref-backed job submission whose executable ref lacks explicit provenance or effect/policy refs
- WHEN the deterministic local worker evaluates the run
- THEN Molten MUST NOT treat the run as admitted execution evidence

### Requirement: Blob-ref content refs are pinned while active
...
#### Scenario: Active pins are held and released with cleanup evidence
- GIVEN a ref-backed job with executable, input, and output refs
- WHEN the job is active and later releases its active pins
- THEN Molten MUST keep those refs pinned while the job is active and emit cleanup evidence when the pins are released
...
### Requirement: Blob-ref job receipts bind lifecycle evidence
...
#### Scenario: Receipt binds lifecycle evidence
...
### Requirement: Blob-ref job evidence enters replay identity
...
### Requirement: Blob-ref local job lifecycle is tested
...
#### Scenario: Local lifecycle test coverage
- GIVEN the ref-backed job test suite
- WHEN it runs
- THEN it MUST exercise submitting, fetching, verifying, running, and completing a local ref-backed job

### Requirement: Blob-ref invariants have property coverage
...
#### Scenario: Property coverage of blob-ref invariants
- GIVEN the ref-backed job property tests
- WHEN they run
- THEN they MUST cover the no-inline-large-bytes, content-verification-before-execution, and pin lifecycle invariants
